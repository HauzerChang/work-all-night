...
6. **收尾**:用清楚訊息 commit & **push 回你啟動這個 session 時所在的那條分支**(排程 trigger 會直接
   指向該開發分支,不走 PR/merge 流程;分支名以本 session 啟動時的實際分支為準,勿硬記舊名),
   讓下次排程從同分支接手時能讀到更新後的 `STATE.md`。然後結束。**不要嘗試無限長跑。**
