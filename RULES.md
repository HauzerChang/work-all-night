...
4. **推進一個有界工作塊**：只做一個階段裡的一個明確步驟(見「單次工作量」)，不要一次做完全部。
...
8. **寫執行紀錄**：在 `log/YYYY-MM-DD-NNN.md` 摘要這次做了什麼、產出、決策、下一步。
9. **commit & push**：清楚的訊息提交到開發分支，然後**結束 session**(不要嘗試無限長跑)。

## 單次工作量 (bounded chunk)

- 一次 session 推進**一個階段裡的一個明確步驟**，目標是「穩定、可續跑」而非「跑很遠」。
- 步驟太大 → 先在 `STATE.md` 拆成子步驟，這次只做第一個。
