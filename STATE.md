...
**S3 端到端對照 Award 真實 mesh(里程碑,2026-07-12)**:候選 1 完成。用 `robot_parts.psd`
的 3 件真實 weighted mesh(光暈/身體/左手)跑 S3 生成器,對照 `Award.json` 藝術家 mesh
→ **頂點數 ≤ 藝術家、IoU ≥ 藝術家 全 PASS + setup pose 幾何乾淨**(`validate_award_mesh.py --src atlas` exit 0)。
- **關鍵修正**:邊界容差要用**絕對像素**(`eps_px=3.5`)而非周長比例(v1 預設 0.008 對大件過度簡化,
  光暈覆蓋率掉到 0.929)。已把 `eps_px` 加進 `generate_mesh.generate()` 為向後相容 knob;
  4 mesh main_draw 重驗零回歸。
- footprint 一致性:權威判定用 atlas 切件(藝術家 uvs 為 region-local);psd 來源只當生成品質交叉檢查
  (藝術家基準因 frame 錯位偏低,不判定)。詳見 `knowledge/s3-award-mesh-groundtruth.md`。
- 尚缺:weighted mesh 的**權重(BBW)**未生成(目前只驗拓樸/覆蓋率)。
...
1. ~~PSD件→S3 mesh→對照 Award 真實 mesh~~ **✅ 完成(2026-07-12,見上)**。
2. **切圖→Spine JSON 組裝(SkelToJson)** ⬅ 建議下一個:把 `機器人拆件/<圖層名>` 命名慣例 +
   size+2px padding + mesh/region 分配 + `eps_px` 邊界規則固化成「件→Spine attachment」寫出工具,
   端到端產可載入的 Spine JSON。純 CPU。
3. **weighted mesh 權重(BBW)生成**:S3 目前只驗拓樸/覆蓋率,尚未生成骨骼綁定權重(S3 後段 ↔ S5 交界)。
...
- 2026-07-12:**S3 對照 Award 真實 mesh 端到端驗收(里程碑)** — `validate_award_mesh.py`:機器人 3 件
  真實 weighted mesh(光暈/身體/左手),生成 mesh 頂點數 ≤ 藝術家、IoU ≥ 藝術家全 PASS + 幾何乾淨。
  揪出並修「邊界容差用周長比例對大件過度簡化」bug → 改絕對像素 `eps_px=3.5`(加進 generate_mesh 為
  向後相容 knob,4 mesh main_draw 零回歸)。footprint 一致性:atlas 切件為權威、psd 為交叉檢查。
