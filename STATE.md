...
- **S3×S4 端到端真實驗收(里程碑,2026-07-12)** — PSD→件→mesh 對照 **Award 生產真值**:機器人 3 個 mesh 件
  (光暈/身體/左手)`validate_robot_parts.py` 全 `overall_pass`。這 3 件**無 deform**(骨驅)→ 改用
  靜態拓樸+覆蓋率 IoU 閘(變形閘 N/A)。發現預設 eps 對圓形邊界欠取樣 → 加 v2 **adaptive 邊界取樣**
  (opt-in,`adaptive=True,budget=64`,預設 off → main_draw 4 mesh 回歸零影響)。見 `knowledge/s3-robot-parts-real.md`。
...
1. ~~**PSD件→S3 mesh→對照 Award 真實 mesh**~~ **✅ 已完成(2026-07-12)** — `validate_robot_parts.py`
   對 3 個 mesh 件全 PASS;發現這 3 件無 deform → 用靜態拓樸+覆蓋率閘;加 v2 adaptive 邊界取樣解圓形欠取樣。
   後續可延伸:用 Award.png 做 **texture 級(RGB)對照**(目前只比 alpha 覆蓋率)。
2. **切圖→Spine JSON 組裝(建議下一步)**:把 `機器人拆件/<圖層名>` 命名慣例 + size+2px padding 固化成「件→Spine attachment」
...
- 2026-07-12:**S3×S4 端到端真實驗收(里程碑)** — `validate_robot_parts.py`:機器人 3 個 mesh 件對 Award
  生產真值全 `overall_pass`。這 3 件無 deform(骨驅)→ 用靜態拓樸+覆蓋率 IoU 閘(變形閘 N/A);
  近方形 → 全走 v1 Delaunay。發現預設 eps=0.008 對圓形(光暈)邊界欠取樣(hull14/IoU0.93)→
  加 v2 **adaptive 邊界取樣**(eps 階梯取「nv≤budget 的最細」,opt-in 預設 off)→ 光暈0.966/左手0.988/身體0.971 全過。
  main_draw 4 mesh v2 回歸全綠(零影響)。見 `knowledge/s3-robot-parts-real.md`。
