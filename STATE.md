...
**S3 已泛化到第二份生產資產 Award(里程碑,2026-08-17)**:v2 對 Award 3 個 **weighted** 機器人 mesh
(光暈/左手/身體)靜態 IoU 全達/超藝術家覆蓋率、頂點皆更省(見 `knowledge/s3-award-robot-generalization.md`)。
- 塊狀件(非 row-convex)→ v2 `auto` 正確回退 v1 Delaunay。
- **v1 改自適應邊界取樣**:`epsilon_frac="auto"`(掃 eps 至覆蓋率 ≥ `cover_target=0.98`),尺度無關,
  修掉舊固定 0.008 對複雜生產輪廓過粗的 ~5% 覆蓋缺口。main_draw 4 mesh 全 strip 不受影響;
  v1 curtain_left IoU 0.98→0.99 且 deform 仍 0 自交。工具 `tools/mesh_gen/compare_award_robot.py`(exit 0)。
- ⚠️ **weighted mesh 的 deform 閘尚未支援**(`deform_eval` 對 weighted vertices 格式會壞)→ 已列後續 chunk。

---
以下為前一里程碑(保留):**S3 推廣到全部 4 個 mesh(2026-06-26)**:整合 AC 跑 curtain_left/right + shadow/shadow2。
...
1. **❗最高優先(有真值可比):weighted-aware deform 閘**。補 `deform_eval` 對 weighted mesh 的解析
   (依權重把 bind 座標×骨變換合成 setup local → 套 deform offset),讓 Award 3 件的**真實 deform 穩健度**
   也能驗(目前只驗了靜態 IoU)。這是「S3 對 Award 完整對照」的最後一塊。
2. ~~PSD件→S3 mesh→對照 Award 真實 mesh~~:**✅ 已完成靜態部分(2026-08-17)**,見上里程碑。
...
- 2026-08-17:**S3 泛化到 Award weighted mesh(里程碑)** — v2 對 Award 3 機器人 mesh(光暈/左手/身體,
  皆 weighted)靜態 IoU 全達/超藝術家、頂點更省;塊狀件正確回退 v1。**v1 導入自適應 epsilon(尺度無關,
  cover_target 0.98)**修掉舊 0.008 對複雜輪廓 5% 覆蓋缺口;main_draw strip 不受影響、v1 curtain deform 仍乾淨。
  新增 `compare_award_robot.py`。發現 weighted mesh deform 閘待補(deform_eval 對 weighted 格式會壞)。
