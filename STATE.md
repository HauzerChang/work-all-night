...
- **S3 mesh 生成器：完成且對 4 個真實 mesh 收斂達標**(v2 strip 通用,見 `knowledge/s3-four-mesh-generalization.md`)。
  **並已端到端對照 Award 真實生產 mesh(2026-07-07)**:PSD 件(光暈/身體/左手)→ 生成 topology 覆蓋 ≥ 藝術家且更省頂點,3 件全 PASS(`validate_against_award.py`,見 `knowledge/s3-award-end-to-end.md`)。缺口=weighted 權重(BBW)。
...
**S3 端到端對照 Award 真實生產 mesh 完成(里程碑,2026-07-07)** — 候選 #1 已完成。
`validate_against_award.py`:PSD 件(光暈/身體/左手)→ `generate_mesh` v1 → 對照 Award 藝術家 mesh。
**3 件全 PASS**:生成覆蓋 IoU ≥ 藝術家(0.98/0.97/0.98 vs 0.95/0.95/0.98),且**更省頂點**
(64/60/70 vs 藝術家 78/98/80),幾何全乾淨。發現:
- Award 3 mesh 是 **weighted / 骨骼驅動 / 無 deform timeline**(另一 regime;main_draw 是 unweighted+deform)
  → 「真實位移場轉移」變形閘 **N/A**;公平比對維度 = 靜態覆蓋 + 頂點經濟性。
- **mesh uvs 存於邏輯 upright 空間**(與 atlas rotate 無關)→ 用**乾淨 PSD 件**當真值(frame_sanity 0.95~0.98 確認),比 atlas_crop(0.70縮小+derotate)精確。
- 覆蓋率由邊界取樣密度(Douglas-Peucker `epsilon_frac`)決定(呼應 v2「IoU 由 rows 決定」);工具沿 eps 階梯自主收斂。
- 詳見 `knowledge/s3-award-end-to-end.md`。

下一個 bounded chunk 候選:
1. **❗最高優先(端到端最後一塊):S3 BBW 權重**。Award 3 件是 weighted;S3 目前只產 unweighted topology。
   對生成 topology 算 BBW 骨骼權重(需 Award 骨架 + bind pose),與藝術家權重比對(權重平滑度 / 骨骼影響區 IoU)
   → 補齊 weighted mesh parity。純 CPU 可自驅。
2. **切圖→Spine JSON 組裝(SkelToJson)**:把 `機器人拆件/<圖層名>` 命名慣例 + size+2px padding + mesh/region 分配
   固化成「件→Spine attachment」寫出工具,把生成 mesh 掛回 Spine JSON。
...
> S3+S4 已端到端串起(拓樸維度對真實生產 mesh 驗收)。建議下一步:S3 BBW 權重(端到端最後一塊,
> 純 CPU 可自驅),或 SkelToJson 組裝工具(把件+mesh 寫回 Spine JSON)。
...
- 2026-07-07:**S3 端到端對照 Award 真實生產 mesh(里程碑)** — 候選 #1。`validate_against_award.py`:
  PSD 件(光暈/身體/左手)→ `generate_mesh` v1 → 對照 Award 藝術家 mesh。3 件全 PASS:生成覆蓋 IoU
  ≥ 藝術家(0.98/0.97/0.98 vs 0.95/0.95/0.98)且更省頂點(64/60/70 vs 78/98/80)、幾何乾淨。
  發現 Award 3 mesh 為 **weighted/骨骼驅動/無 deform**(另一 regime,變形閘 N/A);mesh uvs 在邏輯
  upright 空間(與 atlas rotate 無關)→ 用乾淨 PSD 件當真值(frame_sanity);覆蓋率由 Douglas-Peucker
  eps 決定,eps 階梯自主收斂(光暈 eps=0.004 覆蓋達標但重心閘擋下 → 續進 0.002)。
  端到端缺口 = BBW 權重(下一能力)。見 `knowledge/s3-award-end-to-end.md`。
