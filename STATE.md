...
- **S3×S4 端到端對照真實生產 mesh:完成(里程碑,2026-07-30)** — `compare_to_award_mesh.py` 把
  `robot_parts.psd`→件→`generate_mesh_v2`→對照 `Award.json` 的 `機器人拆件/{光暈,身體,左手}` 真實 mesh。
  **OVERALL PASS**:輪廓 IoU(uv+8向 dihedral,免骨頭變換)0.888/0.938/0.967,非對角僅 0.60–0.71(判別力確認);
  生成頂點僅藝術家 0.45–0.74×。光暈靜態覆蓋 0.933 為軟 alpha 已知限制(非 bug)。見 `knowledge/s3-psd-to-award-mesh.md`。
- S1 / S5 尚未開始。
...
1. ✅ **已完成(2026-07-30):PSD件→S3 mesh→對照 Award 真實 mesh**(輪廓 IoU 對真值 OVERALL PASS)。
2. **❗建議下一個:PSD件生成 mesh 的「真實 deform」耐變形對照**。本次只比 setup 輪廓;
   Award 的 `機器人拆件/{光暈,身體,左手}` 是 weighted 且有 deform timeline → 用 `deform_eval` 思路
   取 Award 真實位移場,對生成 mesh 做「真實位移場轉移」耐變形閘(0 自交/0 翻面)。⚠️ 用真實位移場,勿用未校準 stress_field。
3. **切圖→Spine JSON 組裝(SkelToJson)**:把 `機器人拆件/<圖層名>` 命名慣例 + size+2px padding
   固化成「件→Spine attachment」寫出工具,端到端產可載入的 Spine JSON。
...
> S3+S4 已串成端到端並對真實生產 mesh 驗收通過。建議下一步:(2) 把耐變形對照補上
> (setup 輪廓已過,缺 deform 這一維);或 (3) SkelToJson 把件+mesh 組成可載入 Spine JSON。
