...
**✅ 已完成(里程碑,2026-08-14):PSD件→S3 mesh→對照 Award 真實 artist mesh**
(`tools/mesh_gen/compare_award_mesh.py`,見 `knowledge/s3-psd-to-award-mesh.md`)。
- 光暈/身體/左手 3 件對 Award 藝術家 **weighted mesh** 做靜態 IoU 對照(這 5 件無 deform timeline)。
- 公平共同遮罩(真實 atlas region alpha)下:eps=0.004 時 **3/3 落在 artist ±1.4% 且頂點更少**
  (61/69/57 vs 78/98/80);生成器預設 eps=0.008 對大平滑件(光暈)過度簡化,2/3 過。
- **兩個方法論校正**:①Award JSON mesh `uvs` = region 局部正規化 [0,1](非 page;weighted 的 vertices
  是權重格式不能當座標,setup 形狀從 uvs 還原,`u*W,v*H`,自檢 IoU 0.97);②生成與評分須用**同一張
  遮罩**(在 PSD 件生成卻對 region 評分 → 0.70 縮放羽化差注入 ~5% 假 gap)。

下一個 bounded chunk 候選:
1. **❗生成器自適應 epsilon**(接續本次發現):給 v1 加曲率/面積自適應 epsilon(或依 hull 目標點數反解),
   讓**單一預設**對「小 strip 件」與「大平滑件」都達標。**改預設前必須對 main_draw 4 mesh + slicing
   跑回歸**(本次只新增檔、未改共用碼)。做完可讓 `compare_award_mesh` 預設即 3/3。
2. **切圖→Spine JSON 組裝(SkelToJson)**:把 `機器人拆件/<圖層名>` 命名慣例 + size+2px padding +
   mesh/region 分配 + 生成 mesh(uvs 為 region 局部正規化,回寫時直接放 0..1)固化成「件→Spine attachment」
   工具,端到端產 Spine JSON。**本次已釐清 uvs 語意 → 回寫格式明確**。
...
> S3+S4 已串成端到端並對真實藝術家 mesh 驗收通過。建議下一步取「生成器自適應 epsilon」(1)
> 或「SkelToJson 組裝」(2)——皆純 CPU 可自驅。
...
- 2026-08-14:**S3 端到端對照 Award artist mesh(里程碑)** — 新增 `compare_award_mesh.py`;PSD 3 mesh 件
  對 Award 藝術家 weighted mesh 靜態 IoU 對照,eps=0.004 時 3/3 落在 artist ±1.4% 且頂點更少。
  兩方法論校正:①Award mesh uvs 是 region 局部正規化(非 page,weighted 從 uvs 還原,u*W,v*H);
  ②生成與評分須同一張遮罩(否則 0.70 縮放羽化差注入 ~5% 假 gap)。發現預設 epsilon 對大平滑件過度簡化
  → 下一步「自適應 epsilon」。見 `knowledge/s3-psd-to-award-mesh.md`。
