...
- **S3×S4 端到端(里程碑,2026-07-30)** — `benchmark_psd_mesh.py` 把「PSD 切件 → 生成 mesh」串起來,
  對 `robot_parts.psd` 3 個 mesh 件(光暈/身體/左手)生成 mesh,**對照 Award 藝術家手做 mesh(有真值)**:
  覆蓋率 0.93~0.97 ≈ 藝術家、頂點少 26~55%、setup + 外來真實場(scale-normalized)0 自交,3 件全 `overall_pass`。
  第四次差點 miscalibration(藝術家 local vertices 非置中/非跨滿 wh → 覆蓋率改用 **uvs 基準**);雙負對照確認鑑別力。
  見 `knowledge/s3-psd-to-real-mesh.md`。
- S1 / S5 尚未開始。
...
1. ~~PSD件→S3 mesh→對照 Award 真實 mesh~~ **✅ 完成(2026-07-30)** — 見 `knowledge/s3-psd-to-real-mesh.md`、`benchmark_psd_mesh.py`。
2. **❗最高優先:切圖→Spine JSON 組裝(SkelToJson)**。把已驗證的慣例固化成一支工具:
   輸入 psd_slice 的件 + manifest → 每件生成 mesh(mesh 件)或建 region(剛體件)→ 依
   `<PSD名>/<圖層名>` 命名 slot、size(+2px padding)、uvs(x/W,y/H)、vertices(置中)組出**完整 Spine JSON**。
   驗收:對 `robot_parts.psd` 產出的 JSON 能被 `deform_eval`/`evaluate_mesh` 讀取且結構合法;件數/命名/size 對齊 Award。
   ⚠️ 若要對照 Award 既有 mesh 的 local 座標,需知其 attachment transform(本次用 uvs 繞開,SkelToJson 自產可直接置中)。
...
> S3×S4 端到端已對真實生產 mesh 驗收通過。建議下一步:(2) SkelToJson —— 把「件 → Spine attachment」
> 寫成工具,產出完整 Spine JSON,即打通「PSD → 可用 skeleton」。純 CPU 可自驅。
