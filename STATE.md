...
- **S3⇄S4 端到端(里程碑,2026-07-30):生成 mesh 對照 Award 真實生產 mesh(有真值)通過** —
  機器人拆件 光暈/身體/左手 3 件,以藝術家手做 mesh 為 IoU 真值,S3 **3/3 達藝術家級覆蓋率**
  (皆過 0.96、與藝術家差 ≤0.02),頂點只用 ~40–50%(37–38v vs 78–98v)。`tools/mesh_gen/compare_to_award.py`。
  發現**固定 epsilon 對大而彎件(光暈)欠取樣** → 新增 `generate_mesh.generate_adaptive()`
  (IoU 閘驅動自我收斂,只調 epsilon)。更正 uv 慣例:Award mesh uvs 為 **region-local、v 不翻**。
  見 `knowledge/s3-award-mesh-groundtruth.md`。
...
1. ~~PSD件→S3 mesh→對照 Award 真實 mesh~~ **✅ 完成(2026-07-30,見上)**。3/3 達藝術家級覆蓋率。
2. **❗建議下一步:切圖→Spine JSON 組裝(SkelToJson)**。把已驗證的慣例固化:
   `slot=<PSD名>/<圖層名>`、size+2px padding、mesh/region 分配、mesh 用 `generate_adaptive`,
   端到端從 PSD parts 產出可載入的 Spine JSON attachment 區塊。真值:對照 Award 的結構。
...
> S3⇄S4 端到端已對真實 mesh 有真值驗收通過(3/3)。建議下一步:把「件→Spine attachment」慣例
> 固化成 SkelToJson 組裝工具,mesh 件用 `generate_adaptive`,region 件記 size+padding+rotation。
