...
**S3 對照 Award 真實 mesh 端到端驗收通過(里程碑,2026-07-15)** — 見 `knowledge/s3-vs-award-real-mesh.md`。
- 拿 Award 機器人 3 個真實 mesh(光暈/左手/身體)當藝術家真值,S3 自動生成在同一 region 素材、
  同一像素空間比對:**覆蓋率不遜藝術家(0.9656/0.9602/0.968 vs 0.9795/0.9681/0.976)、頂點更精簡
  (61/48/61 vs 78/80/98)、足跡一致 IoU 0.94~0.96**。四條 AC 全過(`compare_to_award.py` overall_pass）。
- 座標框:`atlas_crop` 去旋轉裁 region(0.70 打包);藝術家 uvs 為 **region-local 0..1**(AC-A 0.97~0.98 實證)。
- 關鍵發現:**軟邊/凸形大件覆蓋率由 hull 取樣密度(epsilon)決定、非內部點**(與 strip「rows 決定」同構)。
  → `generate_mesh` 加**覆蓋率自我收斂**:未達 `target_iou`(0.95)就對半調細 epsilon 重生(呼應「每能力必配評估器」)。
- 這些 slot 全動畫**無 deform timeline**(weighted 供綁骨、非時間軸驅動)→ 本比對聚焦靜態覆蓋+拓樸。
- 回歸:refine 只動 v1 Delaunay 分支;main_draw 4 mesh(strip)IoU/si 與改前一致。

前一里程碑(2026-06-26):S3 推廣到全部 4 mesh。v1 散點不通用(真實 deform 自交);v2 strip 通用
(4 mesh 全乾淨,rows=10 預設)。`validate_against_real.py --gen v2` 對 4 mesh 全 overall_pass。

下一個 bounded chunk 候選:
1. **❗最高優先:切圖→Spine JSON 組裝(SkelToJson)**。把 `機器人拆件/<圖層名>` 命名慣例 + region-local uv +
   0.70 打包 + S3 生成的 mesh 固化成「PSD 件→可載入 Spine mesh attachment」寫出工具,端到端產 Spine JSON。
2. **S3 weighted 權重(BBW)**:目前 S3 產 unweighted;藝術家用 weighted(供綁骨變形)。加 BBW 權重是 S3 路線下一塊。
...
> S3 已對真實生產 mesh 端到端驗收。建議下一步:把 S4(切件)+S3(生成)串成 SkelToJson 寫出工具(候選 #1),
> 或補 BBW 權重讓 S3 mesh 可綁骨(候選 #2)。
...
- 2026-07-15:**S3 對照 Award 真實 mesh 端到端驗收(里程碑)** — 3 真實 mesh(光暈/左手/身體)當藝術家真值,
  S3 自動生成覆蓋率不遜藝術家(0.96~0.97)、頂點更精簡、足跡 IoU 0.94~0.96,四條 AC 全過(`compare_to_award.py`)。
  發現「軟邊件覆蓋率由 epsilon(hull 密度)決定」→ 生成器加覆蓋率自我收斂迴圈。回歸:main_draw 4 mesh(strip)不受影響。
