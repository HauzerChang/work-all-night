...
- **S3×S4 端到端串接:對真實生產標的驗收通過(里程碑,2026-07-10)** —
  `validate_psd_to_award.py` 把 psd_slice→generate_mesh_v2 串成端到端,對 Award 機器人 3 件真實藝術家
  mesh(光暈/身體/左手)靜態拓樸+覆蓋 **overall_pass**:生成 mesh 以**更少頂點**達**優於藝術家**覆蓋
  (IoU 0.98~0.99,頂點 72/67/75 vs 藝術家 78/98/80)。新增**評估器驅動自動精修**
  (`generate_mesh_v2._refine_delaunay`:epsilon 遞減至覆蓋達標,不依賴真值)。見
  `knowledge/s3-s4-psd-to-award-mesh.md`。標準指令 `python3 tools/mesh_gen/validate_psd_to_award.py`。
- S1 / S5 尚未開始。
...
1. ~~PSD件→S3 mesh→對照 Award 真實 mesh~~ **✅ 已完成(2026-07-10)** — 見上「S3×S4 端到端」里程碑。
2. **❗最高優先:切圖→Spine JSON 組裝(SkelToJson)**。把 `機器人拆件/<圖層名>` 命名慣例 + size+2px padding
   + 生成 mesh(warp 件)/ region(剛體件)分配,固化成「件→Spine attachment」寫出工具,端到端產可載入的
   Spine JSON。有 Award.json 當結構真值可對照(slot/attachment 命名、size)。純 CPU 可自驅。
...
> S3×S4 已端到端對真實標的驗收通過。建議下一步:(2) SkelToJson 組裝 —— 把「件→attachment」寫出工具,
> 端到端產 Spine JSON(有 Award 結構真值可對照)。之後 S3/S4/組裝就構成完整「PSD→Spine」pipeline 前半。
...
- 2026-07-10:**S3×S4 端到端對真實標的(里程碑)** — `validate_psd_to_award.py` 串 psd_slice→generate_mesh_v2,
  對 Award 機器人 3 件藝術家 mesh 靜態驗收 overall_pass:生成 mesh 更少頂點、更高覆蓋(IoU 0.98~0.99)。
  新增評估器驅動自動精修(epsilon 遞減至覆蓋達標,不依賴真值)。發現覆蓋率單調受 hull 密度支配;
  uvs 比對即使 atlas rotate:true 也對齊(baseline 合理性反查)。3 件無 deform → 靜態驗收,deform 閘仍以窗簾為準。
