...
- **S3×S4 端到端驗收通過(里程碑,2026-07-24)** — `compare_to_award_mesh.py` 把 `robot_parts.psd`
  的 光暈/身體/左手 3 件自動生成 mesh,對照 `Award.json` 藝術家 weighted mesh:形狀 IoU 0.886–0.968、
  覆蓋 0.933–0.966、**頂點數全 < 藝術家**、拓樸全乾淨;跨件負對照有鑑別力(同件≥0.886 > 跨件≤0.711)。
  印證「不規則件走 delaunay-v1、窗簾走 v2-strip」分工。只驗靜態(這 3 件是 weighted mesh,無 deform timeline)。
  見 `knowledge/s3-psd-to-award-mesh.md` + `figures/s3-vs-award-robot-mesh.png`。
- S1 / S5 尚未開始。
...
1. ~~PSD件→S3 mesh→對照 Award 真實 mesh~~ **✅ 完成(2026-07-24,見上)**。3 件形狀/覆蓋/預算/拓樸全過。
2. **切圖→Spine JSON 組裝(SkelToJson)**:把 `機器人拆件/<圖層名>` 命名慣例 + size+2px padding +
   mesh/region 分配 + atlas 0.70 縮放固化成「件→Spine attachment」寫出工具,端到端產 Spine JSON。
   (已有真值可比:對照 Award.json 的 slot/attachment 結構。)
3. **weighted mesh 骨骼權重轉移對照**:把上一課題的 S3 mesh 加 BBW 權重,對照 Award 三件的 bone binding,
   跑骨骼驅動變形對照(補上本次未做的 deform 面向;需解析 Award bone/weights)。
4. **S2 補圖閘 / 骨架閘**(補齊 S2 樞紐;純 CPU)。
5. **S1 反推分析器**:需一支 benchmark 影片(repo 無影片資產,屬使用者層級資源)。
6. ~~spine_inspector 實機 round-trip~~:**⛔ CDN(jsDelivr)被網路政策擋(403);需使用者改政策或提供離線 spine-webgl。**

> S3×S4 端到端(PSD→件→mesh)已對真實生產標的閉環驗收。建議下一步:(2) SkelToJson 件→Spine JSON 組裝
> (把慣例固化成工具,對照 Award 結構),或 (3) 補 weighted mesh 權重轉移的 deform 面向。
...
- 2026-07-24:**S3×S4 端到端驗收(里程碑)** — `compare_to_award_mesh.py`:robot_parts 光暈/身體/左手 3 件
  PSD→S3 mesh,對照 Award 藝術家 weighted mesh。形狀 IoU 0.886–0.968 / 覆蓋 0.933–0.966 / 頂點全 < 藝術家 /
  拓樸全乾淨,跨件負對照有鑑別力。印證 delaunay-v1(不規則件) vs v2-strip(窗簾) 分工。只驗靜態
  (3 件 weighted、無 deform timeline)。見 `knowledge/s3-psd-to-award-mesh.md`。
