...
- **S4→S3 端到端串接:已對真實生產 mesh 驗收通過(里程碑,2026-07-14)** — `robot_parts.psd`
  的 3 個「Award 中是 mesh」的件(光暈/身體/左手)切件後跑 `generate_mesh_v2`(auto),
  與 Award 藝術家 mesh 同底 IoU 對照 **3 件全 overall_pass**(身體反勝 0.966>0.948、
  覆蓋率匹配或勝過藝術家、頂點數更精簡 35/60/59<78/98/80、拓樸乾淨)。
  實測**推翻舊假設**:Award mesh uvs 已是 region-local(as-is IoU 0.95~0.98,唯一高),不需轉座標。
  這 3 件無 deform timeline(weighted、骨骼變形)→ 覆蓋率為真值、auto 回退 v1 為正解。
  工具 `tools/mesh_gen/validate_psd_to_mesh.py`;見 `knowledge/s4s3-psd-to-mesh-endtoend.md`。
- S1 / S5 尚未開始。
...
1. **❗最高優先:切圖→Spine JSON 組裝(SkelToJson)**。現在 S4(切件)與 S3(生成 mesh)都已對真實標的
   驗過,缺的是把兩者**寫回一份可載入的 Spine JSON**:固化 `機器人拆件/<圖層名>` 命名慣例、size+2px
   padding、mesh vs region 分配(光暈/身體/左手→mesh、右手/頭→region+旋轉)、atlas 0.70 縮放。
   端到端產出「PSD → 可載入 Spine skeleton JSON(+atlas 條目)」。**weighted mesh 的權重/骨綁是缺口**
   (生成 mesh 目前 unweighted;region-only 件可先無痛組)—— 可先做 region 件 + unweighted mesh 版。
2. ~~PSD件→S3 mesh→對照 Award 真實 mesh~~:**✅ 已完成(2026-07-14 里程碑,見上)**。
...
> S3、S4 均已對真實生產標的驗收,且已端到端串接(PSD件→生成mesh→對照真值)。建議下一步:
> (1) SkelToJson 組裝工具,把切件+生成 mesh 寫回可載入 Spine JSON(先做 region 件 + unweighted mesh,
> weighted 權重生成另立子題)。這是把四能力接成 pipeline 的關鍵一步。
...
- 2026-07-14:**S4→S3 端到端串接(里程碑)** — 機器人 3 mesh 件(光暈/身體/左手)切件 → `generate_mesh_v2`(auto)
  → 對 Award 藝術家 mesh 同底 IoU 對照,**3 件全 overall_pass**(身體反勝 0.966>0.948、全部更精簡、拓樸乾淨)。
  實測推翻舊假設:Award mesh uvs 已是 region-local(as-is 唯一高),不需轉座標。3 件無 deform timeline
  (weighted 骨骼變形)→ 覆蓋率為真值、auto 回退 v1 為正解。新工具 `validate_psd_to_mesh.py` + 對照圖。
  下一步定為 SkelToJson(切件+生成 mesh → 可載入 Spine JSON)。
