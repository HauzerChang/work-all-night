...
- **S5 pivot 寫入 build_spine 關節骨樹:端到端(里程碑,2026-08-30)** — 接續昨日 pivot 推斷器,
  把「接觸縫 pivot」真的**寫進可載入 Spine 骨樹**。`build_spine.py --rig` 用**中介 joint 骨**模式:
  對結構肢體發 `root→b_身體→(j_子件在pivot)→b_子件在件中心`,attachment 相對件中心不動 →
  **setup pose 逐件 d=0.0000px 與非 rig 相同**(免費繼承 round-trip),而 joint 骨正落 pivot →
  轉 joint 帶動整條肢體繞正確關節旋轉。運動學父子樹取自分析器 #4 note(誠實:tree 由分析器/先驗給)。
  閘 `tools/rig/validate_rig_build.py` 用 Spine 3.8 FK 對 robot_parts **4 AC 全 PASS**:
  ①setup 不變(max d=0.0000px);②joint 落 pivot(max d=0.005px);③轉關節 30°→子件繞 pivot 精準轉
  30.00°±0.003、半徑守恆、父件不動;④pivot 非天真(離件中心 0.255×body 尺度、改變末梢運動 0.132×)。
  使 `pivot_end2end` **L0→L2**(check_readiness 實跑確認)。誠實限制:**僅單一 rig 端到端驗過** → 區塊
  **續 HOLD**;**多 rig 真值卡資產**:Award 其他角色鏈(1_OMG/2_SUP/3_MEG)實查為**單一整片 weighted mesh
  (非拆件 rig)**,無法充當第二 pivot 真值 → 達 L3 需**第二個拆件 rig 資產**(使用者層級)。
  rig 與 weighted 尚未同時支援。無回歸(既有 build_spine/weighted/pivot 三閘全綠)。見 `knowledge/s5-rig-build-integration.md`。
...
5. **S5 骨架半自動**:關節 pivot 推斷 —— **接觸縫子問題 ✅(2026-08-29)**;
   **pivot→bone 父子樹寫入 `build_spine` ✅ 完成(2026-08-30,`--rig`,見上里程碑,pivot_end2end→L2)**。
   **續(達 L3 → 脫離 HOLD)**:(a) **多 rig 真值 ⛔ 卡資產**:Award `1_OMG`/`2_SUP`/`3_MEG` 經實查為
   單一整片 mesh(非拆件 rig),**無法充當第二真值** → 需使用者提供**第二個拆件(multi-part)且有藝術家
   pivot 真值的 rig 資產**(A 類/外部資源);(b) 肢體父子樹自動推斷(目前取自分析器先驗);
   (c) rig 與 weighted 同時支援(關節骨樹 + 骨綁 mesh 變形)。人形 RTMPose/MediaPipe、非人形光流分群為後續。
...
> S2(切圖閘)皆已達里程碑;**S5 rig pivot 接觸縫子問題(2026-08-29)+ 寫入 build_spine 關節骨樹(2026-08-30,
> pivot_end2end→L2)已完成**;S4 已交獨立排程。
> 建議下一個 bounded chunk(擇一):**(1) S5 達 L3 卡在資產** —— 需使用者提供**第二個拆件 rig 真值資產**才能證多 rig
> 泛化(見「下一步動作 5(a)」;Award 其他角色是單片 mesh 不適用)。可自主的替代:肢體父子樹自動推斷、rig+weighted
> 合流;**(2) weighted-forge 併入 spine-asset-forge skill**(C 類需使用者拍板);**(3) S2 骨架閘**(純 CPU 可自主)。
> 建議先做 **(3) S2 骨架閘**(可完全自主、補 S2 樞紐最後一塊)或 **(1) 的自主子項(肢體父子樹自動推斷)**。
...
- 2026-08-30:**S5 pivot 寫入 build_spine 關節骨樹(端到端,里程碑)** — `build_spine --rig`(中介 joint 骨模式:
  joint 骨落接觸縫 pivot、render 骨掛其下、attachment 不動)+ `validate_rig_build.py`(Spine 3.8 FK,4 AC 全 PASS:
  setup 逐件 d=0、joint 落 pivot、轉關節 30° 帶動肢體、pivot 非天真)。`pivot_end2end` L0→L2;區塊續 HOLD(多 rig 卡資產:
  Award 其他角色是單片 mesh)。無回歸。見 `log/2026-08-30-001.md`、`knowledge/s5-rig-build-integration.md`。
