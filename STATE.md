...
- **S3 端到端對照真實生產 mesh:通過(里程碑,2026-08-05)** — `compare_to_real_mesh.py` 對 Award 機器人
  3 個 weighted mesh(光暈/身體/左手)驗收:生成 hull ∩ 美術真實 hull IoU **0.91–0.955** 全過 0.85 閘;
  S3 覆蓋率 0.926–0.966(美術基準 ~0.972)且少 30–40% 頂點;`auto` 正確選 delaunay(團塊件)。
  對映用 alpha 外部真值校正(Award mesh uvs 為 region-local)。局限:weighted 件無 deform timeline → 變形穩健未驗。
  見 `knowledge/s3-vs-real-production-mesh.md`。
...
1. ✅ **已完成(2026-08-05):PSD件/atlas→S3 mesh→對照 Award 真實 mesh**(見上「S3 端到端」里程碑)。
2. **❗最高優先(把 S3+S4 收成產物):切圖→Spine JSON 組裝(SkelToJson)**。把 `機器人拆件/<圖層名>`
   命名慣例 + size+2px padding + `generate_mesh_v2` 的 mesh,固化成「件→Spine attachment/slot」寫出工具,
   端到端產出可載入的 Spine JSON。驗收:對 Award 機器人重建一份 skin,`compare_to_real_mesh` 式對照真實。純 CPU。
3. **S3 變形穩健對真實 weighted 件**:目前只驗靜態輪廓。若要驗變形,需有 deform-timeline 的生產 mesh
   (main_draw 窗簾已驗)或美術提供 bone 權重下的極端幀;可考慮把生成 mesh 綁 Award 骨骼權重再比。
...
> S3 與 S4 皆已對真實生產檔驗收通過。建議下一步:候選 2(SkelToJson)—— 把 S4 切件 + S3 mesh
> 串成「件→可載入 Spine JSON」的產物,是把兩能力收斂成 pipeline 的自然下一步,純 CPU 可自驅。
...
- 2026-08-05:**S3 端到端對照真實生產 mesh(里程碑)** — 新工具 `compare_to_real_mesh.py`;S3 首次對未見資產
  (Award 機器人 3 mesh)驗收。生成 hull ∩ 美術真實 hull IoU 0.91–0.955 全過;S3 覆蓋 0.926–0.966
  (美術 ~0.972)且少 30–40% 頂點;`auto` 正確選 delaunay(團塊)。對映用 alpha 外部真值校正
  (Award uvs 為 region-local、width/height=原尺寸)。局限:weighted 件無 deform timeline → 變形未驗。
  下一步定為 SkelToJson(件→Spine JSON 組裝)。
