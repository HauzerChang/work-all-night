...
- **S3+S4 端到端(里程碑,2026-07-15)** — PSD 件 → `generate_mesh_v2` → 對照 Award 真實生產 mesh
  (機器人 光暈/左手/身體)**3 件全 overall_pass**:覆蓋率在藝術家 ±0.03 內(身體還 +0.018)、
  頂點只用藝術家的 45~74%。工具 `tools/mesh_gen/compare_award_mesh.py`。發現 **Spine mesh uvs 是
  region-local 0..1、logical(未旋轉)方位** → 生成/藝術家 mesh 可在同一件像素空間直接比,0.70 縮放在
  IoU 正規化下抵銷。負對照(錯配 IoU 崩到 0.48~0.58)確認鑑別力。見 `knowledge/s3-s4-end-to-end-award.md`。
- S1 / S5 尚未開始。
...
1. ✅ **已完成(2026-07-15)**:~~PSD件→S3 mesh→對照 Award 真實 mesh(覆蓋率+頂點預算)~~
   → 見上「S3+S4 端到端(里程碑)」。**遺留**:此輪只做幾何/覆蓋對照,**未做 deform 閘**
   (Award mesh 為 weighted,需重現 Award 12 anim 的 weighted deform 位移場才能轉移比較)。
2. **❗建議下一步:件→Spine JSON 組裝(SkelToJson)**。把 `機器人拆件/<圖層名>` 命名慣例 +
   size+2px padding + **uv region-local→atlas 座標 + rotate 處理**(本輪確認 mesh uv 為 region-local
   logical 方位)固化成「件→Spine attachment」工具,端到端產可載入的 Spine JSON。
3. **weighted-mesh deform 閘**(補齊上面 1 的遺留;較難):重現 Award weighted deform → 對生成的
   (unweighted 或補綁權重)mesh 做真實位移場轉移閘。屬 S2 樞紐延伸。
...
> S3+S4 已端到端對真實生產 mesh 驗收(幾何/覆蓋)通過。建議下一步走 (2) 件→Spine JSON 組裝,
> 把整條 PSD→件→mesh→Spine attachment 打通;deform 閘(weighted)可作為 (3) 併行補齊。
...
- 2026-07-15:**S3+S4 端到端對照 Award 真實 mesh(里程碑)** — 新增 `compare_award_mesh.py`;
  PSD 件(光暈/左手/身體)→ `generate_mesh_v2` → 對 Award 藝術家 mesh 做覆蓋率+頂點預算 AC,**3 件全過**
  (覆蓋率藝術家 ±0.03 內、身體 +0.018,頂點只用 45~74%)。發現 **Spine mesh uvs = region-local 0..1
  logical 方位**(uv×件尺寸疊回件 alpha IoU 0.95~0.98)→ 生成/藝術家 mesh 可在件空間直接比。
  負對照(錯配 IoU 0.48~0.58)確認鑑別力;出圖 `figures/s3-award-mesh-compare.png`。
  遺留:未做 weighted deform 閘。
