...
- **S3 mesh 生成器：完成且對 4 個真實 mesh 收斂達標**(v2 strip 通用,見 `knowledge/s3-four-mesh-generalization.md`);
  **並已端到端對真實生產 Award 3 個 mesh 件靜態驗收通過**(`validate_award_static.py`,見 `knowledge/s3-award-mesh-static.md`)。
...
**S3 端到端對真實生產 Award mesh 靜態驗收通過(里程碑,2026-08-11)**:機器人拆件光暈/身體/左手
3 件 `validate_award_static.py` 全 overall_pass(exit 0)。發現 dense/複雜輪廓件 IoU 由邊界簡化
`epsilon_frac` 決定,**0.002** 為甜蜜點(舊預設 0.008 太粗、hull 太少差 0.05);已把 epsilon 透傳進
`generate_mesh_v2`(--epsilon)。這批是 **weighted mesh 且無 deform timeline** → 只驗靜態幾何,
deform 穩健性留待有位移場的標的(不用未校準壓力)。詳見 `knowledge/s3-award-mesh-static.md`。

下一個 bounded chunk 候選(更新後):
1. **SkelToJson 端到端組裝**:把 `PSD名/圖層名` 命名 + size+2px padding + mesh/region 分配 +
   dense 件 epsilon=0.002,固化成「PSD 件 → Spine attachment JSON」寫出工具,產可載入 spine。
2. **weighted mesh 權重生成(BBW)**:目前生成 unweighted;Award 這批是 weighted → 缺骨骼綁定+權重,
   需骨架(S5)先有骨(路線相依)。
3. **S2 補圖閘 / 骨架閘**(補齊 S2 樞紐;純 CPU)。
4. **S1 反推分析器**:需 benchmark 影片(repo 無影片資產)。

---
### (歷史)S3 已推廣到全部 4 個 mesh(里程碑,2026-06-26)
...
- ~~spine_inspector 實機 round-trip~~:**⛔ CDN(jsDelivr)被網路政策擋(403);需使用者改政策或提供離線 spine-webgl。**
...
- 2026-08-11:**S3 端到端對真實生產 Award mesh 靜態驗收(里程碑)** — 機器人拆件光暈/身體/左手 3 件
  (weighted mesh、無 deform timeline)靜態幾何全 overall_pass:先以藝術家自身 IoU(0.968~0.980)校準座標映射,
  生成 IoU(0.983~0.993)全 ≥ 藝術家 + 0 退化/0 孤兒/合法格式。發現 **dense 件 IoU 由 `epsilon_frac` 決定
  (0.002 甜蜜點,舊預設 0.008 差 0.05)**,跨 strip/Delaunay 兩模式證「覆蓋率只吃邊界密度」。
  epsilon 透傳進 generate_mesh_v2;新增 `validate_award_static.py`。deform 穩健性因無位移場暫緩(不用未校準壓力)。
