...
- **S3×S4 端到端:PSD 件 → 生成 mesh → 對照 Award 真實生產 mesh(里程碑,2026-07-12)** —
  `validate_psd_to_award.py` 對 robot_parts 三件(光暈/身體/左手,Award 中皆 mesh)跑
  `generate_mesh_v2`:覆蓋率**不劣於藝術家 mesh**(身體 0.966>0.948)、**頂點更省**(45–61%),
  3 件 `overall_pass=True`;交叉件負對照 gap>0.33(有鑑別力);疊圖 `figures/psd_to_award_mesh.png`。
  **校正 s4 doc 假設:Award mesh `uvs` 是 region-local 0..1、非 atlas-global**(PSD 件框無 flip 直接對齊)。
  誠實範圍:三件 weighted/無 deform timeline → 只驗靜態覆蓋率/拓樸,bone-driven 變形穩健留待 S5。
  見 `knowledge/s3s4-psd-to-award-mesh.md`。
- S1 / S5 尚未開始。
...
1. ✅ **已完成(2026-07-12):PSD件→S3 mesh→對照 Award 真實 mesh**(見上、`s3s4-psd-to-award-mesh.md`)。
2. **❗下一步:切圖→Spine JSON 組裝(SkelToJson)**。把已固化的真實慣例
   (slot 命名 `PSD名/圖層名`、size+2px padding、mesh/region 分配、生成 mesh)寫成工具,
   對一件(或整份 robot_parts)端到端產出可用的 Spine attachment/skin JSON;
   驗收:產出 JSON 的 attachment 幾何/覆蓋率對回 Award 真實件(復用本輪的覆蓋率閘)。純 CPU 可自驅。
3. **S2 補圖閘 / 骨架閘**(補齊 S2 樞紐;純 CPU)。
4. **S5 權重生成(BBW)**:bone-driven 件變形穩健的前提;唯一卡死環節(pivot 需人)。
5. **S1 反推分析器**:需一支 benchmark 影片(repo 無影片資產)。
6. ~~spine_inspector 實機 round-trip~~:**⛔ CDN(jsDelivr)被網路政策擋(403);需使用者改政策或提供離線 spine-webgl。**

> S3+S4 已串成端到端並對真實生產 mesh 驗收通過。建議下一步:(2) SkelToJson 組裝工具,
> 把「件 → Spine attachment JSON」補上,即完成「PSD → 可載入 Spine 資產」全鏈(缺的只剩權重/骨架 S5)。
