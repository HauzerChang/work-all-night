...
- **S3 端到端對照真實生產 mesh:通過(里程碑,2026-07-08)** — 機器人拆件 3 mesh 件(光暈/身體/左手)過 S3
  → 對照 Award 藝術家真實 mesh,靜態覆蓋率/拓樸/精簡度**全 pass**(gen IoU 0.978~0.988 vs 藝術家 0.968~0.980)。
  修 v1 兩問題:孤兒頂點(prune_orphans)+ 預設 epsilon 0.008→0.003(大件/軟邊欠採樣)。
  ⚠️ 這 3 件為 **weighted + 無 deform timeline** → deform 閘 N/A(S3 尚無 BBW 權重)。
  工具 `tools/mesh_gen/validate_award_mesh.py`,見 `knowledge/s3-award-mesh-end-to-end.md`。
...
1. ~~PSD件→S3 mesh→對照 Award 真實 mesh~~:**✅ 已完成(2026-07-08 里程碑,靜態全 pass)**。
   後續延伸:讓生成 mesh 真能替代 weighted mesh → 需 **BBW 權重生成**(見 #2)。
2. **❗BBW 權重生成(S3 最大缺口)**:目前 S3 只產 unweighted 幾何;Award 真實 mesh 是 weighted(靠骨骼變形)。
   要把「靜態幾何對齊」升級成「可動對齊」,需綁骨骼 + BBW(Bounded Biharmonic Weights)。純 CPU,較大工作塊
   → 先在 STATE 拆子步驟(取骨架 → 建 cage/heat/BBW → 寫回 vertices 攤平格式 → 用 Award weighted mesh 對照權重分佈)。
3. **切圖→Spine JSON 組裝(SkelToJson 寫出)**:把 `<PSD名>/<圖層名>` 命名 + size+2px padding + mesh/region 分配
...
> S3+S4 已串成端到端並對真實生產標的(Award)靜態驗收通過。建議下一步聚焦 **BBW 權重生成(#2)**:
> 這是把生成 mesh 從「靜態幾何對」升級到「能像藝術家 weighted mesh 一樣被骨骼驅動」的關鍵缺口,
> 且有 Award 真實 weighted mesh 的權重分佈可當對照真值,純 CPU 可自驅。
...
- 2026-07-08:**S3 端到端對照 Award 真實 mesh(里程碑)** — 機器人 3 mesh 件(光暈/身體/左手)過 S3
  → 對照 Award 藝術家真實 mesh,靜態覆蓋率/拓樸/精簡度全 pass(gen IoU 0.978~0.988 vs 藝術家 0.968~0.980)。
  建 `validate_award_mesh.py`;修 v1 孤兒頂點(prune_orphans)+ 預設 epsilon 0.008→0.003(大件/軟邊欠採樣盲點)。
  deform 閘 N/A(weighted+無 deform timeline,已程式確認)。下一步定為 BBW 權重生成。回歸 main_draw v1/v2 全過。
