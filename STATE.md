...
- **S3 端到端對 Award 真實 mesh 驗收(里程碑,2026-06-27)** — 對 3 個 weighted blobby 生產 mesh
  (光暈/身體/左手)做 ground-truth 對照:`compare_to_award.py`。預設參數過粗全低於藝術家;
  新增**自適應覆蓋**(`generate(target_iou=)`,opt-in 零回歸)後,budget≈100 時 **3/3 達或超過藝術家
  覆蓋且用更少頂點**。確認「覆蓋=hull 密度」對 blobby 件亦成立;量化 **64 預算 vs 藝術家密度(78–98)
  的張力**(細緻件如光暈需 ~73v)。見 `knowledge/s3-award-real-mesh.md`。
- S1 / S5 尚未開始。
...
1. ✅ **(已完成 2026-06-27)PSD件→S3 mesh→對照 Award 真實 mesh** — 見上方里程碑與
   `knowledge/s3-award-real-mesh.md`、`tools/mesh_gen/compare_to_award.py`。
2. **❗最高優先:件→Spine JSON 組裝(SkelToJson)** — 把「PSD 件 + 生成 mesh + `<PSD檔名>/<圖層名>`
   命名慣例 + size+2px padding」寫成 Spine attachment/skin/slot,端到端產 Spine JSON。mesh 端與真值
   對照已備齊(本次),這是把 S3+S4 真正串成「PSD→可載入 Spine」的最後一哩。組裝後可用 evaluate_slicing
   或對照 Award 結構自驗。
...
> S3 已對「strip(main_draw)+ blobby weighted(Award)」兩類真值驗收通過。建議下一步:組裝工具
> SkelToJson(候選 2),把件+mesh 寫成可載入 Spine JSON,完成 PSD→Spine 端到端。
