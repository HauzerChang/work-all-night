...
- **S3 端到端對外部真值驗收通過(里程碑,2026-08-02)** — 機器人 3 mesh 件(PSD→件→自產 mesh)
  對照 `Award.json` 真實藝術家 mesh:自產覆蓋率持平(Δ±0.017)、頂點少 40~55%、拓樸乾淨,
  負對照(交叉件 0.47~0.58 vs 對角 0.95+)確認鑑別力。工具 `compare_to_real_mesh.py`。
  見 `knowledge/s3-psd-to-real-mesh-e2e.md`。(此 3 件 weighted 無 deform,只驗靜態覆蓋+拓樸。)
...
1. ~~**PSD件→S3 mesh→對照 Award 真實 mesh**~~ ✅ **已完成(2026-08-02,里程碑)**,見上。
2. **❗最高優先:切圖→Spine JSON 組裝(SkelToJson)**。把已驗證的慣例固化成工具:
   slot=`<PSD檔名>/<圖層名>`、mesh 件用 `generate_mesh_v2`、rigid 件(頭/右手)用 region、
   size+2px padding、attachment 記原始邏輯尺寸 → 端到端從 PSD 產出可載入的 Spine JSON。
   自驗:對 `robot_parts.psd` 產 JSON,與 `Award.json` 機器人 slot 逐件比 attachment 幾何/型別。
...
> S3+S4 端到端已對真實生產標的(Award)驗收通過。建議下一步:(2) SkelToJson 組裝工具,
> 把 PSD 件端到端變成 Spine JSON(慣例都已確認、有 Award 真值可逐件對照,純 CPU 可自驅)。
