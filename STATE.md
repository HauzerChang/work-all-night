...
- **S3+S4 端到端驗收(里程碑,2026-07-04)** — PSD 切件 → `generate_mesh_v2` → 對照真實生產 spine
  `Award` 的藝術家 mesh:機器人 3 mesh 件(光暈/左手/身體)**全 PASS**(覆蓋不輸藝術家、頂點更省、0 孤島)。
  修正:光暈高曲率輪廓 v1 預設 epsilon 過度簡化(欠覆蓋+孤島)→ 加 **自適應 epsilon**(`gen_v1_adaptive`)。
  工具 `tools/mesh_gen/compare_award_mesh.py`;見 `knowledge/s3-award-mesh-e2e.md`。
...
1. ~~PSD件→S3 mesh→對照 Award 真實 mesh~~:**✅ 已完成(2026-07-04 里程碑)**,3 件全 PASS。
2. **❗最高優先:切圖→Spine JSON 組裝(SkelToJson)**。把已驗證的慣例固化成端到端工具:
   `機器人拆件/<圖層名>` slot 命名 + size+2px padding + mesh/region 分配(mesh 件用 S3 自適應生成、
   剛體件用 region+旋轉),產出可載入的 Spine JSON。純 CPU 可自驅。
3. **weighted mesh 骨綁(S3→S5 橋)**:Award 3 件是 weighted(骨權重變形);S3 目前產 unweighted。
   自動配權重(BBW)需先有骨架 → 屬 S5 範疇,可作為之後的橋接課題。
...
> S3+S4 已端到端對真實生產標的驗收通過。建議下一步:(2) SkelToJson 組裝,把「件→Spine attachment」
> 慣例固化成工具,端到端產出可載入 Spine JSON,完成「PSD → 可用 Spine 資產」這條 CPU pipeline。
...
- 2026-07-04:**S3+S4 端到端驗收(里程碑)** — PSD 切件 → `generate_mesh_v2` → 對照 Award 藝術家 mesh,
  機器人 3 mesh 件全 PASS(覆蓋不輸藝術家、頂點更省、0 孤島)。修光暈高曲率欠覆蓋 → v1 自適應 epsilon。
  實測 Award mesh uvs 為 region-local;藝術家重建用 dihedral 朝向搜尋 + 自一致性校驗(0.975~0.983)。
  main_draw 4-mesh 基準未回歸(全 strip 模式,與自適應 v1 隔離)。新增 `compare_award_mesh.py`、`s3-award-mesh-e2e.md`。
