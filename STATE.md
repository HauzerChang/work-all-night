...
- **S3×S4 端到端(對真實藝術家 mesh)已驗收(里程碑,2026-07-08)** — atlas 切件 → `generate_mesh_v2`
  → 對照 Award 3 個機器人 mesh 件。身體/左手輪廓 IoU 追平藝術家(Δ−0.008)且只用 ~6 成頂點;
  光暈(柔性放射狀)落後 Δ−0.050(hull 太粗)。全件 AC 過、靜態 0 自交。順手修好生成器
  **內部孤兒頂點 bug**(`prune_orphans`)。見 `knowledge/s3-robot-mesh-vs-artist.md`。
- S1 / S5 尚未開始。
...
1. ~~PSD件→S3 mesh→對照 Award 真實 mesh~~ **✅ 已完成(2026-07-08)**,見上與 `knowledge/s3-robot-mesh-vs-artist.md`。
2. **❗最高優先:柔性件 hull 加密**。光暈(圓潤/羽化)IoU 落後藝術家 0.05,因 hull 太粗(epsilon 0.008 → hull=14)。
   對低 solidity 件自動降 epsilon 加密 hull,量測 IoU 是否追平藝術家(有真值可比,純 CPU 自驅)。
3. **切圖→Spine JSON 組裝(SkelToJson)**:把 `機器人拆件/<圖層名>` 命名慣例 + size+2px padding + mesh/region
   分配固化成「件→Spine attachment」寫出工具,端到端產 Spine JSON。
...
> S3×S4 端到端已對真實藝術家 mesh 驗收通過。建議下一步:(2) 柔性件 hull 加密(直接改善唯一落後項,有真值),
> 或 (3) 把命名/尺寸慣例固化成 SkelToJson 組裝工具,把切圖→mesh→Spine JSON 全串起來。
