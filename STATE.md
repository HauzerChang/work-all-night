...
- **S3×S4 端到端驗收(里程碑,2026-07-11)** — PSD 件 → `generate_mesh_v2` → **對照 Award 真實藝術家 mesh**:
  3 mesh 件(光暈/身體/左手)生成覆蓋 IoU 與真值相當(≤0.016)且用 **45–77% 頂點**(35/60/59 vs 78/98/80),
  `compare_award_mesh.py` overall PASS。發現:blob 件 auto 走 Delaunay(strip 是窗簾專屬);**固定 0.95 IoU
  對軟邊發光件不可達(藝術家真值 0.949 也達不到,光暈 soft_ratio 0.236)→ 判準改以真值為上限**。
...
1. ~~PSD件→S3 mesh→對照 Award 真實 mesh~~ ✅ **完成(2026-07-11)** — `compare_award_mesh.py` overall PASS,
   3 mesh 件生成 IoU 與藝術家真值相當且更省頂點;見 `knowledge/s3-vs-award-real-mesh.md`。
2. **❗下一步建議:切圖→Spine JSON 組裝(SkelToJson)**:把 `機器人拆件/<圖層名>` 命名慣例 + size+2px padding
   + mesh/region 分配 + S3 生成 mesh 的 uv 佈局,固化成「件→Spine attachment」寫出工具,端到端產一份可載入的
   Spine JSON(串起 S4 切圖 → S3 mesh → JSON)。純 CPU 可自驅。
3. **S2 補圖閘 / 骨架閘**(補齊 S2 樞紐;純 CPU)。
4. **evaluate_mesh AC1 軟邊自適應**:soft_ratio 高時放寬 IoU 或改判 recall(本次已在 compare 用真值上限規避,
   尚未回寫 evaluate_mesh 本體以免影響既有 4-mesh 結論)。
5. **S1 反推分析器**:需一支 benchmark 影片(repo 無影片資產)。
6. ~~spine_inspector 實機 round-trip~~:**⛔ CDN(jsDelivr)被網路政策擋(403);需使用者改政策或提供離線 spine-webgl。**

> S3×S4 已端到端對真實藝術家 mesh 驗收通過。建議下一步:(2) 把件→Spine attachment 慣例固化成 SkelToJson
> 組裝工具,端到端寫出可載入 Spine JSON。
...
- 2026-07-11:**S3×S4 端到端對真實 mesh 驗收(里程碑)** — PSD 件→`generate_mesh_v2`→對照 Award 真實藝術家 mesh
  (光暈/身體/左手)。生成覆蓋 IoU 與真值相當(≤0.016)且省頂點(35/60/59 vs 78/98/80),`compare_award_mesh.py`
  overall PASS。發現:blob 件 auto 走 Delaunay(strip 是窗簾專屬);**固定 0.95 IoU 對軟邊發光件不可達
  (藝術家真值 0.949 也達不到,光暈 soft_ratio 0.236)→ 判準改以真值為上限**(第 4 次評估器校準洞見)。
