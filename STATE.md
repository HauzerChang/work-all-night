...
**S3 端到端對照 Award 真實 mesh 完成(里程碑,2026-07-13)**:候選 1 已做完。
- PSD件→`generate_mesh_v2`→對照 Award 藝術家 weighted mesh(光暈/左手/身體),同一張真實 alpha 比 IoU。
- **3/3 通過**且生成頂點(61/57/69)< 藝術家(78/98/80),覆蓋率同級或更高。工具 `compare_generated_vs_award.py`。
- **校正 log 006 誤解**:Award mesh uvs 是 **region 局部 0..1、y 向下**,不是 atlas UV(藝術家自身 IoU 0.97–0.98 佐證)。
- **修 v1 覆蓋率缺陷**:IoU 由 hull 邊界密度(epsilon)決定、內部點無關;`generate_mesh.py` 預設 0.008→0.004。
  main_draw 4 mesh 走 strip 不受影響,回歸重驗全 PASS。
- 詳見 `knowledge/s3-award-real-mesh-comparison.md` + `figures/s3-vs-award-robot-mesh.png`。

下一個 bounded chunk 候選:
1. **切圖→Spine JSON 組裝(SkelToJson)**:把 `機器人拆件/<圖層名>` 命名慣例 + size+2px padding + 生成 mesh
   固化成「件→Spine attachment」寫出工具,端到端產 Spine JSON。(S3+S4 串成產檔)
2. **weighted mesh 的 deform 對照**:這 3 件是 weighted(骨變形),本次只比靜態覆蓋率;
   要比變形需重建 Award 骨骼權重變形場(現有 `deform_eval` 只支援 unweighted+deform timeline)。
...
> 建議下一步:候選 1(SkelToJson 寫出工具)——把已驗證的「件→mesh」+ 命名/padding 慣例
> 固化成端到端產 Spine JSON,是 S3+S4 收斂成 pipeline 的自然下一步,純 CPU 可自驅。
...
- 2026-07-13:**S3 端到端對照 Award 真實 mesh(里程碑)** — PSD件→generate_mesh_v2→對照藝術家 weighted mesh
  (光暈/左手/身體),3/3 通過且頂點更精簡。校正 log 006 誤解(uvs 是 region 局部非 atlas UV,藝術家自身 IoU 0.97–0.98)。
  發現 IoU 由 hull 邊界密度決定、內部點無關 → 修 v1 預設 epsilon 0.008→0.004;main_draw 4 mesh(strip)回歸全 PASS。
  新工具 `compare_generated_vs_award.py`(內建 sanity gate 擋 UV 誤解)。見 `knowledge/s3-award-real-mesh-comparison.md`。
