...
- **S3×S4 端到端驗收(里程碑,2026-08-15)** — `compare_award_mesh.py`:PSD 件(光暈/身體/左手)→
  `generate_mesh_v2` → 對照 **Award 真實生產 mesh**,3 件全 `overall_pass`(coverage/IoU 在藝術家基準
  ±margin 內,頂點數少 25~55%,三角品質不輸真值)。負對照第四次確認閘鑑別力(縮小/放大皆正確 FAIL)。
  這 3 件在 Award 為 weighted mesh 且無 deform timeline,deform 閘 N/A(與 curtain/shadow 的
  `validate_against_real` 互補)。見 `knowledge/s3-award-endtoend.md`。
- S1 / S5 尚未開始。
...
1. ✅ ~~PSD件→S3 mesh→對照 Award 真實 mesh~~ **已完成(2026-08-15)** — 見上方里程碑 / `compare_award_mesh.py`。
2. **❗建議下一步:切圖→Spine JSON 組裝(SkelToJson)**。把「件 + 生成 mesh」落成可用 Spine JSON attachment:
   固化 `<PSD名>/<圖層名>` 命名、+2px padding、atlas ~0.70 縮放、mesh/region 分配慣例。端到端產出 spine 檔
   (純 CPU 可自驅;驗收:重載回 evaluate_slicing/compare 仍 pass)。
3. **S3 權重指派(BBW/heat)**:unweighted mesh + 骨架 → weighted mesh,才能完全對齊 Award 真值那類骨驅件。
   需先有骨架(S5)或人工骨定義 → 依賴 S5,順序上排在 SkelToJson 之後。
...
> S3×S4 已端到端對真實生產標的驗收通過。建議下一步:(2) SkelToJson 把件+mesh 落成 Spine JSON(純 CPU 自驅),
> 之後才是 (3) 權重指派(依賴骨架 S5)。
...
- 2026-08-15:**S3×S4 端到端驗收(里程碑)** — `compare_award_mesh.py`:PSD 件(光暈/身體/左手)→
  `generate_mesh_v2` → 對照 Award 真實生產 mesh,3 件全 overall_pass;生成 mesh 頂點少 25~55%、
  三角品質不輸(光暈甚至優於)真值。座標映射直接用 uvs×(W,H)(免 atlas 縮放/weighted bind)。
  這 3 件 weighted 無 deform → deform 閘 N/A(與 curtain/shadow 互補)。負對照第四次確認閘鑑別力。
  見 `knowledge/s3-award-endtoend.md`。下一步定為 SkelToJson(件+mesh→Spine JSON)。
