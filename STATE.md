...
- **S4→S3 端到端串接:對真實生產標的驗收通過(里程碑,2026-07-09)** — `robot_parts.psd` 3 件
  (光暈/身體/左手)經切圖→`generate_mesh_v2` 生成 mesh,對照 Award 藝術家真實 mesh,覆蓋率
  0.961/0.980/0.974(達/逼近藝術家,頂點數更少)。發現機器人件是 **weighted+骨骼驅動、無 deform**
  (deform 閘 N/A,缺 BBW 權重才能真 drop-in);v2 auto 正確回退 delaunay;v1 eps 0.008→**v2 fallback 校準 0.005**
  (高頻輪廓)。見 `knowledge/s3-robot-psd-to-mesh-vs-award.md`。
- S1 / S5 尚未開始。
...
1. ✅ **已完成(2026-07-09)**:PSD件→S3 mesh→對照 Award 真實 mesh(光暈/身體/左手覆蓋率達標)。
   延伸出的**最高優先後續 = BBW 權重生成**(見下 A)。
...
**A. ❗最高優先(承 2026-07-09 發現):BBW 權重生成**。機器人 mesh 是 weighted+骨骼驅動,
生成的 unweighted mesh 幾何/覆蓋率已達標,但要真正 drop-in 替代 Award 的 weighted mesh,
**還缺把 mesh 綁到骨架的 BBW 權重**(S3 路線圖列了未做,純 CPU 可行)。有 Award 真值權重可對照。

> S4 已對真實檔驗收通過,S3 已對真實 mesh 幾何驗收通過。建議下一步:(A) BBW 權重生成
> —— S3 最後一塊、有真值(Award weighted mesh)、純 CPU 可自驅。或 (2) SkelToJson 把件組成 Spine JSON。
...
- 2026-07-09:**S4→S3 端到端驗收(里程碑)** — `robot_parts.psd` 3 件(光暈/身體/左手)切圖→
  `generate_mesh_v2` 生成 mesh,對照 Award 藝術家 mesh,覆蓋率 0.961/0.980/0.974(達/逼近藝術家、頂點更少)。
  發現機器人件 weighted+骨骼驅動、無 deform(deform 閘 N/A,缺 BBW 權重);v2 auto 正確回退 delaunay;
  v1 eps 0.008 對高頻輪廓過疏 → v2 fallback 校準 0.005;固定閾值(0.95/64頂點)為合成期預設,真標的=藝術家基準。
  4 窗簾 v2 回歸不受影響(strip)。開下一里程碑候選 = BBW 權重生成。
