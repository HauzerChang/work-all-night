...
- **S3×S4 端到端(對 Award 真實生產 mesh)完成(里程碑,2026-07-21)** — PSD 切件→`generate_mesh_v2`
  →對照 Award 三個真實 weighted mesh(光暈/身體/左手)。生成 mesh **輪廓覆蓋 ≥ 藝術家(全 3 過覆蓋率閘)
  且頂點少 35–55%**;`mode=auto` 對真實 blob 件正確走 delaunay-v1。座標系自我校驗 + 負對照皆過。
  見 `knowledge/s3-award-real-mesh-e2e.md`、`tools/mesh_gen/compare_award_mesh.py`。
- S1 / S5 尚未開始。
...
1. ~~PSD件→S3 mesh→對照 Award 真實 mesh~~:**✅ 完成(2026-07-21)**,全 3 件覆蓋率過、更精簡。
   見 `knowledge/s3-award-real-mesh-e2e.md`。
2. **❗最高優先:切圖→Spine JSON 組裝(SkelToJson)**。把生成 mesh 的「切件框 uvs」轉 atlas UV
   (0.70 縮放 + 旋轉 + page 位移),配 `機器人拆件/<圖層名>` 命名 + size+2px padding,端到端產出
   可載入的 Spine JSON attachment。這是把 S3+S4 從「驗收」推到「產出」的下一哩。純 CPU 可自驅
   (有 Award atlas/json 當真值:組出的 attachment uvs/size 應對得上 Award 對應件)。
3. **mesh→bone 權重生成(BBW)**:讓生成 mesh 可 weighted,才能對照 Award weighted 綁定(目前只驗靜態輪廓)。
...
> S3×S4 端到端驗收已通過(對真實生產 mesh)。建議下一步:(2) SkelToJson —— 把生成 mesh 轉成
> atlas UV + 命名慣例組出 Spine JSON,用 Award 當真值對照組出的 attachment。
