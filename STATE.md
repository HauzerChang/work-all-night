...
**S3 端到端對照真實生產 mesh 通過(里程碑,2026-07-28)**:`compare_award_mesh.py` 拿 Award
機器人 3 件真實 mesh(光暈/身體/左手)當真值,生成 mesh 靜態 IoU parity 全過(≤0.008、頂點更精簡),
串起「S4 切件 → S3 生成 → 生產真值對照」。由對照揪修生成器 2 缺陷:`generate_adaptive`(邊界自適應
epsilon,hull 覆蓋率自我目標)+ `prune_orphans`(清內部孤兒頂點);只動 delaunay 分支,main_draw
4 mesh strip 回歸 4/4 全過。誠實限制:Award 件無 deform timeline → 只驗靜態、不含變形閘。
詳見 `knowledge/s3-award-real-mesh-parity.md`。

下一個 bounded chunk 候選:
1. **切圖→Spine JSON 組裝(SkelToJson)**:把 `機器人拆件/<圖層名>` 命名慣例 + size+2px padding +
   mesh/region 分配固化成「件→Spine attachment」,端到端寫出可載入的 Spine JSON(可用 Award 當對照)。
2. **weighted mesh 支援**:目前生成器只出 unweighted;Award 生產 mesh 靠骨骼權重變形。研究 BBW/
   heat-diffusion 權重,讓生成 mesh 能綁骨(S3 路線圖第三塊)。
...
- 2026-07-28:**S3 端到端對照真實生產 mesh(里程碑)** — `compare_award_mesh.py` 拿 Award 機器人 3 件
  真實 mesh 當真值,生成 mesh 靜態 IoU parity 全過(光暈/身體/左手 差 ≤0.008,頂點更精簡)。
  由真值對照揪修生成器 2 缺陷:`generate_adaptive`(邊界自適應 epsilon)+ `prune_orphans`(清內部孤兒)。
  只動 delaunay 分支,main_draw 4 mesh strip 回歸 4/4 全過。誠實限制:Award 件無 deform → 只驗靜態。
