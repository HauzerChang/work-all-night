...
**S3×S4 端到端對照 Award 真值完成(里程碑,2026-07-12)**:`compare_to_award.py` 把
`robot_parts.psd` 3 個 mesh 件(光暈/身體/左手)→ `generate_mesh_v2` → 對照 Award **藝術家真值 mesh**。
- **3 件全過**(相對藝術家 tol=2% + 幾何乾淨):IoU 差 光暈−1.55% / 身體 **+1.83%(反超)** / 左手−1.26%;
  我的 mesh 用**少 21–43 頂點**達藝術家覆蓋。auto 全走 v1(件非直條,strip 不適用)。
- **第四次評估器校準**:**絕對 0.95 IoU AC 對軟邊件過嚴 —— 連藝術家真值(光暈 0.9486)都不過** →
  改用「相對藝術家 + 幾何乾淨」判準(工具已把兩者拆開)。負對照確認 coverage 度量鑑別力
  (對角 0.95–0.98 vs 錯件 0.48–0.58)。詳見 `knowledge/s3-end-to-end-award.md`。
- 技術點:Award json mesh `uvs` 為 region-normalized [0,1](非整張 sheet),`(u·W,v·H)` 直接還原件像素;
  v 向下與 generate_mesh 一致。這 3 件為 weighted、無 deform timeline → **無真實位移場可轉,故不做 deform 閘**。

下一個 bounded chunk 候選:
1. **❗最高優先:切圖→Spine JSON 組裝(SkelToJson)**。把 `PSD名/圖層名` 命名 + size+2px padding +
   mesh/region 分配 + generate_mesh 輸出,固化成「件→Spine attachment」寫出工具,端到端產 Spine JSON。
   驗收:產出的 JSON 用 evaluate_mesh + 對照 Award slot 結構(命名/size)一致。純 CPU 可自驅。
2. **S2 補圖閘 / 骨架閘**(補齊 S2 樞紐;純 CPU)。
3. **S1 反推分析器**:需一支 benchmark 影片(repo 無影片資產,屬使用者提供)。
4. ~~PSD件→S3 mesh→對照 Award 真實 mesh~~:**✅ 完成(本次)**。
5. ~~spine_inspector 實機 round-trip~~:**⛔ CDN(jsDelivr)被網路政策擋(403);需使用者改政策或提供離線 spine-webgl。**

> S3+S4 已串成端到端並對真實生產標的(Award 藝術家 mesh)驗收通過。建議下一步:把慣例固化成
> SkelToJson 產檔工具(候選 1),真正走到「件 → 可用 Spine JSON」。
...
- 2026-07-12:**S3×S4 端到端對照 Award 真值(里程碑)** — 新增 `compare_to_award.py`:robot_parts.psd 3 mesh 件
  → generate_mesh_v2 → 對照 Award 藝術家 mesh。3 件全過(相對藝術家 tol=2% + 幾何乾淨),我的 mesh 少 21–43
  頂點達藝術家覆蓋(身體件反超 +1.8%)。**第四次評估器校準:絕對 0.95 IoU AC 對軟邊件過嚴,連藝術家真值
  (0.9486)都不過 → 改「相對藝術家」判準**。負對照確認度量鑑別力(對角 0.95–0.98 vs 錯件 0.48–0.58)。
  技術點:Award mesh uvs 為 region-normalized、v 向下;3 件 weighted 無 deform → 不做 deform 閘。
