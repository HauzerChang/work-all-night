...
- **S3 端到端對照真實生產 mesh:完成(里程碑,2026-07-20)** — PSD 件→`generate_mesh_v2`→對照 Award
  big win 機器人 3 個 mesh 件(光暈/身體/左手):覆蓋率**全達/超藝術家真值**,用 31~48 頂點(藝術家 78~98,
  省 40~60%),靜態 AC 全過。發現 **IoU 純由 hull 密度決定(v1/v2 兩家族一致)**;blob-fallback epsilon
  0.008→0.004 + 加 orphan 清理。見 `knowledge/s3-award-mesh-endtoend.md`;工具 `compare_award_mesh.py`。
...
1. ~~PSD件→S3 mesh→對照 Award 真實 mesh~~ **✅ 完成(2026-07-20,里程碑)**。
2. **❗最高優先:切圖→Spine JSON 組裝(SkelToJson)**。把已固化的慣例寫成工具:
   `<PSD名>/<圖層名>` slot 命名 + size+2px padding + **mesh/region 依需求分配**(blob→v1 epsilon 0.004、
   strip→v2、剛體→region),端到端從 PSD 產出可載入的 Spine JSON attachment 區塊。
   有真值可比(Award.json 對應 slot 的 attachment 結構)。純 CPU 可自驅。
...
> S3+S4 端到端已對真實生產 mesh 驗收通過。建議下一步:(2) 把慣例固化成 SkelToJson 組裝工具,
> 端到端「PSD → Spine JSON」;之後補 S2 補圖/骨架閘。
...
- 2026-07-20:**S3 端到端對照 Award 真實 mesh(里程碑)** — PSD 件→`generate_mesh_v2`→對照 Award
  3 mesh 件(光暈/身體/左手):IoU 全達/超藝術家(0.964/0.983/0.980 vs 0.949/0.948/0.977),
  頂點 31/47/48(藝術家 78/98/80),靜態 AC 全過。發現 **IoU 純由 hull 密度決定**(v1 Delaunay 與
  v2 strip 兩家族一致);修 blob-fallback epsilon 0.008→0.004 + 加 `drop_orphans` 清理。main_draw 回歸全過。
