...
- **S3+S4 端到端串接:已對真實生產標的(有真值)驗收通過(里程碑,2026-07-14)** —
  `compare_to_award.py` 把 `robot_parts.psd` 的 3 個 mesh 件(光暈/身體/左手)走
  「PSD 切件 → `generate_mesh_v2` → 對照 Award.json 真實藝術家 mesh」,**3 件全過**
  (覆蓋率 IoU 皆 ≥ 藝術家 baseline、頂點數皆 < 藝術家、mesh 幾何乾淨;overall_pass EXIT 0)。
  兩個由真值逼出的校準:`epsilon_frac` 0.008→0.004(真實曲邊 hull 需更細)+ 新增 `prune_unused()`
  修內部孤兒頂點;4 main_draw mesh(strip)重驗無回歸。deform 閘 N/A(這 5 件無 deform timeline,
  骨骼/weighted 驅動)。見 `knowledge/s3-psd-to-mesh-award.md`。
- S1 / S5 尚未開始。
...
1. ~~PSD件→S3 mesh→對照 Award 真實 mesh~~ **✅ 完成(2026-07-14,里程碑)** — 見上 + `knowledge/s3-psd-to-mesh-award.md`。
2. **❗最高優先:切圖→Spine JSON 組裝(SkelToJson)**。把「PSD件 → 生成 mesh」的結果**寫回可載入的 Spine JSON**:
   `機器人拆件/<圖層名>` 命名 + size+2px padding + atlas 0.70 縮放;region 件(右手/頭)含旋轉打包。
   驗收:產出的 JSON 能被 spineTool `applySkeletonJSON` 載入(或用 Python 重讀 schema 驗),attachment
   幾何與來源件對齊。這是把 S3+S4 從「驗證」推到「產出可用資產」的下一哩。
3. **S2 補圖閘 / 骨架閘**(補齊 S2 樞紐;純 CPU)。
4. **weighted mesh 綁定(S5 前置)**:本輪生成的是 unweighted 幾何;真實 Award mesh 皆 weighted(骨骼驅動)。
   骨骼權重/pivot 屬 S5,是唯一卡死環節。
5. **S1 反推分析器**:需一支 benchmark 影片(repo 無影片資產)。
6. ~~spine_inspector 實機 round-trip~~:**⛔ CDN(jsDelivr)被網路政策擋(403);需使用者改政策或提供離線 spine-webgl。**

> S3+S4 已對真實生產標的端到端驗收通過。建議下一步:(2) SkelToJson 把生成 mesh 寫回 Spine JSON,
> 產出可載入資產;之後 (4) weighted 綁定進入 S5。
...
- 2026-07-14:**S3+S4 端到端對真實 mesh 驗收(里程碑)** — 新增 `compare_to_award.py`;`robot_parts.psd`
  3 mesh 件(光暈/身體/左手)PSD切件→`generate_mesh_v2`→對照 Award 真實藝術家 mesh,3 件全過
  (覆蓋率 ≥ 藝術家、頂點數 < 藝術家、幾何乾淨)。真值逼出兩校準:`epsilon_frac` 0.008→0.004、
  新增 `prune_unused()` 修內部孤兒頂點;4 main_draw mesh 無回歸。deform 閘 N/A(無 deform timeline)。
