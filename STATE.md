...
- **S3×S4 端到端整合:對真實生產 mesh 驗收通過(里程碑,2026-07-07)** — `validate_psd_mesh.py`
  把「PSD 切件 → S3 生成 mesh」串起,對 `robot_parts.psd` 3 個 mesh 件(光暈/身體/左手)
  與 Award 真實 mesh 對照:**生成 mesh 覆蓋率 IoU 全 ≥ 藝術家(0.966/0.988/0.985 vs 0.949/0.948/0.977)、
  頂點更精簡、基礎拓樸乾淨**。驗明 Spine JSON mesh `uvs`=region-local[0,1]。關鍵旋鈕:
  **v1 `epsilon_frac` 決定覆蓋率**(舊預設 0.008 對 ~700px 大件太粗 → 改 0.003 全過);
  strip 僅適 deform-bearing 直條件。跨件負對照確認鑑別力。見 `knowledge/s3-psd-to-award-mesh.md`。
- S1 / S5 尚未開始。
...
1. ~~PSD件→S3 mesh→對照 Award 真實 mesh~~:**✅ 完成(2026-07-07)** — 見上方里程碑 + `validate_psd_mesh.py`。
2. **❗最高優先:切圖→Spine JSON 組裝(SkelToJson)**。把已驗的三段串成產出:
   `機器人拆件/<圖層名>` 命名慣例 + size+2px padding + region/mesh 分配(剛體件 region、柔性件 mesh)
   + 生成 mesh(eps=0.003)→ 端到端**寫出可載入的 Spine JSON**。這是把 S3/S4 從「驗收」轉成「產出」的關鍵。
   驗收:產出 JSON 的 slot/attachment 結構對得上 Award 慣例;mesh 件用生成 mesh、覆蓋率過閘。
3. **S2 補圖閘 / 骨架閘**(補齊 S2 樞紐;純 CPU)。
4. **(需 S5)把生成 unweighted mesh 綁骨骼權重(BBW)** 才能真的替換 Award 的 weighted mesh
   走骨骼變形 —— 目前只驗到靜態覆蓋率+拓樸,權重是下一道關。
5. **S1 反推分析器**:需一支 benchmark 影片(repo 無影片資產)。
6. ~~spine_inspector 實機 round-trip~~:**⛔ CDN(jsDelivr)被網路政策擋(403);需使用者改政策或提供離線 spine-webgl。**

> S3×S4 端到端已對真實生產 mesh 驗收通過。建議下一步:(2) SkelToJson 把三段串成「件→Spine JSON」產出工具,
> 完成後 S3+S4 即從驗收升級為可用 pipeline;真正替換 weighted mesh 需 S5 權重(候選 4)。
...
- 2026-07-07:**S3×S4 端到端(里程碑)** — `validate_psd_mesh.py` 串「PSD件→生成mesh」對照 Award 真實 mesh。
  3 個機器人 mesh 件生成 mesh 覆蓋率 IoU 全 ≥ 藝術家、頂點更精簡、拓樸乾淨。驗明 Spine mesh uvs=region-local[0,1]。
  關鍵:v1 `epsilon_frac` 是覆蓋率旋鈕,舊預設 0.008 對大件太粗 → 0.003 全過;strip 僅適 deform-bearing 件。
  跨件負對照確認鑑別力(對角 0.97~0.99 vs 錯件 0.48~0.58)。
