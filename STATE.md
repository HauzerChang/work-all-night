...
- **S3+S4 端到端(里程碑,2026-08-10):PSD件→mesh→對照 Award 真實 mesh 通過** —
  `robot_parts.psd` 切 3 個 mesh 件(光暈/身體/左手)跑 `generate_mesh_v2`,生成覆蓋率 IoU
  達/超藝術家 Award mesh(margin 0.02)且頂點更少,3 件全 overall_pass。發現這批非高瘦 →
  v2 auto 選 Delaunay-v1(非 strip);Award mesh 皆 weighted 且無 deform timeline(skinning 驅動)
  → per-vertex deform 閘 N/A。見 `knowledge/s3-psd-to-award-endtoend.md`。
- S1 / S5 尚未開始。
...
1. ~~PSD件→S3 mesh→對照 Award 真實 mesh~~ **✅ 完成(2026-08-10,見上)**。3 件全過。
2. **❗最高優先:切圖→Spine JSON 組裝(SkelToJson)**。把 `機器人拆件/<圖層名>` 命名慣例 +
   size+2px padding + mesh/region 分配固化成「件→Spine attachment」工具,端到端產 Spine JSON。
   有真值可比(Award.json 對應 slot)→ 可自驗:組出的 attachment size/命名/mesh 覆蓋率對得上 Award。
3. **S2 補圖閘 / 骨架閘**(補齊 S2 樞紐;純 CPU)。
4. **S1 反推分析器**:需一支 benchmark 影片(repo 無影片資產)。
5. **(選)Delaunay-v1 skinning 變形穩健性驗證**:Award mesh 無 deform timeline,要驗需 forward-kinematics
   套 skinning 權重 + 骨骼動畫再驗自交(屬 S5 範疇,較複雜)。
6. ~~spine_inspector 實機 round-trip~~:**⛔ CDN(jsDelivr)被網路政策擋(403);需使用者改政策或提供離線 spine-webgl。**

> S3+S4 已串成端到端並對真實 Award mesh 驗收通過。建議下一步:候選 2(SkelToJson 組裝),
> 把「件+命名慣例+padding+mesh/region 分配」固化成產 Spine JSON 的工具,對 Award 逐件自驗。
...
- 2026-08-10:**S3+S4 端到端(里程碑)** — `robot_parts.psd` 切 3 mesh 件→`generate_mesh_v2`→對照 Award 真實 mesh。
  覆蓋率 IoU 達/超藝術家基準(margin 0.02)且頂點更少,3 件全 overall_pass;格式/退化/孤兒全過。
  發現:這批非高瘦(aspect<1.2)→ v2 auto 選 Delaunay-v1(首批未觸發 strip 的真實件);
  Award mesh 皆 weighted + 無 deform timeline(skinning 驅動)→ per-vertex deform 閘 N/A。
  產 `tools/mesh_gen/compare_psd_to_award.py`(一鍵自動切 PSD→生 mesh→對照)、`knowledge/s3-psd-to-award-endtoend.md`。
