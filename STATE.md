...
- **S1 squash 接檔位幅度差異化:耦合放大(里程碑,2026-09-19,candidate G-4''''')** — 補 (G-4'''') 明列的
  honest boundary(「squash 未接 tier —— `_amp_scale` 逐軸只放大 identity 上方會**破壞體積守恆**(scaleY<1
  樓地板保留而 scaleX>1 放大 → scaleX·scaleY≠1、果凍擠壓變單軸拉伸)」)。本次把 `squash` 併入
  `MAIN_SHOW_CATS`,並讓 scale 通道走**耦合放大** `tier_variants._amp_squash_pair`:自 scaleX 還原 squash 量
  `q=scaleX−1`(gen_squash 恆以 scaleX 為拉長軸)、線性放大 `q'=g·q`(同專案 scale 慣例 v'=1+g(v−1),惟
  **伴軸改由守恆式重建而非各自放大**)、再以 **gen_squash 同雙重 round** 重建兩軸 `scaleX'=1+q'`、
  `scaleY'=1/(1+q')`。⇒ **squash 的兩個幅度軸(shear 峰 + 擠壓量)隨檔位嚴格遞增,且 `scaleX·scaleY≡1`
  在每個檔位保持**(shear 通道同 wobble `v'=g·v`)—— **首次讓耦合雙通道同步隨檔位放大**;g=1(Super)
  逐位元同 base(精確 identity)。新增 `tier_variants.VOLUME_CONSERVING_CATS={squash}` 供 `build_animations`
  對 squash 檔位變體傳 `coupled_scale=True`。整合閘 `validate_squash_tier.py`(先驗庫→**真實 build_spine robot
  骨架**→build_animations(tier_gains))**5 AC 全 PASS**:P1 present+backward-compat(每檔位 `squash__{tier}`
  finite/有 bone/**同時**帶 shear+非均勻 scale、名路由回 squash、**base+In/Loop/Out 逐位元不變**)、P2 **crux**
  雙軸單調(峰 |shearX| [16,21.6,27.2,33.6]° 與峰擠壓量 |scaleX−1| [0.16,0.216,0.272,0.336] 皆 Super<Mega<Omg<
  Legend 嚴格遞增且 Super==base)、P3 **crux** 體積守恆+簽章保形(每檔位每 bone 每擠壓幀 |scaleX·scaleY−1|≤0.02
  =`_amp_scale` 會破壞而耦合放大保住的關鍵、非均勻≥0.05、擠壓量隨極值嚴格遞減、shear 首尾 0+變號≥3+極值遞減、
  scale 首尾 identity)、P4 耦合隔離(全 storyboard 只有 squash 及其變體同時帶 shear+非均勻 scale;wobble 有
  shear 無非均勻 scale、其餘等比 scale 無 shear)、P5 負對照(a 平增益全 1.0→雙軸遞增皆 FALSE 且各檔位逐位元
  ==base、b **耦合 vs 逐軸單元測**:合成守恆對用 `_amp_squash_pair` 放大→product≡1 且擠壓量遞減,同一對用逐軸
  `_amp_scale` 放大→product 1.02–1.14 **明顯破壞守恆**證耦合放大在做事)。端到端 `build_spine --animate
  --tier-variants --shear-pivot` 直出 `squash__{Super,Mega,Omg,Legend}`(**extreme 關鍵幀體積守恆精確保留**
  0 違反;densify 中間插值幀偏差 Legend ~1.3e-2 同 (G-4'''') 為 `apply_pivots` 非線性密網格重取樣的既有插值特性
  ——逐軸插值一條守恆曲線不逐點守恆,非本次引入),`validate_build` round-trip overall_pass(premult MAE 0.031)。
  回歸:validate_squash_gen(G-4'''')/wobble_tier/wobble_count/tier_variants/**tier_combo_count(K5c 對
  `VOLUME_CONSERVING_CATS` 豁免 combo 專用 impact-峰計數器——squash 的阻尼 scale 非脈衝列,放大跨門檻造成假外洩,
  其振盪段數本身各檔位恆定非 count-aware)**/shear 系列/全 priors/beat/pivot 系列 **18 閘全綠**。新增 cap
  `squash_tier_amplitude` L2 併入 `spine-anim-forge`(**仍 HOLD**:運動基元先驗、單一真值資產,防固化)。
  **關鍵發現:耦合雙通道的檔位放大須在自然座標(squash 量 q)放大,而非各軸獨立**(逐軸破壞守恆物理約束;同
  (J-2)/(G-4''') 結構軸須 gen 時決定,此處守恆約束須放大時耦合——皆是「事後逐軸/同比 amplify 做不到、須換更本質
  操作」)。**honest boundary(仍在)**:增益階梯沿用 (J)(PROPOSAL 手感留使用者 A 類);shearY≡0;**squash
  count-aware(擠壓段數隨檔位,`gen_squash(nosc=)` 參數已備,比照 (G-4''') 對 wobble)為後續**。
...
> **(G-4''''') ~~squash 接 tier 檔位幅度差異化(耦合 amplify)~~ ✅ 完成(2026-09-19,candidate G-4''''',`squash_tier_amplitude` L2,見上里程碑)** ——
>   `_amp_squash_pair`(自 scaleX 還原 q、線性放大 q'=g·q、以生成器同式重建兩軸)+ `VOLUME_CONSERVING_CATS={squash}` + squash 併入 MAIN_SHOW_CATS;
>   shear 峰 [16,21.6,27.2,33.6]° 與擠壓量 [0.16,0.216,0.272,0.336] 雙軸同步隨檔位遞增且 scaleX·scaleY≡1 每檔位保持;`validate_squash_tier.py` 5AC PASS(P3 體積守恆 crux、P5b 耦合 vs 逐軸單元測證守恆破壞)。
>   **續**(擇一,皆純自主):**squash count-aware(擠壓段數隨檔位,`gen_squash(nosc=)` 已備,比照 (G-4'''))**。
> **建議下一個 bounded chunk(擇一,皆純自主):**
> **(G-4'''''') squash count-aware:擠壓段數隨檔位(nosc 已備參數,結構軸,同 (G-4''') 對 wobble;與本次幅度軸正交可疊)**;
...
- 2026-09-19:**S1 squash 接檔位幅度差異化:耦合放大(里程碑,candidate G-4''''')** — 補 (G-4'''') 的 honest
  boundary(squash 未接 tier,因 `_amp_scale` 逐軸放大破壞體積守恆)。squash 併入 `MAIN_SHOW_CATS`,scale 走
  **耦合放大** `_amp_squash_pair`(還原 q→放大 q'=g·q→重建兩軸,保 scaleX·scaleY≡1)+ `VOLUME_CONSERVING_CATS`。
  首次讓耦合雙通道(shear 峰+擠壓量)同步隨檔位遞增。`validate_squash_tier.py` 5AC PASS(P3 每檔位體積守恆、
  P5b 耦合 vs 逐軸單元測證守恆破壞);tier_combo_count K5c 對 VOLUME_CONSERVING_CATS 豁免 combo 專用峰計數器;
  18 閘全綠 + round-trip overall_pass。cap `squash_tier_amplitude` L2(anim-forge 仍 HOLD)。見
  `knowledge/s1-squash-tier-amplitude.md`。**下一步**:squash count-aware(擠壓段數隨檔位,nosc 已備)。
