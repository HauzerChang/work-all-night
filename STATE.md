...
- **S3 端到端對真實生產 mesh 驗收通過(里程碑,2026-08-10)** — 用 Award atlas 真實 alpha 對機器人拆件
  3 個真實 mesh(光暈/左手/身體)跑 S3;對齊藝術家頂點成本下生成 IoU **全 ≥ 藝術家**(0.986/0.994/0.995)、
  拓樸乾淨 → `validate_award_mesh.py` all_pass。發現「輪廓解析度(epsilon)是 blob mesh 的 IoU 主槓桿、
  interior 無關」(strip「rows 決定」的 Delaunay 版)→ 加 `generate(target_verts=N)` 自適應 epsilon;
  修掉真實件才暴露的 orphan bug(`prune_orphans`)。Award 機器人 mesh 實為 **weighted+無 deform**
  → 靜態幾何是對的閘。見 `knowledge/s3-award-mesh-endtoend.md`。
...
1. ~~PSD件→S3 mesh→對照 Award 真實 mesh~~:**✅ 已完成(2026-08-10,見上)**。3 件靜態幾何 all_pass。
   (註:改用 Award atlas 真實 alpha 直接對照,不需 robot_parts.psd 再切;deform 對照因這些件 weighted+無
   deform timeline 而不適用,改判靜態幾何。)
2. **❗最高優先:切圖→Spine JSON 組裝(SkelToJson)**:把「PSD 切件 → S3 mesh(`target_verts` 對齊)→
   Spine attachment」固化成工具 —— 命名 `<PSD名>/<圖層名>`、size+2px padding、mesh vs region 分配。
   端到端產出可載入 Spine JSON。真值:對 Award 既有件比對 attachment 結構。純 CPU 可自驅。
3. **S2 補圖閘 / 骨架閘**(補齊 S2 樞紐;純 CPU)。
4. **weighted 件的變形驗證**:需 S5(骨架/權重)—— 目前對 weighted 件只保證靜態幾何。
5. **S1 反推分析器**:需一支 benchmark 影片(repo 無影片資產)。
6. ~~spine_inspector 實機 round-trip~~:**⛔ CDN(jsDelivr)被網路政策擋(403);需使用者改政策或提供離線 spine-webgl。**

> S3 已對真實生產 mesh 端到端驗收通過。建議下一步走候選 2(SkelToJson 組裝),把 S4 切件 + S3 mesh
> 串成「件 → 可載入 Spine JSON」,對 Award 既有結構比對。
...
- 2026-08-10:**S3 端到端對真實生產 mesh 驗收(里程碑)** — 對 Award 機器人 3 真實 mesh(光暈/左手/身體)
  用切出的真實 alpha 跑 S3,對齊藝術家頂點成本下生成 IoU 全 ≥ 藝術家(0.986/0.994/0.995)、拓樸乾淨 → all_pass。
  發現「輪廓解析度(epsilon)是 blob mesh IoU 主槓桿、interior 無關」→ 加 `target_verts` 自適應 epsilon;
  修真實件才暴露的 orphan bug(`prune_orphans`)。查明 Award 機器人 mesh 為 weighted+無 deform → 靜態幾何是對的閘。
  新增 `tools/mesh_gen/validate_award_mesh.py`;knowledge `s3-award-mesh-endtoend.md`。
