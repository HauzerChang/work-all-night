...
1. ~~**PSD件→S3 mesh→對照 Award 真實 mesh**~~ **✅ 完成(里程碑,2026-08-10)**:光暈/身體/左手 3 件
   `psd_slice`→`generate_mesh`→對照 Award 藝術家 mesh,靜態幾何三 AC(覆蓋率≥藝術家 / 拓樸 / 頂點預算)
   **3 件全 overall_pass**。發現:這 3 件**無 deform timeline**(weighted-bone 類,與窗簾逐頂點 deform 不同類)→
   real-deform 閘標 N/A,改用靜態幾何。`auto` 正確把 blob 路由 v1;v1 預設 eps=0.008 對羽化/凹形件覆蓋不足
   (光暈/左手)→ 加 **adaptive 加密**自我校準到藝術家 baseline。工具 `tools/mesh_gen/compare_to_award.py`。
   詳見 `knowledge/s3-psd-to-award-mesh.md`。**未改既有工具 → main_draw 回歸不受影響(抽驗 curtain_left 過)。**
2. **❗最高優先:切圖→Spine JSON 組裝(SkelToJson)**:把 `機器人拆件/<圖層名>` 命名慣例 + size+2px padding
   + mesh/region 分配 + adaptive mesh 密度 固化成「件→Spine attachment」
...
> S3+S4 已串成端到端並對 Award 真實 mesh 驗收通過(候選 #1 完成)。建議下一步:候選 #2
> SkelToJson(件→Spine JSON 組裝),把已固化的所有慣例 + adaptive mesh 密度端到端產出可用 Spine JSON;
> 之後才是 weighted-bone 權重(BBW,S3 路線圖後段,複現 Award mesh 的骨骼變形行為)。
...
- 2026-08-10:**S3+S4 端到端對真實 Award mesh 驗收(里程碑)** — robot_parts.psd 的 光暈/身體/左手
  切件→`generate_mesh`→對照 Award 藝術家 mesh,靜態幾何三 AC(覆蓋率≥藝術家 / 拓樸 / 頂點預算)3 件全過。
  發現:這 3 件無 deform timeline(weighted-bone 類);`auto` 正確路由 blob→v1;v1 預設 eps=0.008 對羽化/凹形件
  覆蓋不足 → 新增 `compare_to_award.py` 含 adaptive 加密自我校準到藝術家 baseline。未改既有工具(無回歸)。
