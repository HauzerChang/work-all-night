...
- **S3×S4 端到端串接:對真實生產標的驗收通過(里程碑,2026-07-18)** — `robot_parts.psd`
  的 3 個 mesh 件(光暈/身體/左手)→ `generate_mesh_v2` → 對照 `Award` 真實 mesh:靜態覆蓋
  IoU 同級或更好、頂點少 40~55%,3/3 PASS。發現 mesh **兩種 regime**(deform-driven 高瘦→strip;
  骨骼-driven 團塊→Delaunay),`v2 auto` 分流正確。見 `knowledge/s3-award-end-to-end.md`。
  指令 `python3 tools/mesh_gen/validate_against_award.py`。
- S1 / S5 尚未開始。
...
1. **❗最高優先:切圖→Spine JSON 組裝(SkelToJson)**。把已固化的慣例
   (`PSD名/圖層名` slot 命名、size+2px padding、mesh/region 分配、atlas 0.70 縮放)寫成
   「件清單 → Spine skeleton JSON」產生器,端到端吐出可載入的 spine(有 `Award` 真值可逐欄位對照)。
   ← 這會把 S3+S4 收斂成單一 pipeline(PLAN 第 3 階段的第一塊)。純 CPU 可自驅。
2. **S2 補圖閘 / 骨架閘**(補齊 S2 樞紐;純 CPU)。
3. **weighted mesh 的 deform 閘**(需先做 BBW 權重生成;牽動 S5)。Award 3 件靠骨骼權重變形,
   目前僅驗靜態覆蓋——deform 穩健性待權重生成後才可量測。
4. **S1 反推分析器**:需一支 benchmark 影片(repo 無影片資產)。
5. ~~PSD件→S3 mesh→對照 Award~~:**✅ 已完成(2026-07-18)**,見上。
6. ~~spine_inspector 實機 round-trip~~:**⛔ CDN(jsDelivr)被網路政策擋(403);需使用者改政策或提供離線 spine-webgl。**

> S3×S4 端到端已對真實標的驗收通過。建議下一步:(1) SkelToJson 組裝器,把件清單自動組成 Spine JSON,
> 對 `Award` 逐欄位比對——這是通往第 3 階段(整合 pipeline)的第一步。
...
- 2026-07-18:**S3×S4 端到端驗收(里程碑)** — `validate_against_award.py`:robot_parts.psd 的
  3 個 mesh 件 → generate_mesh_v2 → 對照 Award 真實 mesh,靜態覆蓋 IoU 同級/更好、頂點少 40~55%,3/3 PASS。
  發現 mesh 兩種 regime(deform-driven 高瘦→strip;骨骼-driven 團塊→Delaunay),v2 auto 分流正確。
  負對照(平移/縮放)確認覆蓋閘鑑別力;誠實標記 weighted deform 穩健性未驗(無位移真值)。
  產出對照圖 `knowledge/figures/award-mesh-gen-vs-artist.png`。下一步定為 SkelToJson 組裝器。
