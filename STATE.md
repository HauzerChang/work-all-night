...
**S3×S4 端到端閉環達成(里程碑,2026-07-29)**:PSD 件 → S3 auto mesh → 對照 Award 真實生產 mesh。
- 新增 `tools/mesh_gen/validate_psd_to_award.py`:對 Award 3 個 **weighted** mesh 件(光暈/身體/左手,
  **無 deform timeline**,`validate_against_real` 的真實 deform 閘不適用)做靜態覆蓋對照。
- 結果:身體 0.986 / 左手 0.982 **覆蓋率超過藝術家**(0.976 / 0.968)且頂點少 ~30%;光暈 0.966 vs 0.980
  (差 0.014,在 atlas 0.70 縮放 + 羽化邊的 <2% 重採樣噪聲 margin 內)。mesh-對-mesh 輪廓一致 0.95~0.97。
- 方法關鍵:(a) 用 `extract()` 直立裁圖當**共同畫布**;(b) 發現 **Award mesh uvs 為 region-local**([0,1]
  over region,非 atlas 頁面 UV)→ `px=(u·W,v·H)` 直接映射;(c) **4 朝向 oracle 負對照**(direct 0.98 vs
  vflip/hflip/rot180 0.4~0.76)當場抓到第一版「誤當頁面 UV+CW 旋轉」的轉換 bug(baseline 崩 0.0)。
- 副產發現:**覆蓋率由邊界取樣密度(epsilon)決定,內部點無關**(同 strip「rows 決定」原理)→
  v2 Delaunay 回退預設 epsilon 0.008→0.004(main_draw 4 mesh 走 strip,已重驗未回歸)。詳見
...
1. ~~PSD件→S3 mesh→對照 Award 真實 mesh~~ ✅ **已完成(2026-07-29 里程碑,見上)**。
2. **❗最高優先:件→Spine JSON 組裝器(SkelToJson)**。把 `PSD名/圖層名` 命名慣例 + size+2px padding
   + region-local uvs + 生成 mesh(已證達生產水準)固化成工具:吃 PSD 切件 manifest → 產可用 Spine
   skin/slots/attachments JSON。真正端到端「PSD → Spine 檔」。純 CPU 可自驅。
...
> S3×S4 端到端已閉環(件→mesh 達生產水準)。建議下一步:攻候選 2(SkelToJson 組裝器),把生成 mesh
> + 命名/padding/uv 慣例組出完整 Spine JSON。註:生成為 unweighted mesh,真實件為 weighted(靠骨變形);
> 對映真實綁定需 S5 骨架 + BBW 權重(未做)→ 目前端到端結論限**靜態幾何/覆蓋**。
...
- 2026-07-29:**S3×S4 端到端閉環(里程碑)** — PSD 件→S3 auto mesh→對照 Award 3 個 weighted mesh 件真值。
  身體/左手覆蓋率**超過藝術家**且頂點少~30%,光暈在 2% 重採樣噪聲內。新增 `validate_psd_to_award.py`
  (4 朝向 oracle 負對照抓到「誤當頁面 UV」的轉換 bug → 發現 **Award uvs 為 region-local**)。
  發現覆蓋率由邊界密度決定 → v2 Delaunay 回退 epsilon 0.008→0.004(main_draw strip 路徑未回歸)。
