...
- **S3×S4 端到端 vs Award 真實 mesh:3 件全 PASS(里程碑,2026-07-11)** — PSD 件(光暈/身體/左手)→
  `generate_mesh_v2`(mode=auto)→ 對照 Award 藝術家 mesh。生成 mesh 全有效且覆蓋率 IoU 達/勝藝術家
  (身體 0.966 勝藝術家 0.948、頂點還少四成)。發現:auto 對 blob 件正確選 Delaunay;藝術家頂點多是為
  weighted deform 控制密度(非輪廓覆蓋)→ S3 尚未生成「均勻控制網格」是誠實邊界。
  這 3 件 weighted 且無 deform timeline,故不做 deform 轉移閘。見 `knowledge/s3-award-mesh-endtoend.md`。
...
1. ~~**PSD件→S3 mesh→對照 Award 真實 mesh**~~ → ✅ **已完成(2026-07-11)**,3 件全 PASS,見上。
2. **❗最高優先:切圖→Spine JSON 組裝(SkelToJson)**。把 `機器人拆件/<圖層名>` 命名慣例 + size+2px padding +
   生成 mesh 固化成「件→Spine attachment」寫出工具,端到端產出可載入的 Spine JSON。
   自驗:輸出的 JSON schema 合法、slot/attachment 命名符合 Award 慣例、mesh 頂點在預算內。
   這是把 S3+S4 真正串成「產物」的關鍵一步。
3. **S3 生成「均勻控制網格」變體**(給 weighted deform 用,見 s3-award-mesh-endtoend 發現 3),需 BBW 權重才完整。
...
> S3+S4 端到端已對真實生產 mesh 驗收通過。建議下一步:候選 2(SkelToJson,把 S3+S4 串成可載入產物),
> 有清楚的真實命名慣例可固化、純 CPU 可自驅。
...
- 2026-07-11:**S3×S4 端到端 vs Award 真實 mesh(里程碑)** — PSD 件(光暈/身體/左手)→ `generate_mesh_v2` →
  對照 Award 藝術家 mesh,3 件全 overall_pass(生成 mesh 有效 + 覆蓋率 IoU 達/勝藝術家;身體 0.966 勝 0.948 且頂點少四成)。
  發現:auto mode 對 blob 件正確選 Delaunay(非 strip);藝術家頂點多是為 weighted deform 控制密度,非輪廓覆蓋
  → S3 尚未生成「均勻控制網格」為誠實邊界。負對照矩陣(對角 .95~.98 / 錯配 .48~.58)確認覆蓋率鑑別力。
  新工具 `compare_award_mesh.py`;`knowledge/s3-award-mesh-endtoend.md`。下一步定為 SkelToJson(件→Spine JSON 組裝)。
