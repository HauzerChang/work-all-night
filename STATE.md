...
- **S3×S4 端到端:對真實生產 mesh 驗收通過(里程碑,2026-08-16)** — `validate_psd_to_mesh.py`
  把 `robot_parts.psd` 的 3 個 mesh 件(光暈/身體/左手)跑 `generate_mesh_v2`,對照 Award 真實藝術家 mesh:
  **production-critical(覆蓋率+格式)3/3 全過**,自動 mesh 以約半數頂點達藝術家覆蓋率(gen 38–50v vs artist 78–98v)。
  過程修 2 缺陷:① 生成器孤兒頂點 bug(`compact_vertices`,main_draw 回歸全過)② deform 韌性 gate 第 4 次
  miscalibration(絕對位移未依件尺寸縮放)→ 依目標件等比縮放校正後藝術家真值全 clean 可信;鑑別出光暈(blob→v1 散點)
  韌性缺口。見 `knowledge/s3-psd-to-mesh-award.md`。
- S1 / S5 尚未開始。
...
1. ~~PSD件→S3 mesh→對照 Award 真實 mesh~~ **✅ 完成(2026-08-16 里程碑)**,見上。
2. **❗最高優先:切圖→Spine JSON 組裝(SkelToJson)**。把已驗證的 3 條慣例固化成「件→Spine attachment」工具:
   ① slot/name = `<PSD檔名>/<圖層名>` ② attachment w/h = 件尺寸 +2px ③ 會 warp 的件用 `generate_mesh_v2`
   產 mesh、剛體件(右手/頭)用 region。端到端從 `robot_parts.psd` 產出可載入的 Spine JSON attachment 區塊,
   並用「重建的 attachment ⇄ Award 真值」逐件比對驗收。這是把 S3+S4 成果收斂成單一 pipeline 工具的關鍵一步。
3. **blob 專用生成模式**:光暈類 aspect<1.2 圓形/塊狀件目前落 v1 散點、強 warp 下自交。做邊界密集扇形
   (仿藝術家 78-hull 光暈)補韌性缺口。屬新生成能力(見 `knowledge/s3-psd-to-mesh-award.md` 後續候選)。
...
> S3×S4 端到端已對真實生產 mesh 驗收通過。建議下一步:(2) SkelToJson —— 把三條真實慣例固化成
> 「件→Spine attachment」組裝工具,端到端從 PSD 產出可載入 Spine JSON,對 Award 真值逐件驗收。
