...
- **S3×S4 端到端:PSD 切件→mesh→對照 Award 真值,驗收通過(里程碑,2026-07-31)** —
  `compare_to_award.py`:robot_parts 3 mesh 件(光暈/身體/左手)跑 generate_mesh_v2,覆蓋率 IoU
  全在藝術家基準 ±3% 內且頂點更省(35/60/59 vs 78/98/80)。座標系由藝術家自身 IoU≈0.95 反證
  (uvs region-local,免處理 atlas rotate;禁用 mesh vertices 做貼圖比對)。塊狀件 auto-router 正確
  落 v1 delaunay(這 3 件無 deform 故安全)。見 `knowledge/s3-psd-to-award-mesh.md`。
- S1 / S5 尚未開始。
...
1. ~~PSD件→S3 mesh→對照 Award 真實 mesh~~:**✅ 完成(2026-07-31)** — 見上「S3×S4 端到端」與
   `knowledge/s3-psd-to-award-mesh.md`。工具 `compare_to_award.py`,3 mesh 件全過。
2. **❗最高優先:切圖→Spine JSON 組裝(SkelToJson 寫端)**。把已驗證的三件套固化成組裝工具,端到端吐
   可載入 Spine JSON:①slot 命名 `<PSD檔名>/<圖層名>` ②size+2px atlas padding ③region-local uvs
   ④mesh 幾何走 generate_mesh_v2、region 件走矩形 attachment ⑤合成 atlas rect + placeholder。
   輸出可直接用 `compare_to_award.py`(對 Award)與 `evaluate_slicing.py` 迴歸。純 CPU 可自驅。
...
> S3×S4 端到端已對真實生產 mesh 驗收通過(輪廓保真達生產品質、頂點更省)。建議下一步:(2) 寫
> SkelToJson 組裝端,把「PSD 件 → 可載入 Spine JSON」這條最後的產出管線接通,並用本次的 Award 真值迴歸。
