...
- **S3×S4 端到端串接:對 Award 真實 mesh 驗收通過(里程碑,2026-07-06)** — 把機器人拆件
  光暈/身體/左手三件的真實輪廓(atlas region alpha)餵 `generate_mesh_v2`,生成 mesh 覆蓋率
  **達/超藝術家(0.98)且頂點少 32–55%**。發現**覆蓋率由 hull epsilon 決定、內部點無關**;
  blob 路徑加**覆蓋率驅動 epsilon 精修**(`target_cov`,strip 路徑無回歸)。
  工具 `compare_gen_vs_artist.py`;見 `knowledge/s3-vs-award-real-mesh.md`。
- S1 / S5 尚未開始。
...
1. ~~PSD件→S3 mesh→對照 Award 真實 mesh~~ **✅ 完成(2026-07-06,見上/knowledge)**。
2. **❗最高優先:切圖→Spine JSON 組裝(SkelToJson)**。把 `機器人拆件/<圖層名>` 命名慣例
   + size+2px padding + mesh/region 分配 + S3 生成 mesh 固化成「件→Spine attachment」寫出工具,
   端到端產可載入的 Spine JSON。這是把 S3/S4 成果組成 pipeline 的下一塊。
3. **S3 unweighted→weighted(BBW)**:藝術家 mesh 為 weighted;要完全對齊綁定需加骨權重。
...
> S3×S4 端到端「件→mesh」已對真實生產標的(Award)驗收通過。建議下一步做 SkelToJson
> 把件+mesh 組成完整 Spine JSON(候選 2),或補 S3 weighted(候選 3)以完全對齊藝術家綁定。
