...
**✅ 候選1 已完成(里程碑,2026-08-07):PSD件→S3 mesh→對照 Award 真實 mesh**。
`compare_award_mesh.py` 對機器人 3 件(光暈/身體/左手,Award 中為 mesh)跑 `generate_mesh_v2`,
與 Award 真實生產 mesh 做靜態幾何對照,**4 條 AC 3 件全 overall_pass**:生成用更少頂點
(35/60/59 vs 藝術家 78/98/80)達同等覆蓋(IoU 0.93~0.97;兩者一致性 0.92~0.96)。
關鍵:**真值證實 uvs=region-local;v1 Delaunay 對「團塊狀」真實件通用(auto 正確回退,與 v2-strip 條狀件互補)**。
限制:這 3 件無 deform timeline → 只驗靜態幾何,未加成 deform 穩健(見 `knowledge/s3-award-mesh-endtoend.md`)。

下一個 bounded chunk 候選:
1. **切圖→Spine JSON 組裝(SkelToJson)**:把已確認慣例(uvs=region-local、`PSD名/圖層名` slot 命名、
   size+2px padding、atlas 0.70 縮放)固化成「件→Spine mesh attachment」工具,端到端產 Spine JSON
   → 補上 S3/S4 目前唯一缺的「寫出」端。純 CPU 可自驅,是最自然的下一步。
...
> S3+S4 端到端已對真實生產 mesh 真值驗收通過(靜態幾何)。建議下一步:候選1 SkelToJson,把
> 「件→Spine attachment」寫出端補齊,S4→S3→JSON 完整貫通。機器人件的 deform 驗需骨骼/權重路徑(S5 相關)。
...
- 2026-08-07:**S3 端到端對照 Award 真實 mesh(里程碑)** — `compare_award_mesh.py` 把 PSD→件→
  `generate_mesh_v2` 接到真實生產 mesh 真值(機器人 3 件),4 條靜態 AC 3 件全過;生成更精簡頂點達同等覆蓋。
  真值證實 uvs=region-local、v1 Delaunay 對團塊件通用(與 v2-strip 互補)。限制:這 3 件無 deform,只驗靜態。
  見 `knowledge/s3-award-mesh-endtoend.md` + `figures/s3-award-mesh-overlay.png`。
