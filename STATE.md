...
- **S3×S4 端到端(里程碑,2026-07-25)**:`robot_parts.psd` 3 個 mesh 件(光暈/身體/左手)經
  `psd_slice`→`generate_mesh_v2(auto)`,**靜態覆蓋率全落在 Award 真實藝術家 mesh ±0.03 內、頂點少 40~55%**
  (`tools/mesh_gen/compare_to_award.py` overall_pass)。v2 auto 對 3 blobby 件全正確回退 v1(非-strip 首驗)。
  校驗 Award mesh uvs 為 **region-local v-top-down**(修正 sess006「需轉 region 局部」誤判)。
...
1. **❗最高優先(承接本次缺口):BBW 自動骨權重 → 生成 mesh 綁 Award 對應骨 → deform 對等對照**。
   本次靜態覆蓋率已過,但 Award robot mesh 為 **weighted、無 deform timeline**(變形靠骨權重),
   本生成器產 unweighted mesh,`validate_against_real` 的位移場轉移閘不適用。要補變形穩健度對照,
   須先做 BBW(bounded biharmonic weights)自動骨權重(S3 roadmap 未完項),再用 Award 對應骨的
   pose 差分做位移對照。純 CPU 可自驅。詳見 `knowledge/s3-psd-to-award-mesh.md` 缺口段。
2. **切圖→Spine JSON 組裝(SkelToJson)**:把 `機器人拆件/<圖層名>` 命名慣例 + size+2px padding +
   本次校驗的 region-local v-top-down uv 慣例,固化成「件→Spine attachment」寫出工具,端到端產 Spine JSON。
...
> S3×S4 端到端已對真實生產標的(靜態)驗收通過。建議下一步:(1) 做 BBW 骨權重補上「變形對等對照」缺口
> ——本次唯一未覆蓋的維度;或 (2) SkelToJson 把件→Spine JSON 組裝固化(命名/尺寸/uv 慣例都已就緒)。
...
- 2026-07-25:**S3×S4 端到端驗收(里程碑)** — `compare_to_award.py`:robot_parts.psd 3 mesh 件經
  psd_slice→generate_mesh_v2(auto),靜態覆蓋率全落 Award 藝術家 mesh ±0.03(光暈−.019/身體+.012/左手−.025)、
  頂點少 40~55%,overall_pass。v2 auto 對 3 blobby 件全正確回退 v1(非-strip 首驗)。校驗 Award mesh uvs
  為 region-local v-top-down(修正 sess006 誤判)。**發現 deform 真值缺口**:robot mesh weighted 無 deform
  timeline,變形對照須先做 BBW 骨權重(定為下一步)。
