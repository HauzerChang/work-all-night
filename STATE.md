...
**✅ 已完成(2026-08-06):PSD件→S3 mesh→對照 Award 真實 mesh(端到端里程碑)。**
- `tools/mesh_gen/validate_psd_to_mesh.py`:robot_parts.psd 光暈/左手/身體 → psd_slice 切件 →
  generate_mesh_v2 → 對照 Award artist mesh 覆蓋率。**降預設 epsilon 0.008→0.004 後 3 件 IoU 全過
  artist baseline(0.96/0.98/0.98 ≥ 0.95/0.98/0.95)且頂點更精簡(44/70/69 ≤ artist 78/80/98)**。
- **關鍵發現:Award 3 個機器人 mesh 全是 weighted(骨綁 4_LEG/4_LEG5/4_LEG3)、無 deform timeline**
  → 現有 `transfer_deform_check` 變形閘(為 unweighted+deform 打造)對它們 **N/A**。端到端只驗到
  「切件 + 拓樸/覆蓋率」半邊;**變形正確性驗收缺 BBW 權重生成器(S3 未建元件)**。
- 無回歸:epsilon 改動只影響 v1 delaunay 路徑(v2 strip 與 epsilon 無關);v1 curtain_left IoU 0.98→0.99
  仍 deform 乾淨,v2 4-mesh 三件 overall_pass 不變。詳見 `knowledge/s3-psd-to-mesh-award.md`。

下一個 bounded chunk 候選:
1. **❗最高優先(承接本次缺口):S3 weighted(BBW)mesh 生成 + weighted 變形閘**。為生成 mesh 算骨權重
   (BBW / bone-distance heat),擴 `deform_eval` 支援 weighted 攤平格式(`[骨數,骨idx,bx,by,w,...]`),
   讀 Award 骨姿勢驅動 weighted 變形後比幾何 → 把 S3 從「靜態覆蓋率」推到「真實骨綁變形正確性」。
...
> 建議下一步:攻 #1(weighted mesh 生成 + 閘)—— 這是本次揭示的 S3 最大缺口,有 Award weighted mesh
> 當真值、純 CPU 可自驅,能把 S3 端到端補到「變形正確性」層級。
...
- 2026-08-06:**S3×S4 端到端對照 Award(里程碑)** — `validate_psd_to_mesh.py`:robot PSD 3 mesh 件
  切件→生成→對 Award artist mesh 覆蓋率。降預設 epsilon 0.008→0.004,3 件 IoU 全過 baseline 且更精簡。
  **發現 Award 機器人 mesh 全 weighted(骨綁)無 deform → 變形閘 N/A,S3 缺 BBW 權重生成器**。無回歸。
