...
- 狀態:🔶 切圖 pipeline 對真實檔驗收通過(尚可,美術人員評估);補圖 CPU 基線+閘完成後**凍結**。
- **📌 2026-07-06 使用者拍板重定向**:
  - **切圖 → 深化(P1-A)**:精準切割、圖片認知、部位認知、分件精準度(邊緣閘/認知庫/複雜 PSD 結構)。
  - **補圖 → 定版 v1「填滿區塊」+ 外部合作(P1-B)**:理想補圖=語意完整性,**Claude 是理解型
    多模態、非生成型**,像素生成須外接生成式 AI(ChatGPT/gpt-image-1、SD 等)。研究 adapter 契約:
    我方產(件+遮罩)→ 外部生成 → 回圖過閘(inpaint_eval)+ vision 自評 + 人審。
...
- **Claude 是理解型多模態、非生成型**(2026-07-06):看得懂圖(可當語意評審/認知引擎),
  但不能生成像素 → 語意補圖必須外接生成式 AI;Claude 的價值在「認知+把關」(切圖精準度、
  部位認知、生成結果的自動評審),不在像素生成本身。
