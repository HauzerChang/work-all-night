...
## 動畫品質研究方向(使用者新增,2026-08-24)

- **各部位「時間差」= 律動鮮活的關鍵**(overlapping action / follow-through / 慣性)。身體各部位**非同步**移動:
  主軸先動、末梢因慣性延遲跟上並過衝回穩;同步 = 紙板感。物理類比:每關節為被上游牽引的**阻尼彈簧**
  (剛性→相位延遲/衰減)。**已在 Award_Legend_Loop 真實資產量測證實**美術確用時間差。
  併入 S1 候選 0d(分鏡→keyframe):生成時套用「τ∝骨鏈深度 + 阻尼 follow-through」,並配相位延遲剖面評估器。
