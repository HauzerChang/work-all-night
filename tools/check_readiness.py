...
BLOCKS = [
    ...
    },
    {
        ...
        "caps": [
            CAP("pivot_gate", "pivot 推斷閘(多 rig 真值+負對照)", "L2",
                "python3 tools/rig/validate_pivots.py", "eval",
                note="2 rig 藝術家真值(robot 3 關節 + cat 5 關節)各 AC1-3 全 PASS;隨機/互換/rect 三負對照皆有鑑別力"),
            CAP("contact_seam_infer", "接觸縫 pivot 推斷器", "L2",
                "python3 tools/rig/validate_pivots.py", "gen",
                note="跨 rig 通用:robot(mesh 件)+cat(全 region、左右手共用鏡射貼圖)共 7 關節皆 <10% 軀幹尺度、勝質心 baseline;軸向精修屬美術(A類)"),
            CAP("pivot_end2end", "多 rig + 接 build_spine 骨樹生成", "L1", None, "pipeline",
                note="多 rig ✅(robot+cat 2 rig 全 AC PASS,證跨資產通用);仍缺:pivot→bone 父子樹寫入 build_spine"),
        ],
    },
]
