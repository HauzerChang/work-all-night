...
#
# 候選 9(session 011)延伸:再測「右手←頭」「身體←頭」等小面積配對(<13%),外加
# 「頭←右手」(頭本身內容只有 6405px,遮擋比例反而高達 46.5%——同一組配對兩個方向面積
# 懸殊,值得都測)、「頭←身體」(12.9%)。排除「X←光暈」全系列(頭/右手/身體/左手
# 被光暈完全覆蓋,frac=1.0,整層內容清空,屬於候選 8/taxonomy 講的「情境 2:原圖不存在
# 內容」而非局部遮擋補洞,不適用本方法論)。
PAIRS = [
    ("光暈", "身體"),
    ("光暈", "右手"),
