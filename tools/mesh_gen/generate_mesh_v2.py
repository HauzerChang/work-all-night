...
def generate(path, rows=10, cols=3, mode="auto", target_coverage=None, vertex_budget=64):
    ...
    if not use_strip:
        ...
        # target_coverage:非 strip 的不規則件(如發光暈)靠自適應 hull 解析度達覆蓋目標。
        m, _ = gen_v1(path, target_coverage=target_coverage, vertex_budget=vertex_budget)
        m["_mode"] = "delaunay-v1"
        return m
