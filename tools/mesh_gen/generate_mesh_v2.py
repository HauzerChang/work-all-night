...
def generate(path, rows=10, cols=3, mode="auto", target_iou=None, vertex_budget=64):
    ...
    if not use_strip:
        ...
        # blobby/低長寬比件回退 v1 Delaunay;若給 target_iou 則開自適應覆蓋(對齊藝術家)。
        m, _ = gen_v1(path, target_iou=target_iou, vertex_budget=vertex_budget)
        m["_mode"] = "delaunay-v1"
        return m
