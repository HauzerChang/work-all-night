...
def generate(path, rows=10, cols=3, mode="auto", epsilon=0.002):
    ...
    if not use_strip:
        # 非 strip 件(靜態/bone-driven，如機器人光暈/身體/左手):走 Delaunay-v1。
        # epsilon=0.002 為對真實 Award 藝術家 mesh 校準的操作點:靜態輪廓 IoU 由
        # Douglas-Peucker epsilon 決定,0.008(v1 預設)對有機/羽化邊過粗;0.002 在
        # 「頂點數 ≈ 藝術家預算」下達到 IoU ≥ 藝術家基準(見 knowledge/s3-award-mesh-static-iou.md)。
...
def main():
    ...
    ap.add_argument("--epsilon", type=float, default=0.002,
                    help="Delaunay 回退的 Douglas-Peucker 邊界簡化比例(靜態件輪廓 IoU 操作點)")
    a = ap.parse_args()
