...
def run():
    ...
    # (c) count 只作用於 combo:設 tier_combo_hits 不得改動任何**非-combo** 主秀 beat 的檔位變體。
    # 直接比對 full(gains+hits)vs amp_only(gains-only)的非-combo 變體逐位元相同 → 精確隔離
    # combo 連擊數的效應(不倚賴 impact-peak 啟發式;對 squash 等體積守恆/非-scale-overshoot 節拍亦穩健,
    # 其檔位差異純屬 (J) 幅度增益、與 combo count 無關 → 兩 build 對它逐位元相同)。
    leak = []
