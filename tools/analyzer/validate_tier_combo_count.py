...
def run():
    ...
    #     以「帶/不帶 tier_combo_hits 的變體逐位元相同」(full vs amp_only)認定 —— 比舊「各檔位峰數
    #     不變」更精確:candidate G-4''''' 讓 squash 入 MAIN_SHOW_CATS,其 scaleX overshoot 會隨檔位
    #     跨過 impact 門檻(1.10)使 `_min_peaks` 峰數變動,但那是**幅度**效應(耦合 amplify),非 count
    #     外洩;逐位元比對只認 count 真正改動了結構,squash/wobble/hit 等皆應與 amp_only 相同。
    leak = []
