...
# (G-4''''''-vol)voltwist 加入(反相雙軸 shear + 均勻面積補償 scale → 完整 det≡1)成為第四個合法 shear 產出者。
SHEAR_CATS = {"wobble", "squash", "twist", "voltwist"}

# candidate G-4''''''-vol — 產出**第二條 shear 軸 shearY**(反相雙軸)的類別(dual-axis / shearY-emitting)。
# 原僅 twist(G-4'''''' 首度驅動 shearY);voltwist 加入後(反相雙軸 shear + 均勻補償 scale)成為第二個。
# shearY-isolation 閘(twist_gen TW6c「twist 獨佔 shearY」)以此集合認定「合法 shearY 產出者」,集中一處便於
# 後續再加(避免每加一個 shearY 節拍就改硬編碼 'twist')—— 同 SHEAR_CATS 對 shearX 產出者的作法。
SHEARY_CATS = {"twist", "voltwist"}
