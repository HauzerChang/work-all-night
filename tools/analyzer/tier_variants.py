...
# (G-4''''''-vol)twistvol 加入(反相雙軸 shear + 均勻體積守恆 scale):它同帶 shear(且首度同時帶
# coupled scale),故為合法 shear 產出者;各 shear-isolation 閘以此集合認定,twistvol 的 shear 不算洩漏。
SHEAR_CATS = {"wobble", "squash", "twist", "twistvol"}

# candidate G-4''''''-vol — 驅動**第二條 shear 軸(shearY)**的節拍類別。原僅 twist(反相雙軸 shear);
# twistvol 加入(twist + 均勻體積守恆 scale,shear 通道完全同 twist,故亦驅動 shearY)。
# 各「shearY 隔離」閘(如 validate_twist_gen TW6c「twist 獨佔 shearY」)以此集合認定合法 shearY 產出者,
# 集中一處便於後續再加(避免每加一個 shearY 節拍就改多個閘的硬編碼 'twist')。
SHEARY_CATS = {"twist", "twistvol"}
