...
# candidate (G-4''''') — 加入 `squash`(斜拉果凍擠壓,G-4'''' 的 shear + 耦合體積守恆 scale 節拍):
# 亦為主秀,強度應隨檔位遞增;惟其 scale 通道是**體積守恆的非均勻擠壓**(scaleX=1+q、scaleY=1/(1+q)),
# 逐軸獨立放大會破壞 scaleX·scaleY≡1(舊 `_amp_scale` 只放大上方、壓扁樓地板不動 → 積≠1);
# 故 squash 需**耦合 amplify**(見 COUPLED_SCALE_CATS / `_amp_scale_coupled`),補上 G-4'''' 的 honest boundary。
...
# candidate G-4''''' — squash 已接檔位差異化(見 COUPLED_SCALE_CATS);兩者皆在 MAIN_SHOW_CATS。
SHEAR_CATS = {"wobble", "squash"}

# candidate G-4''''' — scale 通道為**體積守恆非均勻擠壓**的節拍(需耦合 amplify)。
# 這些類別的 scale 幀滿足 scaleX·scaleY≡1(squash & stretch);逐軸獨立放大(`_amp_scale`)會把
# 拉長軸放大、壓扁軸的樓地板保留 → 破壞面積守恆。改用 `_amp_scale_coupled`:放大擠壓量、以倒數
# 導出另一軸,使 scaleX·scaleY 仍≡1、阻尼比不變、identity 幀不動。build_animations 依 cat 路由是否耦合。
# (非耦合類別如 hit/combo/charge 的 scale 是等比 pulse + anticipation 樓地板,仍走逐軸 `_amp_scale`。)
...
def _amp_scale_coupled(sx, sy, g):
    """candidate G-4''''' — **體積守恆**的耦合 scale 幅度增益(squash & stretch)。

    輸入為一組非均勻擠壓幀(scaleX·scaleY≈1,一軸拉長一軸壓扁)。放大**擠壓量** q(以拉長軸
    =max(sx,sy)−1 認定,對稱處理拉長在 x 或 y 兩種情形、絕不換軸),再以倒數導出另一軸:
        拉長軸' = 1 + g·q       (線性放大 q → 阻尼比 r 逐極值不變)
        壓扁軸' = 1 / 拉長軸'    (面積守恆:scaleX'·scaleY' ≡ 1)
    identity 幀 (1,1) → q=0 → (1,1) 不動(介面契約對所有檔位保持);g=1.0 → 逐位元同輸入
    (擠壓量原樣;拉長軸=1+1·q=sx、壓扁軸=1/sx≈sy → 向後相容)。等比 pulse(sx==sy>1)不會被送進
    此路徑(僅 COUPLED_SCALE_CATS 路由),故不會把等比脹縮誤扭成擠壓。"""
    hi = max(sx, sy)                       # 拉長軸值(squash 幀 ≥1;identity 為 1)
    q = hi - 1.0                           # 擠壓量(≥0)
    hi2 = 1.0 + g * q                      # 放大擠壓(線性 → 阻尼比保形)
    lo2 = 1.0 / hi2 if hi2 else 1.0        # 倒數 → 面積守恆
    return (hi2, lo2) if sx >= sy else (lo2, hi2)


def amplify_bone_tl(b, g, coupled_scale=False):
    """對單一 bone timeline 套幅度增益 g(deepcopy,保留曲線鍵)。g=1.0 → identity 變換。

    `coupled_scale=True`(squash 等體積守恆擠壓節拍):scale 通道走 `_amp_scale_coupled`
    (放大擠壓量、倒數導另一軸,scaleX·scaleY≡1 守恆);否則逐軸獨立 `_amp_scale`(等比 pulse +
    anticipation 樓地板)。shear/rotate/translate 兩路徑相同(對 0 對稱 → v'=g*v)。"""
    b = copy.deepcopy(b)
    for f in b.get("scale", []):
        if coupled_scale:
            sx, sy = _amp_scale_coupled(f["x"], f["y"], g)
            f["x"] = round(sx, 4)
            f["y"] = round(sy, 4)
        else:
...
def amplify_anim(anim, g, coupled_scale=False):
    ...
    `coupled_scale`(candidate G-4''''')透傳到 `amplify_bone_tl`,squash 等體積守恆擠壓節拍
    的 scale 通道走耦合放大(scaleX·scaleY≡1 守恆);其餘節拍逐軸獨立放大(向後相容)。"""
    out = {}
    if "bones" in anim:
