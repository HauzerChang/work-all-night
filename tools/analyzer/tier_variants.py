...
# candidate (G-4''''') — 加入 `squash`(shear + 耦合非均勻 scale 的體積守恆擠壓,G-4'''' 的節拍):
# 它也是主秀節拍(愈高檔位愈斜愈擠),但 scale 通道是**體積守恆對**(scaleX·scaleY==1),
# 一般 `_amp_scale`(只放大 identity 上方、下方樓地板不動)會**破壞守恆**;故 squash 走
# **耦合 amplify**(log 空間 v'=v^g,見 VOLUME_CONSERVING_CATS / _amp_scale_coupled)。
...
# candidate G-4''''' — scale 通道為**體積守恆對**(scaleX·scaleY==1)的節拍類別。
# 這類節拍的 scale 檔位放大**不能**用 `_amp_scale`(它只放大 identity 上方、下方樓地板不動 →
# scaleX>1 被放大、scaleY<1 不動 → 積≠1,破壞守恆);必須用 **log 空間耦合 amplify**
# (`_amp_scale_coupled`:v'=v^g)—— 兩軸同以 g 次冪放大 ⇒ 積 (scaleX·scaleY)^g==1^g==1
# (守恆對任意 g 保持)、且非均勻 |scaleX−scaleY| 隨 g 遞增、identity(1)→1^g==1 不動、
# g=1.0 逐位元不變(v^1==v)。build_animations 依 cat∈VOLUME_CONSERVING_CATS 路由到耦合路徑。
...
def _amp_scale_coupled(v, g):
    """candidate G-4''''' — **體積守恆**節拍(squash)的 scale 耦合放大:log 空間 v'=v^g。

    兩軸同以 g 次冪放大 → 積 (scaleX·scaleY)^g;若基底為守恆對(積==1)則放大後仍==1
    (**守恆對任意 g 保持**,與哪一軸拉長無關 → 軸無關/一般)。identity(v==1)→ 1^g==1
    (端點介面契約保持);g==1.0 → v(逐位元向後相容)。v>0 恆成立(scale 值皆正)。"""
    if g == 1.0 or v <= 0.0:
        return v
    return v ** g
...
def amplify_bone_tl(b, g, coupled_scale=False):
    ...
    `coupled_scale`(candidate G-4''''')=True 時,scale 通道走**體積守恆耦合放大**(v'=v^g,
    兩軸同冪 → 積守恆);False(預設)時走 `_amp_scale`(只放大 identity 上方,樓地板不動)。
    shear/rotate/translate 兩路徑一致(對 0 對稱 v'=g*v)。"""
    b = copy.deepcopy(b)
...
def amplify_anim(anim, g, coupled_scale=False):
    ...
    `coupled_scale`(G-4''''')透傳給 `amplify_bone_tl`(squash 等體積守恆節拍走 log 空間 v^g)。"""
    out = {}
    if "bones" in anim:
