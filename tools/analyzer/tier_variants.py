...
# (G-4''''''-vol)vtwist 加入後(反相雙軸 shear + 耦合 uniform scale,full-matrix 體積守恆)成為第四個 ——
# 亦合法帶 shear(且帶 uniform scale;其 scale 均勻 sx==sy,異於 squash 的非均勻,故**不**在 COUPLED_SCALE_CATS)。
...
# (vtwist 尚未併入 MAIN_SHOW_CATS:tier 幅度差異化為後續 honest boundary,同 twist_gen→twist_tier 的推進。)
SHEAR_CATS = {"wobble", "squash", "twist", "vtwist"}
