...
# 它也是主秀節拍,強度理應隨檔位遞增(愈斜、擠得愈狠),但 scale 軸是**體積守恆的耦合對**
# (scaleX·scaleY≡1),不能用逐通道 `_amp_scale`(只放大 identity 上方 → 破壞守恆);需**耦合 amplify**
# (見 `VOLUME_COUPLED_CATS` 與 `_amp_scale_coupled`)。shear 軸同 wobble 對 0 對稱放大。
...
# candidate G-4''''' — scale 通道為**體積守恆耦合對**(scaleX·scaleY≡1)的節拍類別。
# 這些 beat 的檔位幅度放大**不能**用逐通道 `_amp_scale`:squash 的 scaleX>1(拉長)會被放大、
# scaleY<1(壓扁)樓地板被保留 → scaleX·scaleY≠1(體積守恆被破壞)。故對這類 beat 走**耦合 amplify**
# (`_amp_scale_coupled`):放大 stretch 量 q=scaleX−1 → (1+g·q, 1/(1+g·q)),product 恆等 1
# (等價於以 Q'=g·Q 重生成 `gen_squash`)。這正是把 G-4'''' honest boundary(「squash 未接 tier」)接上的關鍵。
VOLUME_COUPLED_CATS = {"squash"}
...
def _amp_scale_coupled(sx, sy, g):
    """candidate G-4''''' — 體積守恆 squash 的**耦合** scale 幅度增益(x/y 一起處理,保 scaleX·scaleY≡1)。

    squash 的 scale 極值為 (scaleX=1+q, scaleY=1/(1+q)),q=stretch 量(scaleX 恆 ≥1)。逐通道 `_amp_scale`
    只放大 scaleX(>1)、保留 scaleY(<1)樓地板 → 破壞面積守恆。耦合作法:抽出 q=scaleX−1、放大成
    q'=g·q,再重建互為倒數的一對 → `(1+g·q, 1/(1+g·q))`,scaleX'·scaleY' **恆等 1**(面積守恆)、
    scaleX'≠scaleY'(仍非均勻)、q=0(identity)→ (1,1) 不動。**等價於以 Q'=g·Q 重生成 `gen_squash`**
    (`gen_squash` 於極值 i 產 q_i=Q·rⁱ → 放大即 g·Q·rⁱ = g·q_i,兩路數值一致)。

    g==1.0 或 q≈0 → 原樣返回(base 檔位逐位元不變、identity 端點不動)。"""
    q = sx - 1.0
    if g == 1.0 or abs(q) < 1e-12:
        return sx, sy
    sx2 = 1.0 + g * q
...
def amplify_bone_tl(b, g, coupled_scale=False):
    ...
    `coupled_scale`(G-4'''''):True 時 scale 走**體積守恆耦合** amplify(`_amp_scale_coupled`,x/y 一起
    重建互為倒數對,保 scaleX·scaleY≡1);False(預設)走逐通道 `_amp_scale`(只放大 identity 上方,
    hit/combo/charge/reveal 的 anticipation squash 樓地板不動)。squash 類走 True,其餘主秀走 False。"""
    b = copy.deepcopy(b)
...
def amplify_anim(anim, g, coupled_scale=False):
    ...
    `coupled_scale`(G-4''''')轉傳給 `amplify_bone_tl`:squash 類 beat 走體積守恆耦合 scale amplify。"""
    out = {}
    if "bones" in anim:
