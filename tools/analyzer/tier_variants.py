...
# 它同樣是主秀節拍,強度(斜拉+擠壓量)理應隨檔位遞增。難點:squash 的 scale 是**體積守恆**對
# (scaleX=1+q 拉長、scaleY=1/(1+q) 壓扁,scaleX·scaleY==1);若沿用 `_amp_scale` 兩軸各自獨立放大
# (只放大 identity 上方 → scaleX 變大、scaleY<1 樓地板不動)會**破壞體積守恆**(積≠1)。故 `amplify_bone_tl`
# 對 scale 通道改用**耦合放大** `_amp_scale_pair`:偵測 squash 對後放大拉長量 q→g·q、另一軸取倒數保積==1。
# 這解除了 G-4''''留下的 honest boundary(「squash 未接 tier 幅度,需耦合 amplify」)。
...
# candidate (G-4''''') — squash 已接檔位差異化:其 scale 為體積守恆對,`amplify_bone_tl` 對 scale 通道
# 改用 `_amp_scale_pair`(耦合放大:偵測 squash 對後放大拉長量、另一軸取倒數保 scaleX·scaleY==1),
# 故 squash 現已在 MAIN_SHOW_CATS(見上)。SHEAR_CATS 仍用於各 shear-isolation 閘認定「合法 shear 產出者」。
...
# candidate (G-4''''') — squash 對的**體積守恆耦合放大**判準。
# squash 幀是「一軸拉長(>1)、一軸壓縮(<1)且 scaleX·scaleY≈1」的對:此時兩軸不可各自獨立放大
# (會破壞面積守恆),須放大拉長量後令另一軸取倒數。判準用「異側 + 積≈1」雙條件辨識(與等比 pulse
# x==y 及一般 overshoot 積≠1 乾淨分離),故只作用 squash 對、對既有等比主秀零影響(向後相容)。
_PAIR_VOL_TOL = 0.02   # |scaleX·scaleY − 1| 上限(gen_squash 4 位小數殘差 <5e-5 → 巨大餘裕)


def _amp_scale_pair(x, y, g):
    """對 (scaleX, scaleY) 一起套幅度增益 g,回傳 (nx, ny)。

    偵測到**體積守恆 squash 對**(異側 + 積≈1)→ 放大拉長量 q→g·q、另一軸取倒數 → **保 nx·ny==1**
    (面積守恆),squash 幅度隨檔位遞增而**非均勻性亦隨之遞增**、體積始終守恆。
    否則(等比脈衝 x==y、或積≠1 的一般 scale)→ 兩軸各自 `_amp_scale` 獨立放大(**原行為,逐位元向後相容**)。"""
    if (x - 1.0) * (y - 1.0) < 0.0 and abs(x * y - 1.0) <= _PAIR_VOL_TOL:
        s = max(x, y)                      # 拉長軸(>1)
        ns = 1.0 + g * (s - 1.0)           # 放大拉長量(同 _amp_scale 對 overshoot)
        no = 1.0 / ns                      # 壓縮軸取倒數 → nx·ny==1(體積守恆)
        return (ns, no) if x >= y else (no, ns)
    return _amp_scale(x, g), _amp_scale(y, g)
...
def amplify_bone_tl(b, g):
    ...
    for f in b.get("scale", []):
        # (G-4''''')scale 兩軸**一起**放大:squash 體積守恆對耦合放大(保積==1)、其餘各自獨立(原行為)。
        nx, ny = _amp_scale_pair(f["x"], f["y"], g)
        f["x"] = round(nx, 4)
        f["y"] = round(ny, 4)
