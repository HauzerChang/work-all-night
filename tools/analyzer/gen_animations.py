...
try:
    ...
    _DISPATCH["twistvol"] = _gen_twistvol  # candidate G-4''''''-vol:反相雙軸 shear + 各向同性體積補償(守完整仿射面積)
    DUR.update(_DUR_EXT)  # 讓 hit/reveal/combo/charge/cascade/wobble/squash/twist/twistvol 時長對 spine_anim.duration 一致
    # 主秀類別置前:exact/substring 命中優先於泛用 pulse。**twistvol 必須排在 twist 之前**(名含 "twistvol"
    # 亦含 "twist" 子字串 → 靠順序讓 twistvol 先命中,避免被 twist 吞掉);squash/twist 亦置前;放 wobble 前避免關鍵字爭用。
