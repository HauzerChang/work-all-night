...
try:
    from beat_templates import gen_hit as _gen_hit, gen_reveal as _gen_reveal, \
        ...
        gen_wobble as _gen_wobble, gen_squash as _gen_squash, \
        ...
        SQUASH_KEYWORDS as _SQUASH_KW, DUR as _DUR_EXT
    ...
    DUR.update(_DUR_EXT)  # 讓 hit/reveal/combo/charge/cascade/wobble/squash 時長對 spine_anim.duration 一致
    # 主秀類別置前:exact/substring 命中優先於泛用 pulse(squash 亦置前;放 wobble 前避免關鍵字爭用)
    _CAT_KEYWORDS = {"cascade": _CASCADE_KW, "combo": _COMBO_KW, "charge": _CHARGE_KW,
                     "squash": _SQUASH_KW, "wobble": _WOBBLE_KW, "hit": _HIT_KW,
...
from tier_variants import MAIN_SHOW_CATS as _MAIN_SHOW_CATS, \
    COUNT_AWARE_CATS as _COUNT_AWARE_CATS, \
    VOLUME_CONSERVING_CATS as _VOLUME_CONSERVING_CATS, amplify_anim as _amplify_anim
...
def build_animations(skeleton, storyboard, tier_gains=None, tier_combo_hits=None, tier_wobble_cycles=None):
    ...
    for beat in storyboard["beats"]:
        ...
        if tier_gains and cat in _MAIN_SHOW_CATS:
            ...
            # candidate G-4''''':squash 的 scale 通道須體積守恆放大(v^g,兩軸同指數 → scaleX·scaleY≡1
            # 不破);其餘主秀走預設 `_amp_scale`(只放大 identity 上方)。依 cat 路由。
            vc = cat in _VOLUME_CONSERVING_CATS
            for tier, g in tier_gains.items():
                ...
                if cnt is not None:
                    ...
                    anims["{}__{}".format(name, tier)] = _amplify_anim(variant, g, vc)
                else:
                    anims["{}__{}".format(name, tier)] = _amplify_anim(anim, g, vc)
    return anims
