...
try:
    ...
    _DISPATCH["twistvol"] = _gen_twist_vol  # candidate G-4''''''-vol:反相雙軸 shear + uniform 面積守恆 scale(det≡1)
    DUR.update(_DUR_EXT)  # 讓 hit/reveal/combo/charge/cascade/wobble/squash/twist/twistvol 時長對 spine_anim.duration 一致
    # 主秀類別置前:exact/substring 命中優先於泛用 pulse(twistvol 置最前,單一 token 'twistvol' 精確命中;
    # squash/twist 置前;放 wobble 前避免關鍵字爭用)
    _CAT_KEYWORDS = {"twistvol": _TWISTVOL_KW, "cascade": _CASCADE_KW, "combo": _COMBO_KW,
                     "charge": _CHARGE_KW, "twist": _TWIST_KW, "squash": _SQUASH_KW,
