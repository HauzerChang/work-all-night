...
def _build_beat(beat, cat, bone_of, cx, cy, count=None, twist_vol_conserve=False):
    ...
    `twist_vol_conserve`(G-4''''''-vol,opt-in)只對 twist 生效:補均勻 scale 使 det≡1(擰而不變面積);
    預設 False → 逐位元同舊 twist(向後相容)。"""
    bones_tl, slots_tl = {}, {}
    limb_seen = 0
    ...
    for pi, pe in enumerate(valid):
        ...
        if cat in _PHASE_AWARE:
            ...
            # twist 另吃 vol_conserve(G-4''''''-vol,opt-in;只有 gen_twist 接此 kw,故以 cat 守衛)。
            kw = {"vol_conserve": True} if (cat == "twist" and twist_vol_conserve) else {}
            if count is None:
...
def build_animations(skeleton, storyboard, tier_gains=None, tier_combo_hits=None,
    ...
    twist_vol_conserve(G-4''''''-vol,opt-in;預設 False → 逐位元同舊 twist):只對 twist 生效,補均勻 scale
    使 Spine local `det≡1`(擰而不變面積)。⚠️ **honest boundary**:此旗標作用於**生成**(base 與段數重生成);
    tier **幅度** amplify 對 vol-conserve twist 的 scale/shear 各自 ×g 是**非線性破守恆**(需比照 squash 的耦合
    amplify 沿守恆流形放大 → 為後續 G-4''''''-vol-tier),故本旗標與 tier_gains 併用時變體暫不再守恆。"""
    ...
    for beat in storyboard["beats"]:
        ...
        anim = _build_beat(beat, cat, bone_of, cx, cy, twist_vol_conserve=twist_vol_conserve)
        anims[name] = anim
