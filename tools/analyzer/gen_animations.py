...
def build_animations(skeleton, storyboard, tier_gains=None, tier_combo_hits=None, tier_wobble_cycles=None):
    ...
    for beat in storyboard["beats"]:
        ...
        if tier_gains and cat in _MAIN_SHOW_CATS:
            ...
            # G-4''''':體積守恆節拍(squash)的 scale 走耦合放大(log 空間 v^g,守恆對任意 g 保持)。
            coupled = cat in _VOLUME_CONSERVING_CATS
            for tier, g in tier_gains.items():
                ...
                if cnt is not None:
                    ...
                    anims["{}__{}".format(name, tier)] = _amplify_anim(variant, g, coupled)
                else:
                    anims["{}__{}".format(name, tier)] = _amplify_anim(anim, g, coupled)
    return anims
