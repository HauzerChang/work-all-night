...
try:
    from beat_templates import gen_hit as _gen_hit, gen_reveal as _gen_reveal, \
        ...
        VTWIST_KEYWORDS as _VTWIST_KW, DUR as _DUR_EXT
    ...
    _DISPATCH["vtwist"] = _gen_vtwist  # candidate G-4''''''-vol:反相雙軸 shear + 耦合 uniform scale(full det≡1)
    DUR.update(_DUR_EXT)  # 讓 hit/reveal/combo/charge/cascade/wobble/squash/twist/vtwist 時長對 spine_anim.duration 一致
    # 主秀類別置前:exact/substring 命中優先於泛用 pulse。vtwist 置於 twist **之前**(避免 "vtwist" 被 twist 子字串先命中)
    _CAT_KEYWORDS = {"cascade": _CASCADE_KW, "combo": _COMBO_KW, "charge": _CHARGE_KW,
                     "vtwist": _VTWIST_KW, "twist": _TWIST_KW, "squash": _SQUASH_KW,
