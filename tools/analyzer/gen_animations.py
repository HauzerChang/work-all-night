...
try:
    from beat_templates import gen_hit as _gen_hit, gen_reveal as _gen_reveal, \
        ...
        TWIST_VOL_KEYWORDS as _TWIST_VOL_KW, DUR as _DUR_EXT
    ...
    _DISPATCH["twist_vol"] = _gen_twist_vol  # candidate G-4''''''-vol:反相雙軸 shear + 體積守恆耦合 scale(完整 det≡1)
    DUR.update(_DUR_EXT)  # 讓 hit/reveal/combo/charge/cascade/wobble/squash/twist/twist_vol 時長對 spine_anim.duration 一致
    # 主秀類別置前:exact/substring 命中優先於泛用 pulse(twist_vol 置於 twist 前,exact "twist_vol" 精確路由不誤入 twist)
    _CAT_KEYWORDS = {"cascade": _CASCADE_KW, "combo": _COMBO_KW, "charge": _CHARGE_KW,
                     "twist_vol": _TWIST_VOL_KW, "twist": _TWIST_KW, "squash": _SQUASH_KW,
