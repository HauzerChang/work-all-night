...
try:
    ...
    _DISPATCH["twistvol"] = _gen_twistvol  # candidate G-4''''''-vol:反相雙軸 shear + 耦合等軸 scale(全仿射 det≡1)
    DUR.update(_DUR_EXT)  # 讓 hit/reveal/combo/charge/cascade/wobble/squash/twist/twistvol 時長對 spine_anim.duration 一致
    # 主秀類別置前:exact/substring 命中優先於泛用 pulse。twistvol 置最前(其 kw 皆完整 token、
    # 不含 'twist' 子字串;beat_category 先做 exact 比對故 'twistvol' 正確路由,不被 twist 的 substring 攔)。
