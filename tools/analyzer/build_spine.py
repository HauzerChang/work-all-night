...
def build(psd_path, out_dir, genre="slot_bigwin", weighted=False, animate=False, rig=False,
    ...
    if animate:
        ...
        # candidate J-2:combo 另依檔位遞增**連擊數**;candidate G-4''':wobble 另依檔位遞增**擺動段數**
        # (幅度×結構數兩效正交疊加)。
        tg = tch = tws = None
        if tier_variants:
            from tier_variants import gains_for, combo_hits_for, wobble_segs_for
            tg = gains_for(genre)
            tch = combo_hits_for(genre)
            tws = wobble_segs_for(genre)
        skeleton["animations"] = build_animations(skeleton, spec["3_motion_storyboard"],
                                                  tier_gains=tg, tier_combo_hits=tch,
                                                  tier_wobble_segs=tws)
