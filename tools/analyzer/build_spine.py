...
def build(psd_path, out_dir, genre="slot_bigwin", weighted=False, animate=False, rig=False,
    ...
    if animate:
        ...
        # candidate (J):--tier-variants → 主秀 beat 額外產每檔位幅度變體(<Tier>_<beat>,愈高檔位愈大)
        tg = None
        if tier_variants:
            from genre_priors import tier_gains as _tier_gains
            tg = _tier_gains(genre)
...
def main():
    ...
    ap.add_argument("--tier-variants", dest="tier_variants", action="store_true",
                    help="candidate J:主秀 beat 額外產每檔位幅度變體 <Tier>_<beat>(愈高檔位愈大;需 --animate)")
    a = ap.parse_args()
