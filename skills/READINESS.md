...
> 產生於 2026-09-06 run(S1 candidate (H):combo/charge 接進 genre 先驗庫 —— 把 0g 的 combo(連擊)/charge(蓄力充能)節拍併入 `genre_priors.slot_bigwin`,`build_spine --animate` 直出。`spine-anim-forge` 新增 cap `combo_charge_priors_integration` L2(pipeline)→ 區塊仍 HOLD;運動基元先驗、單一真值資產,防固化。副產:H5 逼出 charge vs reveal 鑑別子(squash-floor)。可 skill 化 2 區塊 spine-mesh-doctor/spine-weighted-forge 不變)。
> (前次:0i 件繞關節 pivot 轉/G-3 pivot 縮放 / 0h cascade / 0g combo+charge 模板 / 0f big-win beat 模板 / 0e mesh deform。)
