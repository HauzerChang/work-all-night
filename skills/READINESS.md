...
> 產生於 2026-09-04 run 001(S1 candidate 0f→E:主秀 beat 接進已驗證 genre 先驗庫。`spine-anim-forge` 新增 cap `priors_beats_wiring` L2(GREEN pipeline)→ 區塊仍 HOLD;填補接線覆蓋、非新演算法,運動基元先驗、單一真值資產,防固化。3 區塊仍 READY 不變)。
> (前次 run:candidate 0f big-win 主秀 beat 模板 hit/reveal;candidate 0e mesh deform timeline 生成,新增區塊 `spine-anim-forge`。)
...
■ spine-anim-forge — 分鏡 → 會動 Spine timeline(bone/slot + mesh deform)
  ...
  目標:HOLD:讓 build --animate 素材『會動』;運動基元為手感先驗(非學自真值),達 L3 前不打包
    ...
    [L2] 主秀 hit/reveal 接進已驗證 genre 先驗庫(0f→E)    閘:GREEN (pipeline)  «genre_priors beat 加 `cat` 明確宣告運動基元;slot_bigwin In=reveal 現身、slot_reveal open=reveal/hit=hit → build_spine --animate 對真實 genre 直接輸出主秀簽章。5AC(cat有效+覆蓋率仍1.0/In reveal簽章/open+hit簽章/尾identity可串接/剝cat退回intro的鑑別負對照)全 PASS。cat 只改基元選擇不動關鍵字 → validate_priors 覆蓋率不受影響»
