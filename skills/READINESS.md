...
> 產生於 2026-09-07 run session 002(S1 candidate (J-2):combo 連擊數隨檔位遞增 —— 續 (J)(檔位**幅度**差異化),讓 combo 檔位變體連擊**數**隨檔位遞增(Super 3 連 → Legend 6 連),(J) 幅度增益仍疊加其上。`gen_combo(...,npeaks=3)`(npeaks==3 逐位元不變、≥4 通用 N 連佈局)+ `tier_variants.combo_peaks_for` + `build_animations(...,tier_combo_peaks=)`(combo 變體「重生成→amplify」)+ opt-in `build_spine --tier-variants --tier-combo-escalate`;`combo__Super` 逐位元 == base combo。`spine-anim-forge` 新增 cap `tier_combo_escalation` L2(pipeline)→ 區塊仍 HOLD。`validate_tier_combo.py` 5AC PASS(K3 峰數 3<4<5<6 嚴格遞增;K5 平峰數守衛全設 3→K3 FALSE 證閘可信)。**關鍵:結構性檔位差異必須「重生成」不能「後製縮放」;幅度增益 v'=1+g(v−1) 對峰數/峰序保形 → J(幅度)與 J-2(峰數)正交可疊加**。又一「宣告就緒 ≠ 生成器接上」延伸。可 skill 化 2 區塊 spine-mesh-doctor/spine-weighted-forge 不變)。
> ⚠️ 已知(先前即存在,非本次造成):`analyze_target`(validate_analyzer_award)overall_pass=false → spine-asset-forge/spine-target-analysis 的『反推分析』閘 RED(#3 分鏡 露出項);待後續 session 專門處理(屬 S1 分析器範圍,非 J 系列)。
> (前次:(J) 檔位幅度差異化 / (I) cascade 接進先驗庫 / (H) combo/charge 接進先驗庫 / 0i 件繞關節 pivot 轉 / G-3 pivot 縮放 / 0h cascade 模板 / 0g combo+charge 模板 / 0f big-win beat 模板 / 0e mesh deform。)
...
■ spine-anim-forge — 分鏡 → 會動 Spine timeline(bone/slot + mesh deform)
  ...
  目標:HOLD:讓 build --animate 素材『會動』;運動基元為手感先驗(非學自真值),達 L3 前不打包
    ...
    [L2] combo 連擊數隨檔位遞增(J-2,build --tier-variants --tier-combo-escalate) 閘:GREEN (pipeline)  «續 (J)(檔位幅度差異化):combo 檔位變體連擊**數**隨檔位遞增(Super 3 連 → Legend 6 連),(J) 幅度增益仍疊加其上。gen_combo(...,npeaks=3):npeaks==3 走原三連逐位元不變、npeaks≥4 走通用 N 連佈局(p_i=1+q(0.66+0.34·i/(N−1))、p_0≈1.119 穩過 impact 門檻、p_{N−1}=role peak)。tier_variants.TIER_COMBO_PEAKS+combo_peaks_for;build_animations(...,tier_combo_peaks=) 對 combo 變體改『重生成(npeaks)→ amplify(g)』(其餘沿用 J 僅幅度);opt-in build_spine --tier-variants --tier-combo-escalate;combo__Super 逐位元 == base combo。從**先驗庫**→**真實 build_spine robot 骨架**→build_animations 5AC PASS(K1 present+routing+base 不變/K2 每檔位首尾 identity/K3 crux 每 bone 峰數==宣告連擊數且各檔位 3<4<5<6 嚴格遞增/K4 每檔位仍 combo 簽章且非 charge・finale 幅度 [1.347,1.463,1.582,1.718] 隨檔位嚴格遞增/K5 負對照:平峰數守衛全設 3→K3 FALSE・不帶 tier_combo_peaks 各檔位皆 3 峰・escalate 不外洩非 combo beat・無宣告 genre 回 None)。關鍵:結構性檔位差異必須『重生成』不能『後製縮放』(峰數是拓樸量;amplify 只縮放高度);幅度增益 v'=1+g(v−1) 對峰數/峰序保形故 J 與 J-2 可安全疊加(兩簽章正交)。與 anim-forge 同 HOLD»
