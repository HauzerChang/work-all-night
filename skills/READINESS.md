...
> 產生於 2026-09-03 run 001(S1×S5 candidate 0g:生成分鏡動畫繞關節擺。`spine-rig-pivot` 新增 cap `anim_rig_articulation` L2 → 區塊仍 HOLD;單一 robot rig,多 rig 真值屬使用者資源,防固化。3 區塊仍 READY 不變)。
> (前次 run:2026-09-01 candidate 0f big-win 主秀 beat 模板,`spine-anim-forge` 新增 `storyboard_beat_templates` L2。)
...
■ spine-rig-pivot — S5 rig pivot 推斷(關節=父子件接觸縫)
  ...
  目標:HOLD:接 build_spine 骨樹已完成(L2);達 L3 尚缺『多 rig 真值』(Award 僅 1 個可拆肢體 rig,屬資源類),補齊後併入 forge 或開新 skill
    ...
    [L2] 生成分鏡動畫繞關節擺(--rig × --animate 組合,0g)    閘:GREEN (pipeline)  «補 S1×S5 缺口:既有閘只驗『手動 25° 旋轉』(validate_rig_build AC4),沒驗 gen_animations 『生成的』keyframe 有繞關節性質。用生成 Loop 峰值角 pose 肢體(以已驗關節 J 錨定 seam/distal):4AC PASS(G1 生成即接關節/G2 seam 位移 rig<<非rig 4.7–8.0×/G3 rig 末梢-縫比 10–17 vs 非rig≈1/G4 0° 負對照殘餘 0)。內建負對照=非 rig(骨在件中心→縫撕裂)。仍 L2:同 pivot_end2end,單一 robot rig»
