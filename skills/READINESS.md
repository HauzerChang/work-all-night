...
> 產生於 2026-09-07 session 002 run(S1 candidate (J-2):**combo 連擊「數」隨檔位遞增** —— 續 (J):(J) 檔位變體只差**幅度**,combo 各檔位仍同樣三連擊(有「多爆」沒「連幾下」)。本次補上 combo 的 impact 峰**數**=`nhits` 隨檔位嚴格遞增(Super 3→Mega 4→Omg 5→Legend 6)。**幅度增益加不出連擊數**(峰「數」是關鍵幀拓樸,必須 gen 時決定)→ 走 `tier_combo_hits` 對 combo 檔位變體以該檔位 nhits **重生成**再套幅度增益,與 (J) 幅度軸**正交可疊**(峰數 [3,4,5,6] × overshoot [0.347,0.469,0.591,0.730] 皆遞增);`gen_combo(nhits=3)` 逐位元同 0g、`build_animations(tier_combo_hits=None)` 逐位元同 (J)(加性 opt-in)。`build_spine --tier-variants` 直出。`spine-anim-forge` 新增 cap `tier_variant_combo_count` L2(pipeline)→ 區塊仍 HOLD。`validate_tier_combo_count.py` 5AC PASS(K2 峰數 [3,4,5,6] 嚴格遞增、K4 正交、K5 平連擊數守衛+count 不外洩);踩雷=連擊變多恐誤入 charge 簽章,擊間微回 0.985 使 hold-frac 反降(0.30→0.12)保 combo↔charge 互斥。可 skill 化 2 區塊 spine-mesh-doctor/spine-weighted-forge 不變。前個里程碑:(J)檔位幅度差異化 —— 把 `genre_priors.slot_bigwin` 宣告已久卻從未被生成器使用的 `tiers=[Super,Mega,Omg,Legend]` 接上產線:主秀 beat 依檔位產出**幅度差異化**變體 `{beat}__{tier}`(檔位愈高愈爆)。`build_spine --tier-variants`。增益規則對介面契約與結構簽章皆保形——scale **只放大 identity 上方 overshoot**、下方 squash/collapse 樓地板不動、rotate/translate 對 0 對稱放大、alpha 不動;base=Super g=1.0 逐位元同無檔位輸出。`spine-anim-forge` 新增 cap `tier_variant_amplitude` L2(pipeline)→ 區塊仍 HOLD;運動基元先驗、單一真值資產,防固化。`validate_tier_variants.py` 5AC PASS(J3 crux 幅度 Super<Mega<Omg<Legend 嚴格遞增;J5 平增益守衛全 1.0→單調性 FALSE 證閘可信)。又一「宣告就緒 ≠ 生成器接上」實例。可 skill 化 2 區塊 spine-mesh-doctor/spine-weighted-forge 不變)。
...
■ spine-anim-forge — 分鏡 → 會動 Spine timeline(bone/slot + mesh deform)
  ...
  目標:HOLD:讓 build --animate 素材『會動』;運動基元為手感先驗(非學自真值),達 L3 前不打包
    ...
    [L2] 檔位幅度差異化(J,build --animate --tier-variants 直出各檔位主秀變體) 閘:GREEN (pipeline)  «genre_priors.slot_bigwin 宣告 tiers=[Super,Mega,Omg,Legend] 已久但生成器從未用它——又一「宣告就緒 ≠ 生成器接上」缺口。把檔位轉成主秀幅度增益 g(檔位愈高愈爆),端到端接進 build_animations,每主秀 beat 依檔位產出 {beat}__{tier} 變體。tier_variants.py 增益規則(對介面契約與結構簽章皆保形):scale 只放大 identity 上方 overshoot(v'=1+g(v−1) 僅當 v≥1;下方 squash/collapse 樓地板不動)、rotate/translate 對 0 對稱放大、alpha 不動;base=Super g=1.0 逐位元同無檔位輸出。build_spine --tier-variants;變體名經 beat_category 仍路由回原類別。從先驗庫經 build_storyboard→真實 build_spine robot 骨架→build_animations 對真實 robot 5 拆件 5AC PASS(J1 present+routing/J2 每檔位介面契約首尾 identity・burst 首 collapsed/J3 crux scaleX overshoot 與 rotate 幅度 Super<Mega<Omg<Legend 嚴格遞增/J4 每檔位結構簽章保持 combo≥3 遞增峰・charge 長蓄力・hit anticipation+settle・cascade 跨件峰時刻遞增散佈/J5 負對照 In/Loop/Out 不產變體・無 tier 的 slot_reveal 不產變體且 base 相同・平增益守衛全 1.0→J3 單調性 FALSE 證閘可信)。關鍵:幅度增益只放大 identity 上方 overshoot、不動下方樓地板與時間軸 → 端點/簽章對所有檔位保形(蓄力深度/藏匿是結構語意非大獎強度,誠實地檔位無關;天真均勻縮放會把 reveal collapsed 0.02 在 g>1 推成負值翻面)。回歸全綠(含 --tier-variants round-trip build)。honest:主秀運動仍先驗手感、增益階梯數值為 PROPOSAL;單一真值資產。與 anim-forge 同 HOLD»
