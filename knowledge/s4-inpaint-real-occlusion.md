...
其餘配對(右手/頭/身體 等小面積重疊,<13%)當時因面積太小或本身尺寸太小未列入(見下方全表);
候選 9(session 011)已延伸補測,見下方對應章節。
...
## 候選 9(session 011)延伸:擴大配對樣本至 8 組

在原本 4 組(全部 >13% 面積)之外,新增 4 組小面積/懸殊比例配對(排除「X←光暈」全系列——
頭/右手/身體/左手被光暈完全覆蓋 `frac=1.0`,整層內容清空屬於 taxonomy 情境 2「原圖不存在
內容」,不是局部遮洞補圖能解的問題):

| target ← occluder | 洞面積/target 內容 | 洞絕對像素 | mode | 1a(3 baseline) | 1b |
|---|---|---|---|---|---|
| 右手 ← 頭 | 6.5% | 2978px | interior | 全 fail(ssim 0.25~0.34) | 全 pass |
| 身體 ← 頭 | 1.0%(本檔最小面積) | 829px | **edge** | 全 fail | not applicable |
| 頭 ← 右手 | **46.5%**(本檔最大比例) | 2978px | **edge** | 全 fail(ssim 0.13~0.24,最差) | not applicable |
| 頭 ← 身體 | 12.9% | 829px | **edge** | 全 fail | not applicable |

`calibration.pass` 維持 `True`(8 組 gt 1a/1b 全 pass、none/random 全 fail,無一因新配對破功)。

**核心發現 1(驗證既有結論可攜到新材質)**:`右手←頭`(machinery 紋理,interior)與既有
`身體←左手`/`左手` 結論完全同型——1a 三個 CPU baseline 全 fail、1b 全 pass。連最小面積
配對 `身體←頭`(829px,本檔測過最小的絕對洞尺寸)1a 失敗模式依然一致,無論面積多小、
機械紋理材質 1a 都補不動,校準沒有因為樣本縮小而失真。

**核心發現 2(揭露 1b 覆蓋率的真實缺口,非理論假設)**:`頭` 這個小尺寸圖層(內容僅
6405px,是 5 層中最小的)兩個被佔比例配對(`頭←右手` 46.5%、`頭←身體` 12.9%)**都被
`classify_mode()` 判成 edge**,不是 interior——因為圖層本身小,遮擋洞很容易同時碰到它
自己的內容邊界。這代表**小尺寸素材的真實遮擋洞,天生更容易落在 1b 目前不覆蓋的 edge
模式**,不是「edge 模式比較少見、可以晚點做」——用真實遮擋洞測才發現這一點(合成挖洞閘
挖洞時是刻意控制在 interior 或 edge 兩種模式各自測,不會像真實情況一樣被小圖層尺寸逼著
落入 edge)。這讓候選 2(1b edge 模式支援)的優先度應該提高:對小尺寸拆件素材,目前
1b 完全沒有驗收線可用,只能回退到 1a 嚴格標準(而 1a 對機械紋理材質全 fail)——即這批
「頭」這類小尺寸機械材質的補圖成果,目前**沒有任何一個聞得過的量化閘可以判定「動態下
是否穿幫」**,是個真實的評估器覆蓋率缺口。

## 下一步

- **候選 2(1b edge 模式支援)優先度上修**:見上方「核心發現 2」——不是理論性延伸,是
  真實遮擋樣本(頭)已經卡住、完全沒有驗收線可用的缺口。
- 其餘候選(4:探測 LaMa、6:擴大 Symbol_Ww 樣本、7:1b 閾值反向校準、10:光暈 1a 邊界)仍待推進。
- 若要繼續深挖候選 1:Symbol_Ww 沒有多層互相遮擋的真實案例可用,若要延伸需另找/另造有
  真實重疊的分層素材。
