...
- [S5 多 rig pivot 泛化(方法對應資產表徵)](s5-multi-rig-pivot.md) — 把「只單一 rig 驗過」拆掉,泛化到 Award 另 3 角色(OMG/SUP/MEG,各為**單一 weighted mesh** 非拆件式)。**核心發現:一個演算法不通吃兩種資產表徵** —— 拆件式(有幾何空隙)用**幾何接觸縫法**;連續 weighted mesh(件邊界是模糊權重過渡)用**權重法** `proximal_joint`(關節 = 子骨近端邊 = `w_child²·w_parent` 頂點加權質心)。幾何法在連續 mesh 上手臂爆掉 95–113px(dominant 硬切把肩部分給手臂,最近點落在末梢)。**4 AC 全 PASS**:19 關節 pooled 中位 0.034、84% <0.10、勝 baseline、random/swap 負對照爆閘;合計 robot(幾何法)= **5 rig 泛化**。硬案例 = 連續網格外張肢體(如實標,不硬 fail)。仍 HOLD(端到端接 build_spine 未做,無 ≥1 L3)。
