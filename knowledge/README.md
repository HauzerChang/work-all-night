...
- [P1 分鏡→動畫 keyframe 生成器](p2-storyboard-to-keyframe.md) — **物理主線第一步:讓 `build_spine` 產物會動**。`storyboard_to_anim.py` 把分鏡 idle/loop beat 轉成 Spine `animations`(v1=無縫待機呼吸 Loop)。校準自真實 `main_draw.main_idle2`(呼吸=scaleY 主導脈動、非體積守恆);bezier ease-in-out → inertia_index=1.0。4 條可機讀 AC(loop 閉合 gap=0/有界運動/物理簽名/round-trip)對 main_draw + robot 產物全 PASS。⭐ 測真實產物揪出 **build_spine 是扁平 rig**(靠名稱關鍵字選對軀幹;follow-through 須待階層 rig)。交棒 P2 物理注入(共用 `motion_physics` 評估器)。

- [P/S6 動畫物理可信度分析器](p1-motion-physics-analyzer.md) — **使用者新增研究項目「spine 動畫中的物理世界」的 v1**:把材質/質量/慣性/阻尼落成**可量測運動學簽名**(ease 慣性、follow-through 末梢跟隨、overshoot 回穩、squash 體積守恆、soft-body 形變波),從真實動畫抽取。`tools/analyzer/motion_physics.py`;**雙控驗證**:負對照(線性化→inertia 0)+ 正對照(相位延遲精確回收,**抓修一個符號 bug**)→ `--selftest` validated。發現:此風格物理詞彙=**ease+overshoot 為主、體積守恆 S&S 幾乎不用**;follow-through 視關節度(Award 腿鏈 0.58 > 窗簾 0.32);窗簾=cloth(deform 行進波)。誠實界定:v1 是分析/評估器,注入生成端待續。
