...
- [S3 端到端:PSD 件→mesh→對照 Award 真實 mesh](s3-psd-to-award.md) — **里程碑**:robot_parts 3 件(光暈/身體/左手)PSD→`generate_mesh_v2`→對照 Award 生產 mesh,**靜態 IoU 全過藝術家基準**(margin +0.013~+0.043)。三發現:①Award 這 3 件是 **weighted + 無 deform**(骨骼權重驅動,非 deform)→ S3 真缺口是 **BBW 權重生成**;②Award mesh uvs 是 **region 局部 0..1**(更正 session006);③v1 epsilon 0.008→**0.002**(真實件輪廓保真;strip 模式不受影響,回歸全過)。新工具 `validate_psd_to_award.py`。
