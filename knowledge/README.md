...
- [S1 分鏡 → mesh deform timeline](s1-mesh-deform-timeline.md) — **candidate 0e:讓「軟件」會變形**(補 0d 缺口:0d 的 mesh 只剛性隨骨走)。為 skin 內每個 mesh attachment 生成逐頂點 `deform` timeline(窗簾飄/光暈脹縮)。核心=**仿射保證 + 波幅閘控**:affine swing(`dx=A·fy·swing(t)`,線性 in y)與 uniform-scale(radial)皆仿射 → 恆乾淨;非仿射行進波經 `deform_eval` 逐幀自動遞減至乾淨(最差退純仿射)。`gen_mesh_deform.py` + `validate_mesh_deform.py`(對 main_draw 4 真實美術 mesh)**4 AC 全 PASS**(合法/round-trip、全幀 si/flip/degen=0、無縫+identity 介面、非平凡+折疊負對照被抓+極端仿射剪切仍乾淨);`build_spine --animate --deform` 端到端(robot 光暈/身體 294 幀全乾淨)+ 0d/round-trip 回歸 PASS。**關鍵教訓:平面雙射≠mesh 保拓樸**(直線邊是位移後重畫的,只有仿射才精確保邊;非線性剪切會讓邊互穿)。動作美感主觀 → 區塊 `spine-motion` 維持 HOLD。
