...
- [S3+S4 端到端對 Award 真實 mesh](s3-s4-award-endtoend.md) — 「PSD件→生成 mesh」對 Award 3 個真實 mesh(光暈/左手/身體)靜態驗收全過。揪修 v1 delaunay 固定 epsilon 對「邊界主導柔邊件(halo)」取樣過疏 → 改**預算內自適應加密**(`gen_delaunay_adaptive`)。誠實邊界:此 3 mesh 無 deform,耐變形不可對此標的驗(已在 main_draw 驗過)。
