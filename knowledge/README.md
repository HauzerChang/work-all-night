...
- [S3 端到端:PSD件→mesh→對照 Award 真實 mesh](s3-psd-to-award-mesh.md) — 機器人 光暈/身體/左手(**無 deform timeline、weighted-bone 類**)PSD 切件→generate_mesh→對照 Award 藝術家 mesh,靜態幾何三 AC(覆蓋率≥藝術家 / 拓樸 / 頂點預算)3 件全過。auto 正確路由 blob→v1;v1 預設 eps=0.008 對羽化/凹形件覆蓋不足→加 **adaptive 加密**自我校準到藝術家 baseline(工具 `compare_to_award.py`)。
