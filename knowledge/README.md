...
- [S1 分鏡 → 動畫 keyframe](s1-storyboard-to-animation.md) — **讓產出素材「會動」**:把 `3_motion_storyboard` role-based beats 確定性轉成 Spine 3.8 `animations`(In/Loop/Out,bone TRS + slot alpha),已整合進 `build_spine.py`。5 條量化 AC(結構/Loop 無縫/有動/有界/FK 軌跡)對 robot+Symbol_Ww **全 PASS**,round-trip setup pose 不受影響;負對照 5 種缺陷各觸發對應 AC。誠實界定:bone 原點在件中心(非關節 pivot,S5 未做)、緩動手感留給使用者目視、時長幅度為類型先驗未對影片校準。工具 `storyboard_to_anim.py`+`validate_anim.py`。
