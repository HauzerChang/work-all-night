...
- [S3+S4 端到端:PSD→mesh 對照 Award 真實 mesh](s3s4-psd-to-award-mesh.md) — **端到端里程碑**:robot_parts 三件(光暈/身體/左手)PSD→`generate_mesh_v2`,覆蓋率不劣於藝術家 mesh(身體更高)、頂點更省(45–61%),3 件 overall_pass。**校正 s4 doc 假設:Award mesh uvs 是 region-local 非 atlas-global**(PSD 件框無 flip 直接對齊,IoU 0.95/0.95/0.98)。三件走 delaunay-v1(團塊非 strip);weighted/無 deform → 只驗靜態覆蓋率,bone-driven 變形留待 S5。
