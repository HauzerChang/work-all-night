...
- [S5 骨架 pivot 閘 + baseline 推斷](s5-pivot-gate-and-baseline.md) — **S2 骨架閘完成 + S5 首個 baseline(里程碑)**:pivot 品質閘 `pivot_eval.py`(關鍵數學:骨帶 attachment 是剛體,繞錯 pivot 的世界位移差 = 2·sin(θ/2)·‖Δ‖,與點無關且等向 → pivot 歐氏誤差就是充分正確的量;判準 err/len≤0.15)。對 Award 真 rig 三道校驗 OVERALL PASS(自洽 truth→0、噪音 σ 單調上升 σ=1.0 pass 4%、baseline 分級)。baseline `infer_pivots.py`:parent_tip 只在序列中段關節成立(serial 60% vs branch 0%,優於 parent_origin),證明 branch 關節須靠 per-part mask 重疊區證據 → 指引 S5 下一步。防固化:閘就緒但生成仍 L1,S5 維持 HOLD。
