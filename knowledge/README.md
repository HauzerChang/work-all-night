...
- [S3×S4 端到端:PSD件→生成mesh→對照Award真實mesh](s3-psd-to-real-mesh.md) — robot_parts 3 個 mesh 件對真實生產 mesh(外部真值)驗收:校準後三件全 **BEAT 藝術家覆蓋率且頂點更少**。發現 coverage 由邊界密度 `epsilon_frac` 決定(v1 預設 0.008→0.004);凹形輪廓孤兒頂點 → 加 `prune_orphans()`。新閘 `validate_psd_mesh.py`。
