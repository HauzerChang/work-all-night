...
- [S4→S3 端到端(PSD件→生成mesh→對照Award真實mesh)](s4s3-psd-to-mesh-endtoend.md) — **里程碑**:機器人 3 個 mesh 件(光暈/身體/左手)切件後跑 `generate_mesh_v2`,對藝術家手做 mesh 同底 IoU 對照 **3 件全 PASS**(身體反勝 0.966>0.948、全部更精簡、拓樸乾淨)。實測推翻舊假設:Award mesh uvs **已是 region-local**(as-is 唯一高),不需轉座標。這 3 件無 deform timeline(weighted、骨骼變形)→ 覆蓋率為真值,auto 回退 v1 為正解。工具 `validate_psd_to_mesh.py`。
