...
- [S3 對照 Award 真實生產 mesh(端到端)](s3-award-real-mesh-parity.md) — 生成 mesh 對機器人 3 件(光暈/身體/左手)真實生產 mesh **靜態 IoU parity 全過**(≤0.008,頂點更精簡)。由真值對照揪修 2 個生成器缺陷:**邊界自適應 epsilon**(`generate_adaptive`,hull 覆蓋率自我目標)+ **內部孤兒頂點清除**(`prune_orphans`)。只影響 delaunay 分支,main_draw 4 mesh 回歸全過。誠實限制:Award 件無 deform timeline,只驗靜態、不含變形閘。
