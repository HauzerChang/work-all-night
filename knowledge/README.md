...
- [S5 接觸縫 pivot 寫入 build_spine 關節骨樹(端到端)](s5-rig-build-integration.md) — **接續 pivot 推斷,把關節寫進可載入 Spine 骨樹**:`build_spine --rig` 以「中介 joint 骨」模式(joint 骨置於接觸縫 pivot、render 骨掛其下、attachment 相對件中心不動)發關節骨樹,運動學父子樹取自分析器 #4 note。閘 `validate_rig_build.py` 用 Spine 3.8 FK 對 robot_parts **4 AC 全 PASS**:setup 逐件 d=0(繼承 round-trip)、joint 落 pivot d<0.005px、轉關節 30° 子件繞 pivot 精準旋轉且父件不動、pivot 顯著異於件中心(0.255×body 尺度)且改變末梢運動。使 `pivot_end2end` L0→L2。誠實限制:僅單一 rig(Award 其他角色是單一整片 mesh 非拆件 rig,無法充當第二真值)、rig 與 weighted 尚未同時支援 → 區塊續 HOLD,達 L3 需第二個拆件 rig 資產。無回歸(既有 3 閘全綠)。
