...
- [S3×S4 端到端對照 Award 真實 mesh](s3-s4-award-mesh-endtoend.md) — **PSD 件→生成 mesh→對照生產 spine 藝術家真值**(光暈/身體/左手 3 件),覆蓋率達/超藝術家、頂點更精簡、三件全 PASS。校驗 Award uvs 實為 region-local v-top(推翻 log 006 假設);v1 Delaunay 首度在真實團塊 mesh 過驗;deform 閘對這 5 件不適用(無 deform timeline)。工具 `compare_award_mesh.py`。
