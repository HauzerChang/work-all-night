...
- [S3×S4 端到端對照 Award 真實 mesh](s3-psd-to-award-mesh.md) — **里程碑**:PSD 件→S3 生成 mesh→對照 Award 3 個機器人 mesh(光暈/身體/左手)全 `overall_pass`;S3 覆蓋率 ≥ 藝術家、頂點僅 ~一半、拓樸乾淨。發現:① 這些件用 v1(非高瘦,strip/v1 分工互補)② 皆 weighted 骨驅、無 deform timeline → 用靜態閘非 deform 閘 ③ 更正交接誤解:Award mesh uvs 已是 region-local。負對照 self/cross IoU 落差 ~0.4。
