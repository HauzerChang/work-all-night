...
- [S3+S4 端到端對照 Award 真實 mesh](s3-s4-end-to-end-award.md) — PSD 件→generate_mesh_v2→對照 Award 生產 mesh(光暈/左手/身體)**3 件全過**:覆蓋率在藝術家 ±0.03 內、頂點只用 45~74%。發現 **Spine mesh uvs 是 region-local 0..1、logical 方位** → 生成/藝術家 mesh 可在件像素空間直接比。負對照(錯配崩到 0.48~0.58)確認鑑別力。⚠️ 未含 deform 閘(Award mesh 為 weighted,屬下個 chunk)。
