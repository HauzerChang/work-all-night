...
- [S1/S3 mesh deform timeline 生成器(candidate 0e)](s1-storyboard-deform-generator.md) — **補 candidate 0d 缺口:讓軟件會逐頂點變形**(之前只被父骨剛體搬動)。`gen_deform.py` 對 unweighted mesh 生成**駐波微顫**deform(位移=A·env(s)·sin(2πk·s)·temporal(τ),temporal 端點回 identity → loop 無縫 + 與 bone timeline 共用 identity 介面)。對 main_draw 4 真實 mesh 用校準過的 deform_eval 閘 **AC1–4 + NC 全 PASS**;整合驗注入 12/12 全乾淨 + JSON 可載入。發現:此類 deform 對 strip 拓樸無條件安全(amp 拉到近真實 314px 仍 si=0)→ 負對照改用高頻雙軸扭轉壞場證鑑別力。deform 只對 unweighted(weighted 靠 skinning);env/幅度屬美術先驗(A 類)。
