...
- [S3 端到端:PSD 件→生成 mesh→對照 Award 真實 mesh](s3-psd-to-award-mesh.md) — S3×S4 串接對真實生產標的驗收(3 個 mesh 件)。發現 v1 Delaunay 舊預設 eps=0.008 邊界太粗、覆蓋率低於藝術家;**校準 eps=0.002 後 3 件 IoU 全 ≥ 藝術家且頂點數 ≤ 藝術家**。推廣「IoU 由邊界取樣密度決定」到 Delaunay 路徑。這 3 件 weighted/無 deform timeline → deform-transfer 閘不適用(bone-skin 場列下一步)。
