...
- [W4 批2:臉部套件+地標校正+薄件緊縮](s4-w4b2-face-kit.md) — +13 臉部件;**地標色彩偵測取代目測**(讀格線錯 15px 教訓);薄件 tight 遮罩(眉 chamfer 0.9px)。**51.6→70.6**、recall 0.525。殘餘:GT 基底/覆蓋雙件模式、薄件 IoU 嚴苛、掛帶空件。

- [W5 收斂定版](s4-w5-final.md) — v13(27件):基底/覆蓋雙件、z 歸屬修正(可見件必須贏)、alpha 繼承。24/40 配對、completeness 全 1.0、chamfer 4.9px;**扣資訊極限後實際 recall 0.89**。冗餘超標→評分器非對稱懲罰列待辦(不改尺自肥)。W1~W5 完成。
