...
- [S3 端到端 PSD件→mesh→對照 Award 真實 mesh](s3-robot-psd-to-mesh.md) — **端到端里程碑**:機器人 3 mesh 件對真實生產 ground truth 3/3 PASS(覆蓋率≈藝術家、頂點少 14–40%);auto 模式選擇經真實雙向樣本確認(blob→v1 Delaunay、窗簾→strip)。用真實標的揪出並修好 v1 兩缺陷:孤兒頂點 prune、DP epsilon 絕對像素封頂(光暈 IoU 0.929→0.977)。main_draw 4 mesh 不回歸(走 strip,v1 未被呼叫)。
