...
- [S3 PSD件→mesh→對照 Award 真實 mesh](s3-psd-to-award-mesh.md) — **S3+S4 首次對真實生產標的端到端驗收**:光暈/左手/身體 3 件生成 mesh 覆蓋達藝術家水準(省 18~39% 頂點),3 件全 PASS。校正 STATE 誤記(**Award mesh uvs 是 region 局部非 atlas UV**)、反證 atlas_crop CW derotation。這 3 件無 deform→靜態對照。新增 evaluator 驅動覆蓋自收斂(v1 大柔邊件 epsilon 過粗問題)。
