...
- [S3 PSD件→mesh 對照 Award 真實 mesh](s3-psd-to-mesh-vs-award.md) — 端到端「PSD→件→S3 mesh」對真實生產標的驗收。auto-refine epsilon 後 3 個 mesh 件(光暈/身體/左手)靜態 IoU 全達/超藝術家,頂點反而少 12~45%。發現:固定 eps=0.008 對細緻件太粗、64 頂點預算對生產件偏緊、光暈藝術家用純外環(78v 全 hull)拓樸。限制:件為 weighted 且無 deform timeline,故只驗靜態輪廓+頂點經濟。
