...
- [S3 端到端對 Award 真實 mesh](s3-psd-to-award-mesh.md) — **PSD 機器人 3 mesh 件 → generate_mesh_v2 對照 Award 藝術家 mesh 全過(生成覆蓋率追平/超越、頂點更省 65~82%)**。校正 log 006 UV 誤解(uvs 是 region 局部非 atlas 頁);修 v1 兩缺陷(孤兒頂點、平滑曲邊自適應簡化)。真值都是 weighted mesh → 下一缺口 = **BBW 骨權重**。工具 `robot_mesh_gt.py`。
