...
- [外部知識吸收:GenieLabs `spine-animation-ai`(開源 Claude skill,chunk 38)](s4-genielabs-spine-ai-knowledge.md) —
...
- [S4 AI Viewer v3:統一主素材架構(chunk 40)](s4-ai-viewer-v3-unified.md) — 使用者要求
  補圖/切片/拆解對焦到同一份已載入檔案,不要各別上傳。拿掉拆解分頁獨立的 file picker,
  改成單一「主素材」狀態(`setMainAsset()` 為唯一寫入點)持續顯示、跨模式共用;新增
  「◆整體(composite)」偽圖層列,PSD/manifest 載入後自動選取顯示。**寫測試時抓到一個
  真實 bug**:遮罩畫布只在「切換到補圖模式」時才被 JS 啟用互動,但補圖是頁面預設模式,
  從未觸發那個切換事件,導致剛載入頁面時遮罩根本畫不出來(肉眼看畫面完全正常,只是滑鼠
...
- [拆解功能真實失敗案例:預設prompt寫死人形假設(chunk 42)](s4-decompose-prompt-bug-wings-case.md) —
  使用者用 viewer 拆解分頁對一對機械翅膀跑真實付費呼叫,**輸出完全不相干**(回傳一套黑色
  皮甲人形部件,跟翅膀毫無關係)。讀程式碼確診根因:拆解分頁預設 prompt 寫死
  「separate the head, torso, arms, and legs/feet」,假設主體是人形角色,對翅膀這種非
  人形輸入直接文字內容矛盾,模型選擇跟著文字走、忽略實際輸入圖——**是預設 prompt 設計
  疏失,不是模型隨機失效**。已修正:prompt 改主體無關通用版本+明確錨定語句(禁止替換
  主體);順帶修正 size 下拉未依輸入長寬比自動建議的次要風險。Playwright 驗證邏輯正確
...
- [研究分流:圖片切割 vs 圖片切片/圖層分解(chunk 45)](s4-cut-vs-slice-research-split.md) —
  使用者要求把「拆解」拆成兩個獨立研究區塊:**切割**(裁部件,邊緣允許破損)vs **切片**
  (單層拆多層,淺層如物件/陰影/光源分離,深層如遮擋物+底下被遮內容分離)。核心判斷:
  切割是空間切分,像素資訊都還在,已有明確路徑(即既有六階段第3點,不需另開新研究);
  切片是訊號分離/反推,依「底下內容是否還在畫面裡」分兩種難度——陰影/光源分離屬於
  訊號分解,原則上CPU傳統方法(反推blend mode數學)可能做到,提出具體構想(色塊分群
  找基準色→反推multiply/screen疊色的alpha)但**未經真實素材驗證**;遮擋物底下內容
  (如墨鏡下的眼睛)像素在原圖裡不存在,必須用生成式inpainting「腦補」,跟情境2視角
  外推同一難度等級,且不能直接沿用candidate17已驗證的「邊緣修補」可靠度結論(mask
  用途完全不同)。純研究筆記,零API花費零production代碼異動。留4個問題請使用者裁示
  (含是否有實際素材需要深層應用,避免投資源在假設性案例)。

- [拆解第3點正式落地:裁切工具+PSD組裝+使用者真實決策檔實測(chunk 46)](s4-decompose-cut-tool.md) —
  使用者裁示「排程上優先區塊一(切割),到一定技術水準再進區塊二」,建
  `tools/mesh_gen/s4_decompose_cut.py`(決策檔→硬邊界矩形裁切→manifest.json)+
  `tools/mesh_gen/psd_node/manifest_to_psd.js`(Node/ag-psd 組回PSD,發現node-canvas
  在此環境裝不起來缺pangocairo,改用ag-psd的imageData純像素陣列介面+pngjs,不需要
  真Canvas)。**自驗閘經歷兩次真實失敗才收斂**:第一版直接照搬psd_slice.py門檻,對
  九尾焰蓮測試素材兩項全fail,畫圖定位根因是該素材無alpha通道(矩形面板裁圖非去背
  角色),孤兒檢查被背景/標題文字大量誤報——不是裁切邏輯錯,是套錯判準;改用
  robot_parts.psd(真alpha)的ground-truth座標回歸測試證實孤兒檢查邏輯本身正確
  (orphan=0.0),並用兩次負對照(拿掉部件重跑)confirm能抓真漏洞、也誠實記錄一個
  盲點(某部件bbox幾何涵蓋另一個被拿掉部件的區域時抓不到)。最終版:AC1硬性、AC2降級
  為資訊性不擋關(矩形窗口邊緣落差是設計本身接受的代價)、AC3對無意義alpha來源自動
  skip。端到端管線(裁切→PSD)用獨立psd-tools交叉驗證通過。**使用者接著提供親自用
  assist viewer調整過的真實決策檔實測**(不是我方草稿)——視覺逐格檢視20個裁出部件後
  誠實發現:多個部件的bleed比「邊緣破損」嚴重得多(head框裝了大半標題文字、earrings
...
- [點提示能否修正SAM靜默錯誤?負面結果+更有用的發現(chunk 48)](s4-sam-point-prompt-investigation.md) —
  使用者選chunk47提案的方向1(點提示)。**在蓋UI前先於Python層直接驗證演算法有沒有用**
  ——第一輪憑印象猜點失敗(方法論有瑕疵,誠實記錄:連自己選的點都可能沒點準,不算有效
  測試)。**第二輪放大逐格目視確認框內容才選點**,意外發現比點提示測試本身更重要的事:
  chunk47統計的5個靜默錯誤裡,**`leg_left`跟`boot_left`根本不是分割問題**——放大一看
  這兩個框裡完全沒有腿/靴子(分別是手+尾巴毛髮、絲帶布料+尾巴毛髮),決策檔框位置從
  一開始就錯了,不管換哪種分割演算法都救不回來,正確修法是回到現有assist viewer手動
  重框,不需要新開發。對真正「框正確但SAM選錯」的案例(bodice/sleeve_right)測試點提示
  (正向點、正向+負向點組合共4次嘗試)**全部沒有解決**,即使點是目視放大確認過的精準
  位置。**誠實結論**:點提示在這批測試案例上沒有展現可靠修正能力,原因未確認(MobileSAM
  的TinyViT表達力較弱?或這批素材本身結構性困難?完整版SAM未測試,是唯一還沒排除的
  技術變因)。**沒有蓋UI**——中途加的點選UI骨架驗證無效後已用`git checkout`還原,repo
  無殘留半成品。把chunk47的「25%靜默錯誤」修正拆解為:2個是決策檔框位置錯誤(現成
  修法)+3個是真正演算法歧義(目前無解)。留3個未執行選項給使用者裁決:先修2個錯誤框
  (零成本)/對剩下3個接受人工處理/測試完整版SAM(成本高很多,需使用者先確認)。

- [修正leg_left/boot_left決策檔框位置錯誤(chunk 49)](s4-decompose-box-fix-legs.md) —
  執行chunk48留下的選項(a)(零成本、唯一已知有效的改進動作),無活人裁決依既有慣例
  執行不需授權的選項。**追溯錯誤來源**:chunk46-48用的使用者手動確認決策檔未持久化
  進repo,改查repo裡持久化的`suggestions.json`(chunk43我自己的原始草稿),換算
  `leg_left`/`boot_left`的`bbox_pct`→像素,跟chunk48報告的錯誤座標完全吻合——確認
  錯誤從chunk43就已存在,使用者調整決策檔時沒改到這兩格。用網格疊圖工具放大檢視來源
  圖,發現兩腿在畫面上緊貼重疊、肉眼分不出左右腿分界,靴子同理只有單一連續靴筒輪廓;
  比照已驗證正確的`leg_right`/`boot_right`(框住連續膚色/靴筒右半),把兩個錯誤框改
  框到左半部,confidence從high下修為medium並記錄材質限制。自驗:20部件`--contour rect
  --eval`AC1 pass,逐格複核修正後裁圖確實是大腿膚色/黑色靴筒(不再是手/布料+毛髮)。
  **誠實限制**:框位置修對後,`leg_left`/`leg_right`裁出的內容仍是同一團連續膚色的
