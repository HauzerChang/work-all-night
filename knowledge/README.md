...
- [S3 weighted-mesh 骨骼變形平滑度閘](s3-weighted-deform-gate.md) — **補上唯一未驗維度**:`spine_skin.py` 重現 Spine 3.8 蒙皮,對 Award 3 個 weighted mesh 件施**真實動畫 bone pose**量化變形品質。三段驗證全 PASS(setup 乾淨 / Loop 穩態全程 0 自交 / 硬綁最近骨負對照有鑑別力)。關鍵校準:**In/Out=入出場 squash/pop 過場(原始 mesh 本就自交,非品質訊號),Loop=穩態才 must-be-clean**。附向量化幾何檢查 `geom_fast.py`(與 reference 0 mismatch、~20×)。閘就緒 → 下一步生成我方 weighted mesh + BBW 權重比對。
