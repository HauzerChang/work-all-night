...
- [S1/S3 mesh deform 生成器(窗簾/軟體形變)](s1-mesh-deform-generator.md) — **補 candidate 0d 最後缺口**(mesh 不會變形):`gen_deform.py` 把 beat 類別 → 空間位移場 × 時間 shape 具體化為 Spine 3.8 `deform` timeline。**位移場模型 grounded 於藝術家真值**——量 main_draw 4 個真實 mesh 得:窗簾 = 頂錨定(|d|=0)、自由端(底)|d|=max、水平掃、幅度**線性於距錨距離**。確定性場 `offset_perp[i]=A·w_i·shape(τ)`。`validate_deform_gen.py` 對 4 mesh×4 beat 類別(loop/pulse/intro/outro)**AC1–5 全 PASS**(拓樸乾淨/loop 無縫 err=0/類別包絡 0.60·長軸內/**隨機場負對照撕裂 si=62**)。已接 `build_spine --animate` + `gen_animations`;經 Spine reader round-trip 全 clean。發現:**平滑 shear 場極耐變形**(loop 幅度灌爆 6× 仍乾淨,呼應藝術家 315px strip)→ 鑑別力靠隨機非平滑場而非加大幅度。honest boundary:預設幅度窗簾級(shadow 類須 caller 傳 amp_frac)、reveal 為非零端點、weighted mesh 不適用。新增 cap `mesh_deform_gen` L2。
