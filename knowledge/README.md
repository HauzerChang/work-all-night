...
- [S3 端到端對照 Award 藝術家真值](s3-award-mesh-e2e.md) — **PSD→件→S3 mesh 對真實生產 spine 首次端到端驗收(里程碑)**:光暈/左手/身體 3 mesh 件全 PASS(覆蓋不輸藝術家、頂點更省、0 孤島)。修正:光暈高曲率輪廓 v1 過度簡化 → **自適應 epsilon**。實測 Award mesh uvs 為 region-local;藝術家重建做 dihedral 朝向搜尋 + 自一致性校驗。
