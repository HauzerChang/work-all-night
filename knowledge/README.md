...
- [S3 端到端 件→mesh 對真實生產 mesh](s3-psd-to-mesh-real.md) — PSD/atlas 切件→`generate_mesh_v2`→對照 Award 3 個 mesh 件(光暈/身體/左手),6 組全 PASS(IoU 0.99+ ≥ 藝術家基準)。發現:blobby 件 IoU 由 hull 密度(epsilon)決定→加 budget-targeted auto-epsilon;誠實邊界:這些件靠骨骼權重(無 deform timeline)、S3 尚缺 BBW 權重。
