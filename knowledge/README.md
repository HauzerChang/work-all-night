...
- [S3 對照 Award weighted 件 + v1 epsilon 校準](s3-award-weighted-parts.md) — 端到端「PSD件→S3 mesh→對照真實 Award 藝術家 mesh」對 3 件全過 IoU 基準(里程碑)。揭示新 regime:weighted+無 deform+面/環狀 → 修 toolchain 崩潰(deform-transfer N/A)、把 v1 邊界密度 epsilon 0.008→0.002(對藝術家真值校準;shadow/curtain fallback 也變好,無迴歸)。
