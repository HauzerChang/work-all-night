...
- [S1 (0h) cascade 跨件錯開 reveal](s1-cascade-stagger.md) — 第一個**跨件(cross-part)時序**主秀簽章,補 0f/0g 全是**單件**包絡的缺口:件依空間序(bone x)**錯開** onset 逐一 reveal(掃出),非同時炸開。`gen_cascade_part`(reveal 家族介面 collapse→identity)+ `build_animations` **特判**(件間相位編排,不進 `_DISPATCH`,因需全 beat 件集合)+ `validate_cascade.py` **6 AC 全 PASS**(well-formed/介面/每件真峰≥1.12/**跨件簽章=stagger spread≥0.25 且峰時間沿 x 序嚴格遞增**/負對照/回歸 reveal 未破壞)。**負對照證鑑別力**:同時 reveal→spread≈0、**打亂 onset(逆序)→spread 大但非遞增**(證單調掃向為必要條件,非只看跨度)、單發 hit→identity 起非 collapsed。真值=結構簽章(美感留使用者)。cap `cascade_stagger` L2;anim-forge 仍 HOLD;尚未併入 genre_priors(待 (H') 直出)。
