...
- [S3×S4 端到端對真實生產 mesh](s3-psd-to-mesh-vs-award.md) — **里程碑**:PSD 件→`generate_mesh_v2`→對照 Award 藝術家 mesh(ground truth)。3 個 mesh 件覆蓋率與藝術家持平(±1.5%,身體反超)且**頂點少 26–55%**;內建正/負(翻Y)對照確認指標可信。工具 `validate_psd_mesh_vs_spine.py` exit 0。機器人件為 weighted、無 deform → deform 閘 N/A,故以 coverage IoU 當真值。
