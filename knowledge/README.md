...
- [S5×S1 組合:rig 關節樹 × 生成主秀動畫(關節接合閘)](s5-rig-anim-articulation.md) — **證明 `build_spine --rig --animate` 組起來後肢體真的繞關節擺、且黏在父件身上**(補 `validate_rig_build` 只驗單一固定角、`validate_anim` 只驗非 rig 的缺口)。`validate_rig_anim.py` 逐幀量「子件接縫點在父件座標系中的脫槽距離」,**5 AC 全 PASS**:rig 接縫黏住(≤6.3px)、負對照非 rig 散架(脫槽 21/6.4/7px、單調 rig<flat、總和比 2.56/最大比 6.2×)、三 beat 逐幀 mesh si=0、radial 修正。**負對照=完全相同動畫、只差 rig 結構**。同塊**修一個潛伏組合 bug**:rig 下 In/Out 徑向誤用件骨 local 座標當畫布座標算方向(頭/左手被翻反 cos −0.74/−0.95)→ 加 `_bone_world_origin` 沿父鏈累加修正(非 rig 逐位相容)。誠實界定:左手件中心≈關節故鑑別弱(用單調+總和/最大比,非 min>門檻);單一真值資產 → `spine-anim-forge`/`spine-rig-pivot` 兩區塊仍 HOLD(強化組合證據非跨門檻)。圖 `figures/s5_rig_anim_articulation.png`。
