...
- [S3 端到端真實驗收(機器人 mesh 件)](s3-robot-parts-real.md) — PSD→件→mesh 對照 **Award 生產真值**:3 個 mesh 件(光暈/身體/左手)全 PASS。這 3 件**無 deform**(骨驅)→ 改用靜態拓樸+覆蓋率 IoU 閘(變形閘 N/A)。發現預設 eps 對圓形邊界欠取樣(光暈 IoU 0.93)→ 加 **adaptive 邊界取樣**(opt-in,零回歸)→ 全過。main_draw 4 mesh 回歸全綠。
