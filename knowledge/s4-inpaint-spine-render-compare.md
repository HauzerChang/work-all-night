...
## 第三個案例:機器人拆件/光暈(chunk 25,2026-09-01)——第三種材質類型(平滑漸層)

`光暈` 是 1a 嚴格標準下「CPU 補得動」的代表材質(候選 0/10 已知:平滑漸層,ssim 高),與
`左手`/`身體`(機械紋理,1a fail)互補,補齊候選 16 路徑 (b) 的第三種材質類型。atlas region
`機器人拆件/光暈` 在 `Award2.png`,`rotate=true`,尺寸 496×480(比 `左手`/`身體` 大不少)。

跑法與前兩案例相同:`--slot "機器人拆件/光暈" --att-name "機器人拆件/光暈"`(其餘用預設,
`Award_Legend_In`/`Award_Legend_Loop` 共 11 個時間點,`frac=0.12` interior 洞)。

**結果**:

- **1b 盲選**:`nearest` 勝出(`pass_1b`,`alpha_gap=0.0, seam_ratio=0.427, tone_gap=0.597`
  三項皆 pass,`hole_frac_of_content=0.118`),與前兩案例同型。
- **零外洩驗證**:對全部 11 個時間點的全場景截圖(900×900)逐像素比對 orig vs patched,
  差異像素(閾值>2)全部落在該 slot 螢幕框內、0 外洩到其他 slot/頁(如
  `Award_Legend_Loop t=0.6`:69 個差異像素,框 x∈[358,535] y∈[320,482],差異像素實際
  bbox 落在 x∈[464,486] y∈[383,419],完全在框內)。
- **量級遠低於前兩案例**:前 3 個取樣時間點(`Award_Legend_In` t=0/0.233/0.367)`mae_0_255`
  恰好 = 0(材質此時尚未出現,同前兩案例已知的 scale 過渡期為 0 現象);材質完全出現後
  `mae_0_255` 穩定在 **0.01~0.04**——比 `左手`/`身體`(0.9~1.05)低約兩個數量級,且每幀
  差異像素數僅 52~83 px(對比框面積 ~30,000+ px,差異像素佔比 <0.3%)。
  **這驗證了 chunk 24「下一步」的預期**:平滑材質的真實渲染差異確實比機械紋理材質小得多。
- **實際螢幕佔比**:目標 slot 在此相機框架下約佔全場景 170~320×130~220px(≈3.4~7.3%
  畫布面積,隨動畫時間點變化較大,因為光暈特效本身有縮放/擴散動態)——遠大於 `左手`
  (0.5~0.6%)、`身體`(1.0~1.1%),是三案例中佔比最大的一個。
- **人眼複查(8x 放大差異像素 bbox 周圍區域,`Award_Legend_Loop t=0.6`)**:即使放大到
  8 倍盯著差異像素集中的區域看,orig/patched 兩張圖**視覺上無法分辨**——不像 `左手`/
  `身體` 在 6x/10x 放大下還能看出「奶油糊」(高頻細節被抹平)的痕跡,`光暈` 這裡放大後
  仍看不出任何差異,補丁完全融入。

**結論**:候選16路徑(b)對第三種材質類型(`光暈`,平滑漸層,rotate=true)驗證成功,且結果比
前兩個機械紋理案例更乾淨——**平滑材質的 CPU baseline 補丁不只是「不構成一眼可見穿幫」,
在本次測試的放大倍率下已經達到「肉眼看不出差異」的程度**,同時佔比是三案例中最大的
(3.4~7.3% vs 0.5~1.1%),排除了「因為佔比小才不明顯」的另一種可能解釋——這裡佔比更大、
差異反而更小,呼應材質類型(紋理複雜度)才是決定因素、而非佔比,與 1a/1b 既有結論(候選
0/8/10:機械紋理 vs 平滑漸層是本專案補圖難度的主軸,不是面積)方向一致。未改動任何既有
production 代碼。
...
- ✅ ~~擴大到 `光暈`(平滑材質,理論上 CPU 補圖在 1a 都能過,預期這裡的差異會更小)~~
  chunk 25 已完成,見上方——確認差異比機械紋理案例小約兩個數量級,且在此案例佔比更大時
  依然成立,排除「佔比小才不明顯」的替代解釋。
- 若能取得真實遊戲對這個特效的實際顯示縮放比例(UI 設計稿或遊戲截圖),重算相機框架驗證
  「佔比小所以不明顯」這個結論在正式畫面尺度下是否依然成立——`光暈` 的 3.4~7.3% 佔比已經
  比 `身體`(1.0~1.1%)、`左手`(0.5~0.6%)都大很多,若真實遊戲把特效再放大,機械紋理材質
  (`左手`/`身體`)的結論可能在某個佔比門檻後翻盤(`光暈` 本身因材質平滑,即使佔比更大依然
  看不出差異,不受這個顧慮影響)。三個案例目前都還沒找到讓機械紋理材質翻盤的佔比門檻。
- 目前的相機聯集只用了 11 個離散時間取樣點,若動畫在取樣點之間有更劇烈的甩動,聯集框可能
  不夠寬——可以考慮用更密集的取樣或直接對整段動畫做逐 frame 掃描來算聯集(成本更高)。
- 三個材質類型(機械紋理×2、平滑漸層×1)都已覆蓋,候選16路徑(b)本身可視為已達成初始
  目標(「回答動態動畫尺度下會不會穿幫」);剩下的延伸都屬於「取得真實佔比門檻」這類需要
  外部資源(遊戲實機截圖/UI 設計稿)的 A 類岔路,非可自主推進的下一步。
