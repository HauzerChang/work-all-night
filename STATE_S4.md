...
14. **`estimate_alpha_taper` 的另一種獨立失敗模式**(候選 13 調查時新發現,見
    `s4-inpaint-alpha-taper-robustness.md`):大樣本數(`ring_count` 50~700+,不是小樣本
    統計不穩)下 15px 環內局部梯度中位數估計仍會系統性抓到不具代表性的邊緣段落,導致
    `alpha_mae` 崩壞到 90~140(如 `右手` edge 小洞、`光暈` 特定橢圓 interior 洞)。`min_ring`
    對這批無效,需要不同的診斷/修法方向(候選:洞邊界周長上均勻取樣、或用局部曲率/走向加權
    取代單純距離環)。
