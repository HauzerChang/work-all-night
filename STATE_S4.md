...
13. **修正 `estimate_alpha_taper` 的小樣本 bug**(候選 10 意外發現,見 `s4-inpaint-1a-shape-boundary.md`):
    特定橢圓 interior 洞下,`ell` 估計因 n=7 小樣本污染催毀性低估 alpha(60 vs 真值 255,RGB 本身
    補對)。既有測試案例(3 機器人材質 + 2 Symbol_Ww 材質 + 8 組真實遮擋)皆未觸發,回歸零反向,
    但這代表函式本身不夠穩健。建議先擴大觸發樣本(對更多材質/洞形狀跑一輪,量化多常發生)
    再決定修法,避免只憑一次意外案例就重寫核心估計邏輯。
