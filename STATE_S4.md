...
**下一個有界工作塊候選(擇一推進,已依上述修正調整優先序):**
1. **★新增:1b 專用寬鬆閘**:同一批合成挖洞測試資料,換一套「防穿幫」導向指標(透明像素比例/接縫銳利度/
   色調連續性),重新判定先前標「CPU 補不動」的機械紋理案例在 1b 情境下是否可接受。
2. **遮擋真值法**:用 Award/機器人多件疊合 composite,找已知被上層遮住、但 PSD 該層本身有畫全的真實
   區域當真值(比合成挖洞更貼近實戰);對照合成挖洞閘的判定是否一致。
3. **探測 Level 3(LaMa)**:深度 inpaint 權重下載是否被網路政策擋?先探測可行性,若可裝則對「CPU 補不動」
   的身體/左手案例跑一輪,量化 Level 3 能否補救(注意:類別 2 視角外推問題 LaMa 也解不了)。
4. **修正 `fill_cv2_inpaint` 的 edge 模式 alpha 處理**:目前洞區強制設不透明,與柔和邊緣真值 alpha
   漸縮不符(`alpha_mae` 28~42)→ 應改為對 alpha 也跑 inpaint 或用距離場漸縮。
5. 用本閘測 `Symbol_Ww.psd` 其他層(icon 類,可能有更多平面色塊),擴大「CPU 補得動」樣本、交叉驗證邊界。
