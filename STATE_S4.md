...
- 🆕 **候選 16(chunk 18 提出,chunk 21 更新)**:「邊界證據延續性」路線(候選 18)已實作校準,
  結論構造本身結構性偏差不可用(見上方 chunk 21)。仍待推進的路徑:(a) 改走「局部高頻能量/
  方差比」方向重新設計第 4 指標,或 (b) 投入把補圖貼回真實 Award spine 場景跑動畫截圖比對
  (更貼近實戰但工作量大)——皆屬下一個可推進的有界工作塊,非阻塞性 A 類岔路。
