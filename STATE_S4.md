...
18. **候選 16 的指標設計方向已具體化(可直接接著做)** — 插件 prompt 的
    ...
    (兩個完全獨立來源收斂)。故候選 16 的第 4 個指標不該是籠統的「高頻細節保留度」,而是
    **「邊界證據延續性」**:量洞內的亮度/梯度場有沒有延續邊界暗邊所暗示的陰影方向與衰減。
    比原構想更可實作、且有外部佐證。
...
- 🆕 **候選 16(chunk 18 提出)**:1b 要不要加第 4 個「高頻細節保留度」指標,或投入把補圖貼回
  真實 Award spine 場景跑動畫截圖比對(更貼近實戰但工作量大)——屬於下一個可推進的有界工作塊,
  非阻塞性 A 類岔路(不強制需要使用者裁決,只是量級較大需獨立排定)。
