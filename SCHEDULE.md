...
> **2026-07-03 使用者拍板**:①排程**開放為全面研究**(不限 PSD 切圖,依 `STATE.md` 優先序自主推進;
> 下次能力驗證時使用者再調整)②頻率 = **每 6 小時一次**。

讓這個研究專案自動推進。**排程 trigger 必須在 Claude Code on the web 介面建立**
...
5. **Trigger**:選 **Schedule** → **每 6 小時一次**(使用者 2026-07-03 拍板;時區自動換算)。
...
更新 `STATE.md` / `knowledge/` / `log/` → commit & push → 結束。下一次(6 小時後)從新斷點接續。

## 你如何掌握進度

- **看 `log/`**:每次執行一個檔,摘要做了什麼。
- **看 `STATE.md`**:永遠是「目前到哪、下一步」。
- **看 commit 歷史**:每次執行至少一筆。
...
## 路線圖(排程依 STATE.md「下一步動作」優先序推進)

第 2 階段四能力(切圖/補圖/mesh/骨架)工具+閘全齊、已串成「PSD→可動 Spine 資產」端到端;
切圖能力經美術真值收斂(W1~W5)。目前開放研究優先序見 `STATE.md`。
